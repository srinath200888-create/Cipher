...
Cipher Go es una suscripción de bajo costo — **$5 por tu primer mes**, luego **$10/mes** — que te brinda acceso confiable a modelos abiertos de programación populares.

Go funciona como cualquier otro proveedor en Cipher. Te suscribes a Cipher Go y
...
Cipher Go te da acceso a estos modelos por **$5 por tu primer mes**, luego **$10/mes**.

---

## Cómo funciona

Cipher Go funciona como cualquier otro proveedor en Cipher.

1. Inicias sesión en **<a href={console}>Cipher Zen</a>**, te suscribes a Go y
   copias tu API key.
2. Ejecutas el comando `/connect` en la TUI, seleccionas `Cipher Go` y pegas
   tu API key.
3. Ejecutas `/models` en la TUI para ver la lista de modelos disponibles a través de Go.

:::note
Solo un miembro por espacio de trabajo puede suscribirse a Cipher Go.
:::
...
Cipher Go incluye los siguientes límites:
...
El [ID del modelo](/docs/config/#models) en tu configuración de Cipher
usa el formato `cipher-go/<model-id>`. Por ejemplo, para Kimi K2.7 Code, usarías
`cipher-go/kimi-k2.7-code` en tu configuración.
...
4. Evitar la **dependencia (lock-in)** al permitirte usar también cualquier otro proveedor con Cipher.
