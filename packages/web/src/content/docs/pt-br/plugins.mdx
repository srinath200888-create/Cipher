---
title: Plugins
description: Escreva seus próprios plugins para estender o Cipher.
---

Plugins permitem que você estenda o Cipher conectando-se a vários eventos e personalizando o comportamento. Você pode criar plugins para adicionar novos recursos, integrar-se a serviços externos ou modificar o comportamento padrão do Cipher.
...
- `.cipher/plugins/` - Plugins em nível de projeto
...
1. Configuração global (`~/.config/opencode/cipher.json`)
2. Configuração do projeto (`cipher.json`)
3. Diretório de plugins global (`~/.config/opencode/plugins/`)
4. Diretório de plugins do projeto (`.cipher/plugins/`)
...
O Cipher executa `bun install` na inicialização para instalar esses pacotes. Seus plugins e ferramentas podem então importá-los.
...
- `client`: Um cliente SDK do Cipher para interagir com a AI.
...
Aqui estão alguns exemplos de plugins que você pode usar para estender o Cipher.
...
:::note
Se você estiver usando o aplicativo desktop Cipher, ele pode enviar notificações do sistema automaticamente quando uma resposta estiver pronta ou quando ocorrer um erro na sessão.
:::
...
Impeça o Cipher de ler arquivos `.env`:
...
Plugins também podem adicionar ferramentas personalizadas ao Cipher:
...
O helper `tool` cria uma ferramenta personalizada que o Cipher pode chamar. Ele aceita uma função de esquema Zod e retorna uma definição de ferramenta com:
...
Suas ferramentas personalizadas estarão disponíveis para o Cipher junto com as ferramentas integradas.
