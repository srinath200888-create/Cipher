...
import { ModelsDevPlugin } from "@cipher-ai/core/plugin/models-dev"
...
describe("ModelsDevPlugin", () => {
  ...
  it.effect("registers key methods for providers with environment variables", () =>
    Effect.acquireUseRelease(
      Effect.sync(() => {
        const previous = {
          path: Flag.CIPHER_MODELS_PATH,
          disabled: Flag.CIPHER_DISABLE_MODELS_FETCH,
        }
        Flag.CIPHER_MODELS_PATH = path.join(import.meta.dir, "fixtures", "models-dev.json")
        Flag.CIPHER_DISABLE_MODELS_FETCH = true
        return previous
      }),
      () =>
        Effect.gen(function* () {
      ...
      (previous) =>
        Effect.sync(() => {
          Flag.CIPHER_MODELS_PATH = previous.path
          Flag.CIPHER_DISABLE_MODELS_FETCH = previous.disabled
        }),
    ),
  )
})
