...
Delete `packages/cipher/src/cli/cmd/tui/plugin/command-shim.ts`.

In `packages/cipher/src/cli/cmd/tui/plugin/api.tsx`, remove:

- the `createCommandShim` import
- the `command: createCommandShim(...)` field from `createTuiApi(...)`

In `packages/cipher/src/cli/cmd/tui/plugin/runtime.ts`, remove:
...
- `bun typecheck` in `packages/cipher`
- TUI plugin loader tests in `packages/cipher` if runtime plugin API wiring changed
