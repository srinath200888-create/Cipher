...
Cipher Go est un abonnement à bas coût — **5 $ pour votre premier mois**, puis **10 $/mois** — qui vous donne un accès fiable aux modèles de codage ouverts populaires.

Go fonctionne comme n'importe quel autre fournisseur dans Cipher. Vous vous abonnez à Cipher Go et obtenez votre clé d'API. C'est **totalement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Cipher.
...
Cipher Go vous donne accès à ces modèles pour **5 $ pour votre premier mois**, puis **10 $/mois**.

---

## Comment ça marche

Cipher Go fonctionne comme n'importe quel autre fournisseur dans Cipher.

1. Vous vous connectez à **<a href={console}>Cipher Zen</a>**, vous vous abonnez à Go et copiez votre clé d'API.
2. Vous exécutez la commande `/connect` dans la TUI, sélectionnez `Cipher Go` et collez votre clé d'API.
3. Exécutez `/models` dans la TUI pour voir la liste des modèles disponibles via Go.

:::note
Un seul membre par espace de travail peut s'abonner à Cipher Go.
:::
...
Cipher Go inclut les limites suivantes :
...
L'[ID de modèle](/docs/config/#models) dans votre configuration Cipher utilise le format `cipher-go/<model-id>`. Par exemple, pour Kimi K2.7 Code, vous utiliseriez `cipher-go/kimi-k2.7-code` dans votre configuration.
...
Nous avons créé Cipher Go pour :
...
4. N'avoir **aucun verrouillage exclusif** en vous permettant d'utiliser n'importe quel autre fournisseur avec Cipher également.
