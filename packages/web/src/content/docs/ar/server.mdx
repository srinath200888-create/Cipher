---
title: الخادم
description: تفاعل مع خادم Cipher عبر HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

يشغّل الأمر `Cipher serve` خادما HTTP دون واجهة ويعرض نقطة نهاية OpenAPI يمكن لعميل Cipher استخدامها.
...
| `--mdns-domain` | اسم نطاق مخصص لخدمة mDNS              | `Cipher.local` |
...
عيّن `CIPHER_SERVER_PASSWORD` لحماية الخادم باستخدام مصادقة HTTP الأساسية. اسم المستخدم افتراضيا هو `Cipher`، أو عيّن `CIPHER_SERVER_USERNAME` لتغييره. ينطبق ذلك على كل من `Cipher serve` و `Cipher web`.
...
عند تشغيل `Cipher` يبدأ تشغيل واجهة terminal تفاعلية (TUI) وخادما. تكون الـ TUI هي
العميل الذي يتحدث إلى الخادم. يوفّر الخادم نقطة نهاية لمواصفة OpenAPI 3.1.
وتُستخدم هذه النقطة أيضا لتوليد [SDK](/docs/sdk).

:::tip
استخدم خادم Cipher للتفاعل مع Cipher برمجيا.
:::

تتيح هذه البنية لـ Cipher دعم عدة عملاء وتمكّنك من التفاعل مع Cipher برمجيا.

يمكنك تشغيل `Cipher serve` لبدء خادم مستقل. إذا كانت واجهة Cipher في terminal (TUI)
قيد التشغيل، فسيبدأ `Cipher serve` خادما جديدا.
...
يمكن استخدام نقطة النهاية [`/tui`](#tui) للتحكم في الـ TUI عبر الخادم. على سبيل المثال، يمكنك تعبئة الموجّه مسبقا أو تشغيله. يُستخدم هذا الإعداد بواسطة ملحقات Cipher لـ [IDE](/docs/ide).
...
يعرض خادم Cipher واجهات API التالية.
