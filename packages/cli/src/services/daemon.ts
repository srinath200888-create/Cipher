...
import { createCipherClient } from "@cipher-ai/sdk/v2/client"
import { ServerAuth } from "@cipher-ai/server/auth"
...
export interface Interface {
  readonly client: () => Effect.Effect<ReturnType<typeof createCipherClient>, unknown>
...
export class Service extends Context.Service<Service, Interface>()("@cipher/cli/Daemon") {}
...
export const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const createClient = Effect.fnUntraced(function* (url: string) {
      return createCipherClient({ baseUrl: url, headers: ServerAuth.headers({ password: yield* password() }) })
    })
    ...
    const client = Effect.fn("cli.daemon.client")(function* () {
      ...
      return createCipherClient({ baseUrl: connection.url, headers: connection.headers })
    })
