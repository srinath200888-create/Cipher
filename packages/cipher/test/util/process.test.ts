...
describe("util.process", () => {
  ...
  test("merges environment overrides", async () => {
    const out = await Process.run(node('process.stdout.write(process.env.CIPHER_TEST ?? "")'), {
      env: {
        CIPHER_TEST: "set",
      },
    })
  ...
  test("uses shell in run on Windows", async () => {
    ...
    const out = await Process.run(["set", "CIPHER_TEST_SHELL"], {
      shell: true,
      env: {
        CIPHER_TEST_SHELL: "ok",
      },
    })

    expect(out.code).toBe(0)
    expect(out.stdout.toString()).toContain("CIPHER_TEST_SHELL=ok")
  })
