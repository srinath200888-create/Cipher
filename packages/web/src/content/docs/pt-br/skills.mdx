...
As habilidades do agente permitem que o Cipher descubra instruções reutilizáveis do seu repositório ou diretório pessoal.
...
O Cipher pesquisa nesses locais:

- Configuração do projeto: `.cipher/skills/<name>/SKILL.md`
...
Para caminhos locais do projeto, o Cipher sobe a partir do seu diretório de trabalho atual até alcançar a árvore de trabalho do git.
Ele carrega qualquer `skills/*/SKILL.md` correspondente em `.cipher/` e qualquer `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` ao longo do caminho.
...
Crie `.cipher/skills/git-release/SKILL.md` assim:
...
O Cipher lista as habilidades disponíveis na descrição da ferramenta `skill`.
...
Controle quais habilidades os agentes podem acessar usando permissões baseadas em padrões em `cipher.json`:
...
**Para agentes embutidos** (em `cipher.json`):
