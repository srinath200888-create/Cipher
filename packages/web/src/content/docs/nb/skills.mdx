...
Agentferdigheter lar Cipher oppdage gjenbrukbare instruksjoner fra repo- eller hjemmekatalogen din.
...
Cipher søker etter disse stedene:

- Prosjektkonfigurasjon: `.cipher/skills/<name>/SKILL.md`
...
For prosjektlokale stier går Cipher opp fra din nåværende arbeidskatalog til den når git-arbeidstreet.
Den laster alle matchende `skills/*/SKILL.md` i `.cipher/` og alle matchende `.claude/skills/*/SKILL.md` eller `.agents/skills/*/SKILL.md` underveis.
...
Lag `.cipher/skills/git-release/SKILL.md` slik:
...
Cipher viser tilgjengelige ferdigheter i `skill`-verktøybeskrivelsen.
...
Kontroller hvilke ferdigheter agenter har tilgang til ved å bruke mønsterbaserte tillatelser i `cipher.json`:
...
**For innebygde agenter** (i `cipher.json`):
