---
title: LSP Servers
description: Cipher integrates with your LSP servers.
---

Cipher can integrate with Language Server Protocol (LSP) servers to use diagnostics as feedback for the agent.

---

## Built-in

Cipher comes with several built-in LSP servers for popular languages:
...
:::note
You can disable automatic LSP server downloads by setting the `CIPHER_DISABLE_LSP_DOWNLOAD` environment variable to `true`.
:::
...
When LSP is enabled and Cipher opens a file, it:
...
You can enable and customize LSP servers through the `lsp` section in your Cipher config.
