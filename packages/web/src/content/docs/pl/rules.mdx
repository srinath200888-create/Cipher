---
title: Zasady
description: Ustaw niestandardowe instrukcje dla Cipher.
---

Możesz dostarczyć niestandardowe instrukcje do Cipher, tworząc plik `AGENTS.md`. Jest to podobne do zasad Cursora. Zawiera instrukcje, które zostaną uwzględnione w kontekście LLM, aby dostosować jego zachowanie do konkretnego projektu.
...
Cipher obsługuje także odczytywanie pliku `AGENTS.md` z wielu lokalizacji. A to służy różnym celom.
...
Możesz także mieć reguły globalne w pliku `~/.config/opencode/AGENTS.md`. Jest to stosowane we wszystkich sesjach Cipher.
...
W przypadku użytkowników migrujących z Claude Code Cipher obsługuje konwencje plików Claude Code jako rozwiązania awaryjne:
...
Po uruchomieniu Cipher szuka plików reguł w następującej kolejności:
...
Możesz określić niestandardowe pliki instrukcji w swoim `cipher.json` lub globalnym `~/.config/opencode/cipher.json`. Dzięki temu Ty i Twój zespół możecie ponownie wykorzystać istniejące reguły, zamiast duplikować je w pliku AGENTS.md.
...
Chociaż Cipher nie analizuje automatycznie odniesień do plików w `AGENTS.md`, możesz osiągnąć podobną funkcjonalność na dwa sposoby:

### Używanie cipher.json

Zalecanym podejściem jest użycie pola `instructions` w `cipher.json`:
...
Możesz nauczyć Cipher czytać pliki zewnętrzne, podając wyraźne instrukcje w swoim `AGENTS.md`. Oto praktyczny przykład:
...
- Upewnij się, że Cipher ładuje pliki tylko wtedy, gdy są potrzebne do konkretnego zadania

:::tip
W przypadku monorepo lub projektów ze wspólnymi standardami użycie `cipher.json` ze wzorami globalnymi (takimi jak `packages/*/AGENTS.md`) jest łatwiejsze w utrzymaniu niż instrukcje ręczne.
:::
