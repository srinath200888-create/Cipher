...
透過 cipher，您可以從多個內建主題中進行選擇，使用能自動適配終端機主題的主題，或者定義您自己的自訂主題。

預設情況下，Cipher 使用我們自己的 `Cipher` 主題。
...
Cipher 支援靈活的基於 JSON 的主題系統，讓使用者可以輕鬆建立和自訂主題。
...
3. **專案根目錄** — 定義在 `<project-root>/.cipher/themes/*.json`
4. **當前工作目錄** — 定義在 `./.cipher/themes/*.json`
