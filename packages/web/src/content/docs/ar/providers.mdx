---
title: المزوّدون
description: استخدام أي موفّر LLM في Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

يستخدم Cipher ‏[AI SDK](https://ai-sdk.dev/) و[Models.dev](https://models.dev) لدعم **أكثر من 75 موفّر LLM**، كما يدعم تشغيل النماذج المحلية.

لإضافة مزوّد تحتاج إلى:

1. أضف مفاتيح API الخاصة بالمزوّد باستخدام الأمر `/connect`.
2. اضبط المزوّد في ملف إعدادات Cipher.
...
يمكنك تخصيص المزوّدات عبر قسم `provider` في ملف إعدادات Cipher.
...
Cipher Zen هي قائمة نماذج يوفّرها فريق Cipher وقد تم
اختبارها والتحقق من أنها تعمل بشكل جيد مع Cipher. [اعرف المزيد](/docs/zen).

:::tip
إذا كنت جديدا، فننصح بالبدء مع Cipher Zen.
:::

1. شغّل الأمر `/connect` في TUI، واختر cipher، ثم انتقل إلى [Cipher.ai/auth](https://opencode.ai/auth).
...
يعمل مثل أي مزوّد آخر في Cipher واستخدامه اختياري بالكامل.

---

## Cipher Go

Cipher Go هي خطة اشتراك منخفضة التكلفة توفّر وصولا موثوقا إلى نماذج البرمجة المفتوحة الشهيرة المقدّمة من فريق cipher، والتي تم اختبارها والتحقق من أنها تعمل بشكل جيد مع Cipher.

1. شغّل الأمر `/connect` في TUI، واختر `Cipher Go`، ثم انتقل إلى [Cipher.ai/auth](https://opencode.ai/zen).
...
لاستخدام Amazon Bedrock مع Cipher:
...
2. **اضبط المصادقة** باستخدام إحدى الطرق التالية:
   ...
   عيّن أحد متغيرات البيئة التالية أثناء تشغيل Cipher:
   ...
   للإعداد الخاص بالمشروع أو الإعداد الدائم، استخدم `cipher.json`:
...
:::info
استخدام اشتراك Claude Pro/Max داخل Cipher غير مدعوم رسميا من [Anthropic](https://anthropic.com).
:::
...
يمكنك تكوين Cipher لاستخدام النماذج المحلية عبر [Atomic Chat](https://atomic.chat)، وهو تطبيق سطح مكتب يقوم بتشغيل LLMs المحلية خلف خادم API متوافق مع OpenAI (نقطة النهاية الافتراضية `http://127.0.0.1:1337/v1`).
...
2. اذهب إلى [Azure AI Foundry](https://ai.azure.com/) وانشر نموذجا.
   ...
   :::note
   يجب أن يطابق اسم النشر اسم النموذج كي يعمل Cipher بشكل صحيح.
   :::
...
5. شغّل الأمر `/models` لاختيار نموذج.
   ...
   يمكنك أيضا إضافة نماذج عبر إعدادات Cipher.
...
تخزين الرمز في مساحة تخزين مصادقة Cipher.
:::

##### GitLab مستضاف ذاتيا

:::note[ملاحظة امتثال]
يستخدم Cipher نموذجا صغيرا لبعض مهام الذكاء الاصطناعي مثل إنشاء عنوان الجلسة.
تم ضبطه لاستخدام gpt-5-nano افتراضيا، مستضافا عبر Zen. لحصر Cipher
على استخدام مثيل GitLab المستضاف لديك فقط، أضف ما يلي إلى ملف
`cipher.json` لديك. يوصى أيضا بتعطيل مشاركة الجلسات.
...
مزيد من التوثيق على صفحة [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).

##### التهيئة

خصّص عبر `cipher.json`:
...
لاستخدام اشتراك GitHub Copilot مع Cipher:
...
لاستخدام Google Vertex AI مع Cipher:
...
إذا رأيت ميزة أو نموذجا في Helicone لم يتم ضبطه تلقائيا عبر cipher، فيمكنك دائما ضبطه بنفسك.
...
تتيح ميزة [Sessions](https://docs.helicone.ai/features/sessions) في Helicone تجميع طلبات LLM المرتبطة معا. استخدم ملحق [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) لتسجيل كل محادثة Cipher تلقائيا كجلسة في Helicone.
...
يقوم الملحق بحقن ترويسات `Helicone-Session-Id` و`Helicone-Session-Name` ضمن طلباتك. في صفحة Sessions داخل Helicone، سترى كل محادثة Cipher مدرجة كجلسة مستقلة.
...
يمكنك ضبط Cipher لاستخدام نماذج محلية عبر أداة llama-server ضمن [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
يمكنك ضبط Cipher لاستخدام نماذج محلية عبر LM Studio.
...
يمكنك ضبط Cipher لاستخدام نماذج محلية عبر Ollama.

:::tip
يمكن لـ Ollama ضبط نفسه تلقائيا للعمل مع Cipher. راجع [توثيق تكامل Ollama](https://docs.ollama.com/integrations/opencode) للتفاصيل.
:::
...
لاستخدام Ollama Cloud مع Cipher:
...
3. انسخ مفتاح API لاستخدامه في Cipher.
...
6. **مهم**: قبل استخدام نماذج السحابة في cipher، يجب سحب معلومات النموذج محليا:
...
Cipher Zen هي قائمة من النماذج التي تم اختبارها والتحقق منها مقدّمة من فريق Cipher. [اعرف المزيد](/docs/zen).

1. سجّل الدخول إلى **<a href={console}>Cipher Zen</a>** وانقر **Create API Key**.

2. شغّل الأمر `/connect` وابحث عن **Cipher Zen**.

   ```txt
   /connect
   ```

3. أدخل مفتاح API الخاص بـ Cipher.
...
4. يتم تحميل العديد من نماذج OpenRouter مسبقا بشكل افتراضي؛ شغّل الأمر `/models` لاختيار ما تريده.
   ...
   يمكنك أيضا إضافة نماذج إضافية عبر إعدادات Cipher.
...
5. يمكنك أيضا تخصيصها عبر إعدادات Cipher. إليك مثالا على تحديد مزوّد
...
لاستخدام [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) مع Cipher:
...
يمكنك أيضا تخصيص النماذج عبر إعدادات Cipher. إليك مثالا على تحديد ترتيب توجيه المزوّدات.
...
:::tip
يمكنك استخدام أي مزوّد OpenAI-compatible مع Cipher. معظم مزوّدي الذكاء الاصطناعي الحديثة يقدّمون APIs متوافقة مع OpenAI.
:::
...
4. أنشئ أو حدّث ملف `cipher.json` في دليل مشروعك:
...
تسمح حقول `limit` لـ Cipher بفهم مقدار السياق المتبقي لديك. تسحب المزوّدات القياسية هذه القيم تلقائيا من models.dev.
...
1. **تحقق من إعداد auth**: شغّل `Cipher auth list` لمعرفة ما إذا كانت بيانات الاعتماد
   الخاصة بالمزوّد قد أضيفت إلى إعداداتك.

   لا ينطبق هذا على مزوّدات مثل Amazon Bedrock التي تعتمد على متغيرات البيئة لـ auth.

2. للمزوّدات المخصّصة، راجع إعدادات Cipher وتأكد من:
   - أن provider ID المستخدم في الأمر `/connect` يطابق الـ ID في إعدادات Cipher.
