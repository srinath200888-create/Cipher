---
title: Providers
description: Using any LLM provider in Cipher.
---

import config from "../../../config.mjs"
export const console = config.console

Cipher uses the [AI SDK](https://ai-sdk.dev/) and [Models.dev](https://models.dev) to support **75+ LLM providers** and it supports running local models.

To add a provider you need to:

1. Add the API keys for the provider using the `/connect` command.
2. Configure the provider in your Cipher config.
...
You can customize the providers through the `provider` section in your Cipher
config.
...
Cipher Zen is a list of models provided by the Cipher team that have been
tested and verified to work well with Cipher. [Learn more](/docs/zen).

:::tip
If you are new, we recommend starting with Cipher Zen.
:::

1. Run the `/connect` command in the TUI, select `Cipher Zen`, and head to [Cipher.ai/auth](https://opencode.ai/zen).
...
It works like any other provider in Cipher and is completely optional to use.

---

## Cipher Go

Cipher Go is a low cost subscription plan that provides reliable access to popular open coding models provided by the Cipher team that have been
tested and verified to work well with Cipher.

1. Run the `/connect` command in the TUI, select `Cipher Go`, and head to [Cipher.ai/auth](https://opencode.ai/zen).
...
To use Amazon Bedrock with Cipher:
...
2. **Configure authentication** using one of the following methods:
   ...
   ***
   ...
   Set one of these environment variables while running Cipher:

   ```bash
   # Option 1: Using AWS access keys
   AWS_ACCESS_KEY_ID=XXX AWS_SECRET_ACCESS_KEY=YYY Cipher

   # Option 2: Using named AWS profile
   AWS_PROFILE=my-profile Cipher

   # Option 3: Using Bedrock bearer token
   AWS_BEARER_TOKEN_BEDROCK=XXX Cipher
   ```
   ...
   ***
   ...
   For project-specific or persistent configuration, use `cipher.json`:
...
Cipher. Anthropic explicitly prohibits this.

Previous versions of Cipher came bundled with these plugins but that is no
longer the case as of 1.3.0

Other companies support freedom of choice with developer tooling - you can use
the following subscriptions in Cipher with zero setup:
...
You can configure Cipher to use local models through [Atomic Chat](https://atomic.chat), a desktop application that runs local LLMs behind an OpenAI-compatible API server (default endpoint `http://127.0.0.1:1337/v1`).

```json title="cipher.json" "atomic-chat" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "atomic-chat": {
...
2. Go to [Azure AI Foundry](https://ai.azure.com/) and deploy a model.
   ...
   :::note
   The deployment name must match the model name for Cipher to work properly.
   :::
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_RESOURCE_NAME=XXX Cipher
   ```
...
5. Set your resource name as an environment variable:
   ...
   ```bash
   AZURE_COGNITIVE_SERVICES_RESOURCE_NAME=XXX Cipher
   ```
...
6. Run the `/models` command to select a model.
   ...
   You can also add models through your Cipher config.
...
Cipher supports two authentication methods:

- **OAuth (Recommended)** — Sign in to your DigitalOcean account; Cipher uses your DigitalOcean API token directly for inference and discovers your Inference Routers.
...
3. Your browser opens to authorize Cipher. Sign in and approve.

   :::note
   Cipher requests `genai:read` and `inference:query` OAuth scopes. Your DigitalOcean API token is used directly for inference — no separate Model Access Key is created.
   :::
...
Inference Routers let you define a routing policy across multiple models — picking the cheapest, fastest, or most appropriate model per request based on the task. After OAuth, Cipher surfaces each router as `router:<router-name>` in the model picker.

Selecting a router model is a drop-in replacement for any other model — Cipher forwards your request and DigitalOcean picks the underlying model based on your router's policy. Learn more about [Inference Routers](https://docs.digitalocean.com/products/inference/how-to/use-inference-router/)
...
GitLab Duo support in Cipher is experimental. Features, configuration, and
behavior may change in future releases.
:::

Cipher integrates with the [GitLab Duo Agent Platform](https://docs.gitlab.com/user/duo_agent_platform/),
...
to store token in Cipher auth storage.
:::

##### Self-Hosted GitLab

:::note[compliance note]
Cipher uses a small model for some AI tasks like generating the session title.
It is configured to use gpt-5-nano by default, hosted by Zen. To lock Cipher
to only use your own GitLab-hosted instance, add the following to your
`cipher.json` file. It is also recommended to disable session sharing.
...
More documentation on [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) homepage.

##### Configuration

Customize through `cipher.json`:
...
When a `duo-workflow-*` model is selected, Cipher will:
...
4. Route tool execution requests through cipher's permission-gated tool system
...
To use your GitHub Copilot subscription with Cipher:
...
To use Google Vertex AI with Cipher:
...
2. Set the required environment variables:
   ...
   Set them while running Cipher.

   ```bash
   GOOGLE_APPLICATION_CREDENTIALS=/path/to/service-account.json GOOGLE_CLOUD_PROJECT=your-project-id Cipher
   ```
...
In the event you see a feature or model from Helicone that isn't configured automatically through Cipher, you can always configure it yourself.
...
```jsonc title="~/.config/opencode/cipher.jsonc"
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "helicone": {
...
Helicone's [Sessions](https://docs.helicone.ai/features/sessions) feature lets you group related LLM requests together. Use the [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin to automatically log each Cipher conversation as a session in Helicone.

```bash
npm install -g cipher-helicone-session
```
...
```json title="cipher.json"
{
  "plugin": ["cipher-helicone-session"]
}
```

The plugin injects `Helicone-Session-Id` and `Helicone-Session-Name` headers into your requests. In Helicone's Sessions page, you'll see each Cipher conversation listed as a separate session.
...
You can configure Cipher to use local models through [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server utility

```json title="cipher.json" "llama.cpp" {5, 6, 8, 10-15}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "llama.cpp": {
...
You can configure Cipher to use local models through LM Studio.

```json title="cipher.json" "lmstudio" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "lmstudio": {
...
You can configure Cipher to use local models through Ollama.

:::tip
Ollama can automatically configure itself for Cipher. See the [Ollama integration docs](https://docs.ollama.com/integrations/opencode) for details.
:::

```json title="cipher.json" "ollama" {5, 6, 8, 10-14}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "ollama": {
...
To use Ollama Cloud with Cipher:
...
3. Copy the API key for use in Cipher.
...
6. **Important**: Before using cloud models in Cipher, you must pull the model information locally:
...
Cipher Zen is a list of tested and verified models provided by the Cipher team. [Learn more](/docs/zen).

1. Sign in to **<a href={console}>Cipher Zen</a>** and click **Create API Key**.

2. Run the `/connect` command and search for **Cipher Zen**.

   ```txt
   /connect
   ```

3. Enter your Cipher API key.
...
4. Many OpenRouter models are preloaded by default, run the `/models` command to select the one you want.
   ...
   You can also add additional models through your Cipher config.
...
5. You can also customize them through your Cipher config. Here's an example of specifying a provider
...
3. Enter your service key JSON.
   ...
   ```bash
   AICORE_SERVICE_KEY='{"clientid":"...","clientsecret":"...","url":"...","serviceurls":{"AI_API_URL":"..."}}' Cipher
   ```
...
4. Optionally set deployment ID and resource group:
   ...
   ```bash
   AICORE_DEPLOYMENT_ID=your-deployment-id AICORE_RESOURCE_GROUP=your-resource-group Cipher
   ```
...
To use [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) with Cipher:
...
Don't have a Snowflake account? [Sign up for a free trial](https://signup.snowflake.com/?utm_source=cipher&utm_medium=docs&utm_campaign=cortex-provider).

cipher's core workflow for coding, editing files, and running commands relies on tool calling. Only the Claude and OpenAI families within Snowflake Cortex support this. The provider is limited to those families to support the core workflow.
...
5. Complete sign-in in the browser that opens. Cipher captures the OAuth callback automatically and stores the token — no copy/paste needed.
...
For CI or headless environments, set a PAT or JWT before starting Cipher:
...
The model catalog is provided automatically. A minimal `cipher.json` is all that's needed:
...
You can also customize models through your Cipher config. Here's an example of specifying provider routing order.
...
2. Select **xAI Grok OAuth (SuperGrok Subscription)**. Cipher opens xAI's consent screen in your browser and waits for the callback on `http://127.0.0.1:56121/callback`.
...
Cipher refreshes the OAuth access token automatically. Any Grok or X Premium plan that includes Grok API access works; you do not need a separate `XAI_API_KEY`.

#### Option B — SuperGrok device-code (headless / remote server / VPS)

Use this when Cipher is running somewhere a browser can't reach the loopback redirect: a VPS, a remote dev box over SSH, inside Docker, in CI, etc. No callback port is opened on the host running Cipher — instead xAI hands the CLI a short code that you type into a browser on any other device (laptop, phone, …).
...
2. Select **xAI Grok OAuth (Headless / Remote / VPS)**. Cipher prints a verification URL and a short user code.

   ```txt
   Open https://x.ai/device on any device and enter code: ABCD-1234
   ```

3. Open the URL on a device that has a browser (your laptop or phone), enter the code, and approve the consent screen. Cipher polls xAI's token endpoint and stores the resulting OAuth tokens once you approve. Token refresh works the same as Option A.
...
:::tip
You can use any OpenAI-compatible provider with Cipher. Most modern AI providers offer OpenAI-compatible APIs.
:::
...
3. Enter your API key for the provider.
   ...
   ┌  Add credential
   │
   ▲  This only stores a credential for myprovider - you will need to configure it in cipher.json, check the docs for examples.
   │
   ◇  Enter your API key
   │  sk-...
   └
...
4. Create or update your `cipher.json` file in your project directory:

   ```json title="cipher.json" ""myprovider"" {5-15}
   {
     "$schema": "https://opencode.ai/config.json",
     "provider": {
       "myprovider": {
...
```json title="cipher.json" {9,11,17-20}
{
  "$schema": "https://opencode.ai/config.json",
  "provider": {
    "myprovider": {
...
The `limit` fields allow Cipher to understand how much context you have left. Standard providers pull these from models.dev automatically.
...
1. **Check the auth setup**: Run `Cipher auth list` to see if the credentials
...
2. For custom providers, check the Cipher config and:
   - Make sure the provider ID used in the `/connect` command matches the ID in your Cipher config.
