---
title: Server
description: Komunicirajte s Cipher serverom preko HTTP-a.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komanda `Cipher serve` pokrece headless HTTP server koji izlaže OpenAPI endpoint koji Cipher klijent moze koristiti.
...
| `--mdns-domain` | Prilagodeni domen za mDNS servis    | `Cipher.local` |
...
Postavite `CIPHER_SERVER_PASSWORD` da zastitite server HTTP basic auth mehanizmom. Korisnicko ime je po defaultu `Cipher`, ili postavite `CIPHER_SERVER_USERNAME` za nadjacavanje. Ovo vazi i za `Cipher serve` i za `Cipher web`.
...
Kada pokrenete `Cipher`, pokrecu se TUI i server. TUI je klijent koji komunicira sa serverom. Server izlaže OpenAPI 3.1 spec endpoint koji se koristi i za generisanje [SDK-a](/docs/sdk).

:::tip
Koristite Cipher server za programsku interakciju sa cipher-om.
:::

Ova arhitektura omogucava Cipher podrsku za vise klijenata i programsku interakciju.

Mozete pokrenuti `Cipher serve` da startate standalone server. Ako je Cipher TUI vec pokrenut, `Cipher serve` ce pokrenuti novi server.
...
Endpoint [`/tui`](#tui) mozete koristiti za upravljanje TUI-jem kroz server. Na primjer, mozete unaprijed popuniti ili pokrenuti prompt. Ovaj setup koriste Cipher [IDE](/docs/ide) pluginovi.
...
Cipher server izlaže sljedece API-je.
