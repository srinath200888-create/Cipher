---
title: Konfiguration
description: Ved at bruge Cipher JSON konfig.
---

Du kan konfigurere Cipher ved hjælp af en JSON-konfigurationsfil.

---

## Format

Cipher understøtter både **JSON** og **JSONC** (JSON med kommentarer) formater.
...
1. **Fjernkonfiguration** (fra `.well-known/cipher`) - organisatoriske standardindstillinger
2. **Global config** (`~/.config/opencode/cipher.json`) - brugerpræferencer
3. **Tilpasset konfiguration** (`CIPHER_CONFIG` env var) - tilpassede tilsidesættelser
4. **Project config** (`cipher.json` i projekt) - projektspecifikke indstillinger
5. **`.opencode` mapper** - agent, kommandoer, plugins
6. **Inline config** (`CIPHER_CONFIG_CONTENT` env var) - runtime tilsidesættelser
...
:::note
`.opencode` og `~/.config/cipher` mapperne bruger **flertalsnavne** for undermapper: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` og `themes/`. Enkelte navne (f.eks. `agent/`) understøtter også bagudkompatibilitet.
:::
...
Organisationer kan levere standardkonfiguration via `.well-known/cipher`-slutpunktet. Dette hentes automatisk, når du godkender med en udbyder, der understøtter det.
...
Placer din globale cipher-konfiguration i `~/.config/opencode/cipher.json`. Brug global konfiguration til brugerdækkende præferencer som temaer, udbydere eller nøglebindinger.
...
Tilføj `cipher.json` i dit projektrod. Project config har den højeste forrang blandt standard config-filer - den tilsidesætter både globale og eksterne config.

:::tip
Placer projektspecifik konfiguration i roden af ​​dit projekt.
:::

Når Cipher starter op, søger den efter en konfigurationsfil i den aktuelle mappe eller går op til den nærmeste Git-mappe.
...
Angiv en brugerdefineret konfigurationsfilsti ved hjælp af miljøvariablen `CIPHER_CONFIG`.
...
Angiv en brugerdefineret konfigurationsmappe ved hjælp af `CIPHER_CONFIG_DIR`
...
Konfigurationsfilen har et skema, der defineres i [**`Cipher.ai/config.json`**](https://opencode.ai/config.json).
...
Du kan konfigurere serverindstillinger for kommandoerne `Cipher serve` og `Cipher web` gennem indstillingen `server`.
...
- `mdns` - Aktiver mDNS-tjenesteopdagelse. Dette gør det muligt for andre enheder på netværket at opdage din cipher-server.
- `mdnsDomain` - Brugerdefineret domænenavn til mDNS-tjeneste. Som standard er `Cipher.local`. Nyttigt til at køre flere forekomster på det samme netværk.
...
Du kan konfigurere de udbydere og modeller, du vil bruge i din cipher-konfiguration, gennem mulighederne `provider`, `model` og `small_model`.
...
Indstillingen `small_model` konfigurerer en separat model til lette opgaver som titelgenerering. Som standard forsøger Cipher at bruge en billigere model, hvis en er tilgængelig fra din udbyder, ellers falder den tilbage til din hovedmodel.
...
Du kan konfigurere det tema, du vil bruge i din cipher-konfiguration, gennem indstillingen `theme`.
...
Du kan også definere agenter ved at bruge markdown-filer i `~/.config/opencode/agents/` eller `.cipher/agents/`. [Learn more here](/docs/agents).
...
Standardagenten skal være en primær agent (ikke en subagent). Dette kan være en indbygget agent som `"build"` eller `"plan"`, eller en [custom agent](/docs/agents), du har defineret. Hvis den angivne agent ikke eksisterer eller er en underagent, vil Cipher falde tilbage til `"build"` med en advarsel.

Denne indstilling gælder på tværs af alle grænseflader: TUI, CLI (`Cipher run`), desktop-app og GitHub Action.
...
Du kan også definere kommandoer ved hjælp af markdown-filer i `~/.config/opencode/commands/` eller `.cipher/commands/`. [Learn more here](/docs/commands).
...
Cipher vil automatisk downloade alle nye opdateringer, når den starter op. Du kan deaktivere dette med indstillingen `autoupdate`.
...
Som standard Cipher **tillader alle operationer** uden at kræve eksplicit godkendelse. Du kan ændre dette ved at bruge indstillingen `permission`.
...
[Plugins](/docs/plugins) udvide Cipher med brugerdefinerede værktøjer, kroge og integrationer.

Placer plugin-filer i `.cipher/plugins/` eller `~/.config/opencode/plugins/`. Du kan også indlæse plugins fra npm gennem indstillingen `plugin`.
...
Dette er nyttigt, når du vil begrænse Cipher til kun at bruge specifikke udbydere i stedet for at deaktivere dem én efter én.
