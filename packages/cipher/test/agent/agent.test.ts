...
it.instance("plan agent denies edits except .cipher/plans/*", () =>
  Effect.gen(function* () {
    ...
    expect(Permission.evaluate("edit", ".cipher/plans/foo.md", plan!.permission).action).toBe("allow")
  }),
)
...
it.instance(
  ...
  () =>
    Effect.gen(function* () {
      ...
      const skillDir = path.join(test.directory, ".cipher", "skill", "perm-skill")
...
`,
      ...
      const home = process.env.CIPHER_TEST_HOME
      process.env.CIPHER_TEST_HOME = test.directory
