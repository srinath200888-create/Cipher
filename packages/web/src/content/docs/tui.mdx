---
title: TUI
description: Using the Cipher terminal user interface.
---

import { Tabs, TabItem } from "@astrojs/starlight/components"

Cipher provides an interactive terminal interface or TUI for working on your projects with an LLM.

Running Cipher starts the TUI for the current directory.
...
When using the Cipher TUI, you can type `/` followed by a command name to quickly execute actions. For example:
...
Add a provider to Cipher. Allows you to select from available providers and add their API keys.
...
This is separate from `cipher.json`, which configures server/runtime behavior.
...
- `leader_timeout` - Controls how long Cipher waits after the leader key. Defaults to `2000`.
...
Use `CIPHER_TUI_CONFIG` to load a custom TUI config path.
...
- `sound_pack` - Sound pack ID to use. Defaults to `Cipher.default`.
