...
import { createOpencode } from "@cipher-ai/sdk"
...
`)
  ...
  const cipher = await createOpencode({ port: 0 })
  ...
  try {
    ...
    const session = await cipher.client.session.create()
    const result = await cipher.client.session
      .prompt({
