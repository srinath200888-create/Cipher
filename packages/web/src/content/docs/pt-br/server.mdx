---
title: Servidor
description: Interaja com o servidor Cipher via HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

O comando `Cipher serve` executa um servidor HTTP sem cabeça que expõe um endpoint OpenAPI que um cliente Cipher pode usar.
...
```bash
Cipher serve [--port <number>] [--hostname <string>] [--cors <origin>]
```
...
| `--mdns-domain` | Nome de domínio personalizado para o serviço mDNS | `Cipher.local` |
| `--cors`        | Origens adicionais de navegador a permitir        | `[]`             |

`--cors` pode ser passado várias vezes:

```bash
Cipher serve --cors http://localhost:5173 --cors https://app.example.com
```
...
Defina `CIPHER_SERVER_PASSWORD` para proteger o servidor com autenticação básica HTTP. O nome de usuário padrão é `Cipher`, ou defina `CIPHER_SERVER_USERNAME` para substituí-lo. Isso se aplica tanto ao `Cipher serve` quanto ao `Cipher web`.

```bash
CIPHER_SERVER_PASSWORD=your-password Cipher serve
```
...
Quando você executa `Cipher`, ele inicia um TUI e um servidor. Onde o TUI é o cliente que se comunica com o servidor. O servidor expõe um endpoint de especificação OpenAPI 3.1. Este endpoint também é usado para gerar um [SDK](/docs/sdk).

:::tip
Use o servidor Cipher para interagir com o Cipher programaticamente.
:::

Essa arquitetura permite que o Cipher suporte múltiplos clientes e permite que você interaja com o Cipher programaticamente.

Você pode executar `Cipher serve` para iniciar um servidor autônomo. Se você tiver o TUI do Cipher em execução, `Cipher serve` iniciará um novo servidor.
...
O endpoint [`/tui`](#tui) pode ser usado para controlar o TUI através do servidor. Por exemplo, você pode preencher ou executar um prompt. Essa configuração é usada pelos plugins do Cipher [IDE](/docs/ide).
...
O servidor Cipher expõe as seguintes APIs.
