---
title: Server
description: Interager med cipher-serveren over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `Cipher serve` kører en hovedløs HTTP-server, som afslører et OpenAPI-endpoint, som en cipher-klient kan bruge.
...
| `--mdns-domain` | Brugerdefineret domænenavn for mDNS-tjeneste | `Cipher.local` |
...
Indstil `CIPHER_SERVER_PASSWORD` for at beskytte serveren med HTTP grundlæggende godkendelse. Brugernavnet er som standard `Cipher`, eller indstil `CIPHER_SERVER_USERNAME` for at tilsidesætte det. Dette gælder både `Cipher serve` og `Cipher web`.
...
Når du kører `Cipher` starter den en TUI og en server. Hvor TUI er
...
:::tip
Brug cipher-serveren til at interagere med Cipher programmatisk.
:::

Denne arkitektur lader Cipher understøtte flere klienter og lader dig interagere med Cipher programmatisk.

Du kan køre `Cipher serve` for at starte en selvstændig server. Hvis du har
Cipher TUI kørende, vil `Cipher serve` starte en ny server.
...
[`/tui`](#tui) endpointet kan bruges til at køre TUI gennem serveren. Du kan for eksempel forududfylde eller køre en forespørgsel. Denne opsætning bruges af Cipher [IDE](/docs/ide) plugins.
...
cipher-serveren viser følgende API'er.
