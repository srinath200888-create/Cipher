...
Le skill degli agenti permettono a Cipher di individuare istruzioni riutilizzabili dal tuo repo o dalla home directory.
...
Cipher cerca in queste posizioni:

- Config di progetto: `.cipher/skills/<name>/SKILL.md`
...
Per i percorsi locali al progetto, Cipher risale dalla directory di lavoro corrente finche' non raggiunge il worktree git.
Carica qualsiasi `skills/*/SKILL.md` corrispondente in `.cipher/` e qualsiasi `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` corrispondente lungo il percorso.
...
Crea `.cipher/skills/git-release/SKILL.md` cosi':
...
Cipher elenca le skill disponibili nella descrizione dello strumento `skill`.
...
Controlla a quali skill gli agenti possono accedere usando permessi basati su pattern in `cipher.json`:
...
**Per agenti integrati** (in `cipher.json`):
