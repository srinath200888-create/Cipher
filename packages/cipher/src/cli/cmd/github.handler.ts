...
const AGENT_USERNAME = "cipher-agent[bot]"
  ...
  yield* Effect.promise(async () => {
    {
      ...
      async function installGitHubApp() {
        ...
        // Open browser
        const url = "https://github.com/apps/cipher-agent"
        const command =
        ...
        async function getInstallation() {
          return await fetch(`https://api.cipher.ai/get_github_app_installation?owner=${app.owner}&repo=${app.repo}`)
            .then((res) => res.json())
...
export const githubRun = Effect.fn("Cli.github.run")(function* (args: { event?: string; token?: string }) {
  ...
  yield* Effect.promise(async () => {
    ...
    const shareBaseUrl = isMock ? "https://dev.cipher.ai" : "https://opencode.ai"
    ...
    try {
      ...
      console.log("cipher session", session.id)
    ...
    function normalizeOidcBaseUrl(): string {
      ...
      if (!value) return "https://api.cipher.ai"
    ...
    async function chat(message: string, files: PromptFiles = []) {
      console.log("Sending message to cipher...")
    ...
    async function getOidcToken() {
      try {
        return await core.getIDToken("cipher-github-action")
      } catch (error) {
    ...
    function footer(opts?: { image?: boolean }) {
      const image = (() => {
        ...
        return `<a href="${shareBaseUrl}/s/${shareId}"><img width="200" alt="${titleAlt}" src="https://social-cards.sst.dev/cipher-share/${title64}.png?model=${providerID}/${modelID}&version=${session.version}&id=${shareId}" /></a>\n`
      })()
      const shareUrl = shareId ? `[cipher session](${shareBaseUrl}/s/${shareId})&nbsp;&nbsp;|&nbsp;&nbsp;` : ""
  ...
  repository(owner: $owner, name: $repo) {
    ...
    function buildPromptDataForIssue(issue: GitHubIssue) {
      ...
      return [
        ...
        "- Git push and PR creation are handled AUTOMATICALLY by the cipher infrastructure after your response",
