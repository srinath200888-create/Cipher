...
import type { SessionNotFoundError } from "@cipher-ai/sdk/v2/client"
...
describe("parseReadableConfigInvalidError", () => {
  test("formats issues with file path", () => {
    ...
    expect(result).toBe(
      ["Arquivo de config em cipher.config.ts invalido: settings.host: Required", "mode: Invalid"].join("\n"),
    )
  })
