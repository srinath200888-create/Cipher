...
تتيح مهارات الوكيل لـ Cipher اكتشاف تعليمات قابلة لإعادة الاستخدام من مستودعك أو من دليل المنزل.
...
يبحث Cipher في هذه المواقع:

- إعدادات المشروع: `.cipher/skills/<name>/SKILL.md`
...
بالنسبة لمسارات المشروع المحلية، يتدرج Cipher صعوداً من دليل العمل الحالي حتى يصل إلى `git worktree`.
ويحمّل أي ملفات مطابقة لـ `skills/*/SKILL.md` داخل `.cipher/` وأي ملفات مطابقة لـ `.claude/skills/*/SKILL.md` أو `.agents/skills/*/SKILL.md` على طول الطريق.
...
أنشئ `.cipher/skills/git-release/SKILL.md` بهذا الشكل:
...
يعرض Cipher المهارات المتاحة في وصف أداة `skill`.
...
تحكم في المهارات التي يمكن للوكلاء الوصول إليها باستخدام صلاحيات مبنية على أنماط داخل `cipher.json`:
...
**للوكلاء المدمجين** (في `cipher.json`):
