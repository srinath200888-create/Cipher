...
import type { LLMClientShape } from "@cipher-ai/llm/route"
...
function statusWithFetch(
  ...
  if (providerID !== "openai" && providerID !== "anthropic" && !providerID.startsWith("cipher"))
    return { type: "unsupported", reason: "provider is not openai, cipher, or anthropic" }
...
export function stream(input: StreamInput): StreamResult {
  ...
  // Integration point with @cipher-ai/llm: native-request lowers session data
...
export function nativeTools(tools: Record<string, Tool>, input: Pick<StreamInput, "messages" | "abort">) {
  return Object.fromEntries(
    Object.entries(tools).map(([name, item]) => [
      name,
      // Tool execution remains cipher-owned. The native runtime only adapts
      // the @cipher-ai/llm tool call back into the AI SDK Tool.execute shape.
      NativeTool.make({
