---
title: LSP Server
description: Cipher lässt sich in Ihren LSP-Server integrieren.
---

Cipher kann sich in Language Server Protocol (LSP)-Server integrieren, um Diagnosen als Feedback für den Agenten zu verwenden.

---

## Integriert

Cipher verfügt über mehrere integrierte LSP-Server für gängige Sprachen:
...
:::note
Sie können LSP-Server-Downloads automatisch deaktivieren, indem Sie die Umgebungsvariable `CIPHER_DISABLE_LSP_DOWNLOAD` auf `true` setzen.
:::
...
Wenn LSP aktiviert ist und Cipher eine Datei öffnet, geschieht Folgendes:
...
Sie können LSP-Server über den Abschnitt `lsp` in Ihrer cipher-Konfiguration aktivieren und anpassen.
