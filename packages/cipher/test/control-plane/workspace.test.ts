...
import { EventSequenceTable } from "@cipher-ai/core/event/sql"
...
import { WorkspaceTable } from "@cipher-ai/core/control-plane/workspace.sql"
...
const originalEnv = {
  CIPHER_AUTH_CONTENT: process.env.CIPHER_AUTH_CONTENT,
  CIPHER_EXPERIMENTAL_WORKSPACES: process.env.CIPHER_EXPERIMENTAL_WORKSPACES,
...
beforeEach(() => {
  restoreEnv()
  process.env.CIPHER_EXPERIMENTAL_WORKSPACES = "true"
})
...
async function initGitRepo(dir: string) {
  ...
  await $`git config user.email "test@cipher.test"`.cwd(dir).quiet()
  ...
  it.instance(
    ...
    () =>
      Effect.gen(function* () {
        const instance = yield* requireInstance
        const workspace = yield* Workspace.Service
        process.env.CIPHER_AUTH_CONTENT = JSON.stringify({ test: { type: "api", key: "secret" } })
        ...
        expect(JSON.parse(recorded.calls.create[0].env.CIPHER_AUTH_CONTENT ?? "{}")).toEqual({
          test: { type: "api", key: "secret" },
        })
        expect(recorded.calls.create[0].env.CIPHER_WORKSPACE_ID).toBe(workspaceID)
        expect(recorded.calls.create[0].env.CIPHER_EXPERIMENTAL_WORKSPACES).toBe("true")
