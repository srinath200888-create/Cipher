...
import { isRecord } from "@cipher-ai/tui/util/record"
...
import { TuiKeybind } from "@cipher-ai/tui/config/keybind"
...
import { TuiConfig } from "@cipher-ai/tui/config"
...
export class Service extends Context.Service<Service, Interface>()("@cipher/TuiConfig") {}
...
const loadState = Effect.fn("TuiConfig.loadState")(function* (ctx: { directory: string }) {
  ...
  const load = (text: string, configFilepath: string): Effect.Effect<Info> =>
    Effect.gen(function* () {
      ...
      // (mirroring the old cipher.json shape) still get their settings applied.
  ...
  // Every config dir we may read from: global config dir, any `.cipher`
  // folders between cwd and home, and CIPHER_CONFIG_DIR.
  const directories = yield* ConfigPaths.directories(ctx.directory)
  yield* Effect.promise(() => migrateTuiConfig({ directories, cwd: ctx.directory }))

  const projectFiles = Flag.CIPHER_DISABLE_PROJECT_CONFIG ? [] : yield* ConfigPaths.files("tui", ctx.directory)
  ...
  // 2. Explicit CIPHER_TUI_CONFIG override, if set.
  if (Flag.CIPHER_TUI_CONFIG) {
    const configFile = Flag.CIPHER_TUI_CONFIG
  ...
  // 4. `.cipher` directories (and CIPHER_CONFIG_DIR) discovered while
  // walking up the tree. Also returned below so callers can install plugin
  // dependencies from each location.
  const dirs = unique(directories).filter((dir) => dir.endsWith(".cipher") || dir === Flag.CIPHER_CONFIG_DIR)

  for (const dir of dirs) {
    if (!dir.endsWith(".cipher") && dir !== Flag.CIPHER_CONFIG_DIR) continue
