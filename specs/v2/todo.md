...
The Cipher server has moved to the Effect HttpApi backend. Remaining work is
...
- remove the public in-memory `@cipher-ai/llm` tool loop after replacing its
...
- plugins get global "Cipher" instance like in that post i showed
- Cipher instance has stuff like `Cipher.session.prompt()` or
  `Cipher.tool.register({...})`
...
- decide whether to preserve deprecated `@cipher-ai/llm` orchestration exports
