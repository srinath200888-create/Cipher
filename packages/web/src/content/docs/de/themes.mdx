...
Mit Cipher kannst du ein eingebautes Theme waehlen, ein an dein Terminal angepasstes Theme nutzen oder ein eigenes Theme definieren.

Standardmaessig verwendet Cipher das `Cipher`-Theme.
...
Cipher liefert mehrere eingebaute Themes mit.
...
- Cipher optisch ans Terminal anpassen wollen
...
Cipher unterstuetzt ein flexibles JSON-basiertes Theme-System.
...
3. **Projekt-Root-Verzeichnis** - Definiert in `<project-root>/.cipher/themes/*.json`
4. **Aktuelles Arbeitsverzeichnis** - Definiert in `./.cipher/themes/*.json`
