import type { ProviderAuthAuthorization, ProviderAuthMethod } from "@cipher-ai/sdk/v2/client"
...
import { List, type ListRef } from "@cipher-ai/ui/list"
import { ProviderIcon } from "@cipher-ai/ui/provider-icon"
import { Spinner } from "@cipher-ai/ui/spinner"
import { Tag } from "@cipher-ai/ui/tag"
...
function ProviderPicker(props: { directory?: Accessor<string | undefined>; onSelect: (provider: string) => void }) {
  ...
  const note = (id: string) => {
    ...
    if (id === "cipher-go") return language.t("dialog.provider.cipherGo.tagline")
    return undefined
  }
    ...
    >
      {(i) => (
        <div class="px-1.25 w-full flex items-center gap-x-3">
          ...
          <span>{i.name}</span>
          <Show when={i.id === "cipher"}>
            <div class="text-14-regular text-text-weak">{language.t("dialog.provider.cipher.tagline")}</div>
          </Show>
  ...
  function ApiAuthView() {
    ...
    return (
      <div class="flex flex-col gap-6">
        <Switch>
          <Match when={provider().id === "cipher"}>
            <div class="flex flex-col gap-4">
              <div class="text-14-regular text-text-base">{language.t("provider.connect.cipherZen.line1")}</div>
              <div class="text-14-regular text-text-base">{language.t("provider.connect.cipherZen.line2")}</div>
              <div class="text-14-regular text-text-base">
                {language.t("provider.connect.cipherZen.visit.prefix")}
                <Link href="https://opencode.ai/zen" tabIndex={-1}>
                  {language.t("provider.connect.cipherZen.visit.link")}
                </Link>
                {language.t("provider.connect.cipherZen.visit.suffix")}
              </div>
            </div>
          </Match>
          <Match when={true}>
