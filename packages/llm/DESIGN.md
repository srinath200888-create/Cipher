...
> current private `@cipher-ai/llm` API. Names and exact TypeScript signatures
> are illustrative until implementation, but the domain boundaries and defaults
> are deliberate.

## Status

- Proposed package: `@cipher-ai/ai`
...
- Secondary audience: Cipher and other durable agent runtimes
...
import { LLM } from "@cipher-ai/ai"
import { OpenAI } from "@cipher-ai/ai/providers/openai"
...
```ts
import { Message } from "@cipher-ai/ai"
...
import { LLM, Tool } from "@cipher-ai/ai"
...
Cipher and other durable runtimes need to own persistence, tool settlement,
...
```ts
import { LLM } from "@cipher-ai/ai/promise"
import { OpenAI } from "@cipher-ai/ai/providers/openai"
...
```ts
import { LLMSchema } from "@cipher-ai/ai/schema"
...
```ts
import { Provider, Protocol } from "@cipher-ai/ai/provider"
...
@cipher-ai/ai/promise
  Promise/AsyncIterable LLM facade

@cipher-ai/ai/schema
  serializable domain schemas

@cipher-ai/ai/provider
  experimental Provider and Protocol authoring APIs

@cipher-ai/ai/providers/openai
@cipher-ai/ai/providers/anthropic
@cipher-ai/ai/providers/google
...
```
...
Cipher should migrate to `generateTurn` / `streamTurn`, preserving its durable
