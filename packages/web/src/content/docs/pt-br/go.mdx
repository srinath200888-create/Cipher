...
O Cipher Go é uma assinatura de baixo custo — **US$ 5 no seu primeiro mês**, depois **US$ 10/mês** — que oferece acesso confiável a modelos abertos de programação populares.

O Go funciona como qualquer outro provedor no Cipher. Você assina o Cipher Go e
...
O Cipher Go lhe dá acesso a esses modelos por **US$ 5 no seu primeiro mês**, depois **US$ 10/mês**.

---

## Como funciona

O Cipher Go funciona como qualquer outro provedor no Cipher.

1. Você entra no **<a href={console}>Cipher Zen</a>**, assina o Go e
   copia a sua chave de API.
2. Você executa o comando `/connect` na TUI, seleciona `Cipher Go` e cola
   a sua chave de API.
3. Execute `/models` na TUI para ver a lista de modelos disponíveis através do Go.

:::note
Apenas um membro por workspace pode assinar o Cipher Go.
:::
...
O Cipher Go inclui os seguintes limites:
...
O [ID do modelo](/docs/config/#models) na sua configuração do Cipher
usa o formato `cipher-go/<model-id>`. Por exemplo, para o Kimi K2.7 Code, você usaria
`cipher-go/kimi-k2.7-code` na sua configuração.
...
4. Não ter **nenhuma restrição de fornecedor (lock-in)**, permitindo que você use qualquer outro provedor com o Cipher também.
