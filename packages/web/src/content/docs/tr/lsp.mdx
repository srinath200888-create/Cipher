...
description: Cipher, LSP sunucularınızla bütünleşir.
---

Cipher, tanılamayı agent için geri bildirim olarak kullanmak üzere Language Server Protocol (LSP) sunucularıyla bütünleşebilir.

---

## Yerleşik

Cipher, popüler diller için çeşitli yerleşik LSP sunucularıyla birlikte gelir:
...
:::note
`CIPHER_DISABLE_LSP_DOWNLOAD` ortam değişkenini `true` olarak ayarlayarak otomatik LSP sunucu indirmelerini devre dışı bırakabilirsiniz.
:::
...
LSP etkinleştirildiğinde ve Cipher bir dosyayı açtığında:
...
LSP sunucularını, Cipher yapılandırmanızdaki `lsp` bölümü aracılığıyla etkinleştirebilir ve özelleştirebilirsiniz.
