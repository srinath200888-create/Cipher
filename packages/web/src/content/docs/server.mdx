---
title: Server
description: Interact with Cipher server over HTTP.
---

import config from "../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

The `Cipher serve` command runs a headless HTTP server that exposes an OpenAPI endpoint that an Cipher client can use.
...
| `--mdns-domain` | Custom domain name for mDNS service | `Cipher.local` |
...
Set `CIPHER_SERVER_PASSWORD` to protect the server with HTTP basic auth. The username defaults to `Cipher`, or set `CIPHER_SERVER_USERNAME` to override it. This applies to both `Cipher serve` and `Cipher web`.
...
When you run `Cipher` it starts a TUI and a server. Where the TUI is the
...
:::tip
Use the Cipher server to interact with Cipher programmatically.
:::

This architecture lets Cipher support multiple clients and allows you to interact with Cipher programmatically.

You can run `Cipher serve` to start a standalone server. If you have the
Cipher TUI running, `Cipher serve` will start a new server.
...
The [`/tui`](#tui) endpoint can be used to drive the TUI through the server. For example, you can prefill or run a prompt. This setup is used by the Cipher [IDE](/docs/ide) plugins.
...
The Cipher server exposes the following APIs.
