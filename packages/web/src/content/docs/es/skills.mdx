...
Las habilidades del agente permiten a Cipher descubrir instrucciones reutilizables de su repositorio o directorio de inicio.
...
Cipher busca estas ubicaciones:

- Configuración del proyecto: `.cipher/skills/<name>/SKILL.md`
...
Para las rutas locales del proyecto, Cipher sube desde su directorio de trabajo actual hasta llegar al árbol de trabajo de git.
Carga cualquier `skills/*/SKILL.md` coincidente en `.cipher/` y cualquier `.claude/skills/*/SKILL.md` o `.agents/skills/*/SKILL.md` coincidente a lo largo del camino.
...
Crea `.cipher/skills/git-release/SKILL.md` así:
...
Cipher enumera las habilidades disponibles en la descripción de la herramienta `skill`.
...
Controle a qué agentes de habilidades pueden acceder utilizando permisos basados ​​en patrones en `cipher.json`:
...
**Para agentes integrados** (en `cipher.json`):
