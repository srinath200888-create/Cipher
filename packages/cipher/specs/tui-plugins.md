...
- Author package entrypoint is `@cipher-ai/plugin/tui`.
...
- Import types from `@cipher-ai/plugin/tui`.
- `@cipher-ai/plugin` exports `./tui` and declares optional peer deps on `@opentui/core` and `@opentui/solid`.

Minimal module shape:

```tsx
/** @jsxImportSource @opentui/solid */
import type { TuiPlugin, TuiPluginModule } from "@cipher-ai/plugin/tui"
...
- The value is a semver range checked against the running Cipher version.
...
- `Cipher plugin <module>` and TUI install both run install → manifest read → config patch.
- Alias: `Cipher plug <module>`.
...
When `plugin` entries exist in a writable `.opencode` dir or `CIPHER_CONFIG_DIR`, Cipher installs `@cipher-ai/plugin` into that dir and writes:
...
That is what makes local config-scoped plugins able to import `@cipher-ai/plugin/tui`.
...
- The host already installs the default OpenTUI bundle (`default keys`, metadata fields, and enabled fields) plus cipher's comma bindings, leader token, base layout fallback, pending-sequence helpers, and managed textarea layer.
...
Cipher registers a `mode` layer field on the host keymap. Plugins can use it to keep bindings active only in the relevant UI state.
...
Mode pushes are automatically tracked by the plugin runtime. If a plugin is disabled, fails during activation, or the TUI shuts down before the plugin calls the disposer, Cipher pops the plugin's pushed modes during plugin cleanup. Calling the disposer yourself is still recommended for component lifetimes; cleanup remains idempotent.
...
- For generic config-to-bindings helpers, import `createBindingLookup` from `@cipher-ai/plugin/tui`.
...
- `message` is required; `title` defaults to `"Cipher"`; `notification` defaults to enabled with `when: "blurred"`; `sound` defaults to enabled with `when: "always"`.
...
- Config `attention.sounds` overrides active-pack sounds by slot. Failed loads fall back to the active pack and then `Cipher.default`.
...
- Local plugins persist installed themes under the local `.cipher/themes` area near the plugin config source.
...
- `--pure` / `CIPHER_PURE` skips external TUI plugins only.
...
- Local smoke plugin: `.cipher/plugins/tui-smoke.tsx`
- Local vim plugin: `.cipher/plugins/tui-vim.tsx`
- Local smoke config: `.cipher/tui.json`
- Local smoke theme: `.cipher/plugins/smoke-theme.json`
