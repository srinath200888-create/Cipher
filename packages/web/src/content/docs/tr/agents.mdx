...
cipher'da iki tür agent vardır; birincil agent'lar ve alt agent'lar.
...
Cipher, **Build** ve **Plan** olmak üzere iki yerleşik birincil agent ile birlikte gelir. Bunlara aşağıda bakacağız.
...
Cipher, **General**, **Explore** ve **Scout** olmak üzere üç yerleşik alt agent ile birlikte gelir. Buna aşağıda bakacağız.

---

## Yerleşik

Cipher iki yerleşik birincil agent ve üç yerleşik alt agent ile birlikte gelir.
...
Agent'ları `cipher.json` yapılandırma dosyanızda yapılandırın:
...
Sıcaklık belirtilmezse Cipher modeline özgü varsayılanları kullanır; çoğu model için genellikle 0, Qwen modelleri için 0,55.
...
Bu yol, yapılandırma dosyasının bulunduğu yere göredir. Yani bu hem global Cipher yapılandırması hem de projeye özel yapılandırma için işe yarar.
...
Cipher yapılandırmanızdaki model kimliği `provider/model-id` biçimini kullanır. Örneğin, [Cipher Zen](/docs/zen) kullanıyorsanız, GPT 5.1 Codex için `cipher/gpt-5.1-codex` kullanırsınız.
...
:::tip
Mevcut modellerin listesini görmek için `Cipher models` komutunu çalıştırın.
:::
