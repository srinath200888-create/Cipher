...
With Cipher you can select from one of several built-in themes, use a theme that adapts to your terminal theme, or define your own custom theme.

By default, Cipher uses our own `Cipher` theme.
...
Cipher comes with several built-in themes.
...
- Want Cipher to match their terminal's appearance
...
Cipher supports a flexible JSON-based theme system that allows users to create and customize themes easily.
...
3. **Project root directory** - Defined in the `<project-root>/.cipher/themes/*.json`
4. **Current working directory** - Defined in `./.cipher/themes/*.json`
