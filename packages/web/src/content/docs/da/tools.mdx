...
Her er alle de innebygde verktøyene som er tilgængelige i Cipher.
...
:::note
Dette verktøyet er bare tilgængelig når `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (eller `CIPHER_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgængelig når du bruger cipher-leverandøren eller når miljøvariabelen `CIPHER_ENABLE_EXA` er satt til en sannverdi (f.eks. `true` eller `1`).
