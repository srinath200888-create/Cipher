...
Narzędzia umożliwiają LLM wykonywanie działań w bazie kodu. Cipher posiada zestaw wbudowanych narzędzi, ale możesz go rozszerzyć o [narzędzia niestandardowe](/docs/custom-tools) lub [serwery MCP](/docs/mcp-servers).
...
Oto wszystkie wbudowane narzędzia dostępne w Cipher.
...
:::note
To narzędzie jest dostępne tylko wtedy, gdy `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (lub `CIPHER_EXPERIMENTAL=true`).
:::
...
:::note
To narzędzie jest dostępne tylko w przypadku korzystania z dostawcy Cipher lub gdy zmienna środowiskowa `CIPHER_ENABLE_EXA` jest ustawiona na dowolną wartość prawdziwą (np. `true` lub `1`).

Aby włączyć przy uruchamianiu Cipher:
