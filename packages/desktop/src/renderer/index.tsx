...
import type { UpdaterState } from "@cipher-ai/app/updater"
...
import "./styles.css"
import { Splash } from "@cipher-ai/ui/logo"
    ...
    integrations: (integrations) => {
      return integrations.filter(
        (i) =>
          i.name !== "Breadcrumbs" &&
          !(
            import.meta.env.CIPHER_CHANNEL === "prod" &&
...
const deepLinkEvent = "cipher:deep-link"
...
const emitDeepLinks = (urls: string[]) => {
  ...
  window.__CIPHER__ ??= {}
  const pending = window.__CIPHER__.deepLinks ?? []
  window.__CIPHER__.deepLinks = [...pending, ...urls]
...
function windowLastActiveUrlKey(windowID: string) {
  return `cipher.desktop.window.${windowID}.last-active-url`
}
...
function DesktopRoot(props: { windowState: DesktopWindowState }) {
  ...
  const loadLocale = async () => {
    const current = await platform.storage?.("cipher.global.dat").getItem("language")
