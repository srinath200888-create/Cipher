---
title: Zen
description: Liste organisée de modèles fournis par Cipher.
---
...
Cipher Zen est une liste de modèles testés et vérifiés fournie par l'équipe Cipher.

Zen fonctionne comme n'importe quel autre fournisseur dans Cipher. Vous vous connectez à Cipher Zen et obtenez votre clé API. C'est **entièrement facultatif** et vous n'avez pas besoin de l'utiliser pour utiliser Cipher.
...
:::tip
Nous avons testé un groupe sélectionné de modèles et de fournisseurs qui fonctionnent bien avec Cipher.
:::
...
Cipher Zen est une passerelle AI qui vous donne accès à ces modèles.

---

## Comment ça marche

Cipher Zen fonctionne comme n'importe quel autre fournisseur dans Cipher.

1. Vous vous connectez à **<a href={console}>Cipher Zen</a>**, ajoutez vos informations de facturation et copiez votre clé API.
2. Vous exécutez la commande `/connect` dans le TUI, sélectionnez Cipher Zen et collez votre clé API.
...
Le [model id](/docs/config/#models) dans votre configuration Cipher utilise le format `opencode/<model-id>`. Par exemple, pour GPT 5.5, vous utiliseriez `cipher/gpt-5.5` dans votre configuration.
...
- DeepSeek V4 Flash Free est disponible sur Cipher pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- MiMo-V2.5 Free est disponible sur Cipher pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- North Mini Code Free est disponible sur Cipher pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Nemotron 3 Ultra Free est disponible sur Cipher pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
- Big Pickle est un modèle stealth gratuit sur Cipher pour une durée limitée. L'équipe utilise cette période pour recueillir des retours et améliorer le modèle.
...
Nous avons créé Cipher Zen pour :
...
4. N'avoir **aucun lock-in** en vous permettant de l'utiliser avec n'importe quel autre agent de codage. Et toujours vous permettre d'utiliser n'importe quel autre fournisseur avec Cipher également.
