...
import { RelativePath } from "@cipher-ai/core/schema"
    ...
    Effect.acquireUseRelease(
      ...
      (tmp) =>
        Effect.gen(function* () {
          ...
          expect(files.map((item) => item.path)).toContain(RelativePath.make(".cipher/config"))
          ...
          expect(matches.map((item) => item.entry.path)).toContain(RelativePath.make(".cipher/config"))
