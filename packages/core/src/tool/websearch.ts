...
import { ToolFailure } from "@cipher-ai/llm"
...
export class ConfigService extends Context.Service<ConfigService, Config>()("@cipher/v2/WebSearchConfig") {}

/** Isolates the retained product environment contract from the generic tool implementation. */
export const defaultConfigLayer = Layer.sync(ConfigService, () =>
  ConfigService.of({
    provider:
      process.env.CIPHER_WEBSEARCH_PROVIDER === "exa" || process.env.CIPHER_WEBSEARCH_PROVIDER === "parallel"
        ? process.env.CIPHER_WEBSEARCH_PROVIDER
        : undefined,
    enableExa: truthy("CIPHER_EXPERIMENTAL") || truthy("CIPHER_ENABLE_EXA") || truthy("CIPHER_EXPERIMENTAL_EXA"),
    enableParallel: truthy("CIPHER_ENABLE_PARALLEL") || truthy("CIPHER_EXPERIMENTAL_PARALLEL"),
