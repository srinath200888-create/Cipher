...
Existem dois tipos de agentes no Cipher; agentes primários e subagentes.
...
Cipher vem com dois agentes primários integrados, **Build** e **Plan**. Vamos
ver isso abaixo.
...
Cipher vem com três subagentes integrados, **General**, **Explore** e **Scout**. Vamos ver isso abaixo.

---

## Integrados

Cipher vem com dois agentes primários integrados e três subagentes integrados.
...
Um agente somente leitura para pesquisa em documentação externa e dependências. Use-o quando você precisar clonar o repositório de uma dependência para o cache gerenciado do Cipher, inspecionar o código-fonte de uma biblioteca ou cruzar o código local com implementações upstream sem modificar seu workspace.
...
Configure os agentes em seu arquivo de configuração `cipher.json`:
...
Se nenhuma temperatura for especificada, o Cipher usa padrões específicos do modelo; tipicamente 0 para a maioria dos modelos, 0.55 para modelos Qwen.
...
Este caminho é relativo ao local onde o arquivo de configuração está localizado. Portanto, isso funciona tanto para a configuração global do Cipher quanto para a configuração específica do projeto.
...
O ID do modelo em sua configuração do Cipher usa o formato `provider/model-id`. Por exemplo, se você estiver usando [Cipher Zen](/docs/zen), você usaria `cipher/gpt-5.1-codex` para GPT 5.1 Codex.
...
```json title="cipher.json" {3-6,9-12}
{
  "$schema": "https://opencode.ai/config.json",
  "tools": {
    "write": true,
...
```json title="cipher.json" {3-5,8-10}
{
  "$schema": "https://opencode.ai/config.json",
  "permission": {
    "edit": "deny"
...
:::tip
Execute `Cipher models` para ver uma lista dos modelos disponíveis.
:::
