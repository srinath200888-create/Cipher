...
Les compétences d'agent permettent à Cipher de découvrir des instructions réutilisables à partir de votre dépôt ou de votre répertoire personnel.
...
Cipher recherche ces emplacements :

- Configuration du projet : `.cipher/skills/<name>/SKILL.md`
...
Pour les chemins locaux du projet, Cipher remonte de votre répertoire de travail actuel jusqu'à ce qu'il atteigne l'arbre de travail git.
Il charge tout `skills/*/SKILL.md` correspondant dans `.cipher/` et tout `.claude/skills/*/SKILL.md` ou `.agents/skills/*/SKILL.md` correspondant en cours de route.
...
Créez `.cipher/skills/git-release/SKILL.md` comme ceci :
...
Cipher répertorie les compétences disponibles dans la description de l'outil `skill`.
...
Contrôlez les compétences auxquelles les agents peuvent accéder à l'aide d'autorisations basées sur des modèles dans `cipher.json` :
...
**Pour les agents intégrés** (dans `cipher.json`) :
