...
Dzięki Cipher możesz wybrać jeden z kilku wbudowanych motywów, użyć motywu, który dostosowuje się do motywu terminala lub zdefiniować własny, niestandardowy motyw.

Domyślnie Cipher używa naszego własnego motywu `Cipher`.
...
Cipher ma kilka wbudowanych motywów.
...
- Chcą, aby Cipher pasował do wyglądu ich terminala
...
Cipher obsługuje elastyczny system motywów oparty na JSON, który pozwala użytkownikom łatwo tworzyć i dostosowywać motywy.
...
3. **Katalog główny projektu** - Zdefiniowany w `<project-root>/.cipher/themes/*.json`
4. **Bieżący katalog roboczy** - Zdefiniowany w `./.cipher/themes/*.json`
