...
Practical reference for a future `packages/server` split after the Cipher
server moved to the Effect HttpApi backend.

## Current State

- The server still lives in `packages/cipher`.
...
Until enough shared service code lives outside `packages/cipher`, a future
`packages/server` should either:

- own pure HttpApi contracts only, or
- accept host-provided services/layers/callbacks from `packages/cipher`

It should not import `packages/cipher` services while `packages/cipher`
...
2. Move stable domain schemas into shared packages only when they no longer
   depend on cipher-local runtime modules.
3. Extract pure HttpApi contract modules into `packages/server` once the contract
   can compile without importing `packages/cipher` implementation details.
