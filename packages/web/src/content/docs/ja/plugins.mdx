---
title: プラグイン
description: Cipher を拡張する独自のプラグインを作成します。
---

プラグインを使用すると、さまざまなイベントにフックして動作をカスタマイズすることで Cipher を拡張できます。プラグインを作成して、新しい機能を追加したり、外部サービスと統合したり、Cipher のデフォルトの動作を変更したりできます。
...
- `.cipher/plugins/` - プロジェクトレベルのプラグイン
...
1. グローバル設定 (`~/.config/opencode/cipher.json`)
2. プロジェクト設定 (`cipher.json`)
3. グローバルプラグインディレクトリ (`~/.config/opencode/plugins/`)
4. プロジェクトプラグインディレクトリ (`.cipher/plugins/`)
...
Cipher は起動時に `bun install` を実行してこれらをインストールします。プラグインとツールはそれらをインポートできるようになります。
...
- `client`: AI と対話するための Cipher SDK クライアント。
...
ここでは、Cipher を拡張するために使用できるプラグインの例をいくつか示します。
...
Cipher が `.env` ファイルを読み取らないようにします。
...
プラグインは Cipher にカスタムツールを追加することもできます。
...
`tool` ヘルパーは、Cipher が呼び出すことができるカスタムツールを作成します。 Zod スキーマ関数を受け取り、次のようなツール定義を返します。

- `description`: ツールの機能
- `args`: ツールの引数の Zod スキーマ
- `execute`: ツールが呼び出されたときに実行される関数

カスタムツールは、組み込みツールと並行して Cipher で使用できます。
