...
Cipher Go is a low cost subscription — **$5 for your first month**, then **$10/month** — that gives you reliable access to popular open coding models.

Go works like any other provider in Cipher. You subscribe to Cipher Go and
...
Cipher Go gives you access to these models for **$5 for your first month**, then **$10/month**.

---

## How it works

Cipher Go works like any other provider in Cipher.

1. You sign in to **<a href={console}>Cipher Zen</a>**, subscribe to Go, and
   copy your API key.
2. You run the `/connect` command in the TUI, select `Cipher Go`, and paste
   your API key.
3. Run `/models` in the TUI to see the list of models available through Go.

:::note
Only one member per workspace can subscribe to Cipher Go.
:::
...
Cipher Go includes the following limits:
...
uses the format `cipher-go/<model-id>`. For example, for Kimi K2.7 Code, you would
use `cipher-go/kimi-k2.7-code` in your config.
...
4. Have **no lock-in** by allowing you to use any other provider with Cipher as well.
