---
title: Plugin
description: Scrivi plugin per estendere Cipher.
---

I plugin ti permettono di estendere Cipher agganciandoti a vari eventi e personalizzando il comportamento. Puoi creare plugin per aggiungere nuove funzionalità, integrare servizi esterni o modificare il comportamento predefinito di Cipher.
...
- `.cipher/plugins/` - plugin a livello progetto
...
1. Config globale (`~/.config/opencode/cipher.json`)
2. Config di progetto (`cipher.json`)
3. Directory plugin globale (`~/.config/opencode/plugins/`)
4. Directory plugin di progetto (`.cipher/plugins/`)
...
Cipher esegue `bun install` all'avvio per installarle. I tuoi plugin e strumenti potranno poi importarle.
...
- `client`: un client SDK di Cipher per interagire con l'AI.
...
Ecco alcuni esempi di plugin che puoi usare per estendere Cipher.
...
:::note
Se usi l'app desktop di Cipher, può inviare automaticamente notifiche di sistema quando una risposta è pronta o quando una sessione va in errore.
:::
...
Impedisci a Cipher di leggere i file `.env`:
...
I plugin possono anche aggiungere strumenti personalizzati a Cipher:
...
L'helper `tool` crea uno strumento personalizzato che Cipher può chiamare. Accetta una funzione di schema Zod e restituisce una definizione di tool con:
...
I tuoi strumenti personalizzati saranno disponibili in Cipher insieme agli strumenti integrati.
