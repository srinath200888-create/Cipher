...
describe("filesystem", () => {
  ...
  describe("findUp()", () => {
    ...
    test("rootFirst preserves json then jsonc order per directory", async () => {
      ...
      await fs.writeFile(path.join(tmp.path, "cipher.json"), "{}", "utf-8")
      await fs.writeFile(path.join(tmp.path, "cipher.jsonc"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cipher.json"), "{}", "utf-8")
      await fs.writeFile(path.join(project, "cipher.jsonc"), "{}", "utf-8")

      const result = await Filesystem.findUp(["cipher.json", "cipher.jsonc"], nested, tmp.path, {
        rootFirst: true,
      })

      expect(result).toEqual([
        path.join(tmp.path, "cipher.json"),
        path.join(tmp.path, "cipher.jsonc"),
        path.join(project, "cipher.json"),
        path.join(project, "cipher.jsonc"),
      ])
    })
  })
