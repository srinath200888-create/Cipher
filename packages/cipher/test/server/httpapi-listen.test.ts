...
const original = {
  ...
  envPassword: process.env.CIPHER_SERVER_PASSWORD,
  envUsername: process.env.CIPHER_SERVER_USERNAME,
}
const auth = { username: "cipher", password: "listen-secret" }
...
afterEach(async () => {
  ...
  if (original.envPassword === undefined) delete process.env.CIPHER_SERVER_PASSWORD
  else process.env.CIPHER_SERVER_PASSWORD = original.envPassword
  if (original.envUsername === undefined) delete process.env.CIPHER_SERVER_USERNAME
  else process.env.CIPHER_SERVER_USERNAME = original.envUsername
  await disposeAllInstances()
  await resetDatabase()
})

async function startListener() {
  Flag.CIPHER_SERVER_PASSWORD = auth.password
  Flag.CIPHER_SERVER_USERNAME = auth.username
  process.env.CIPHER_SERVER_PASSWORD = auth.password
  process.env.CIPHER_SERVER_USERNAME = auth.username
...
async function startNoAuthListener() {
  ...
  Flag.CIPHER_SERVER_USERNAME = auth.username
  delete process.env.CIPHER_SERVER_PASSWORD
  process.env.CIPHER_SERVER_USERNAME = auth.username
  return Server.listen({ hostname: "127.0.0.1", port: 0 })
}
...
describe("HttpApi Server.listen", () => {
  testPty("serves HTTP routes and upgrades PTY websocket through Server.listen", async () => {
    ...
    try {
      const response = await fetch(new URL(PtyPaths.shells, listener.url), {
        headers: { authorization: authorization(), "x-cipher-directory": tmp.path },
      })
      expect(response.status).toBe(200)
  ...
  test("plugin client requests reuse the listening server instance", async () => {
    ...
    const previous = process.env.CIPHER_DISABLE_DEFAULT_PLUGINS
    process.env.CIPHER_DISABLE_DEFAULT_PLUGINS = "1"
    ...
    try {
      ...
      if (previous === undefined) delete process.env.CIPHER_DISABLE_DEFAULT_PLUGINS
      else process.env.CIPHER_DISABLE_DEFAULT_PLUGINS = previous
    }
  })
