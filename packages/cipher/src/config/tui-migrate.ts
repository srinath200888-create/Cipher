...
/**
 * Migrates tui-specific keys (theme, keybinds, tui) from cipher.json files
 * into dedicated tui.json files. Migration is performed per-directory and
 * skips only locations where a tui.json already exists.
 */
export async function migrateTuiConfig(input: MigrateInput) {
  const cipher = await opencodeFiles(input)
  for (const file of cipher) {
...
async function opencodeFiles(input: { directories: string[]; cwd: string }) {
  const files = [
    ...ConfigPaths.fileInDirectory(Global.Path.config, "cipher"),
    ...(await Filesystem.findUp(["cipher.json", "cipher.jsonc"], input.cwd, undefined, { rootFirst: true })),
  ]
  for (const dir of unique(input.directories)) {
    files.push(...ConfigPaths.fileInDirectory(dir, "cipher"))
  }
  if (Flag.CIPHER_CONFIG) files.push(Flag.CIPHER_CONFIG)
