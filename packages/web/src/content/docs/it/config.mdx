...
description: Usa la configurazione JSON di Cipher.
---

Puoi configurare Cipher usando un file di configurazione JSON.

---

## Formato

Cipher supporta sia **JSON** sia **JSONC** (JSON con commenti).
...
1. **Config remota** (da `.well-known/cipher`) - default dell'organizzazione
2. **Config globale** (`~/.config/opencode/cipher.json`) - preferenze utente
3. **Config personalizzata** (variabile d'ambiente `CIPHER_CONFIG`) - sovrascritture personalizzate
4. **Config di progetto** (`cipher.json` nel progetto) - impostazioni specifiche del progetto
5. **Directory `.opencode`** - agenti, comandi, plugin
6. **Config inline** (variabile d'ambiente `CIPHER_CONFIG_CONTENT`) - sovrascritture a runtime
...
:::note
Le directory `.opencode` e `~/.config/cipher` usano **nomi al plurale** per le sottodirectory: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` e `themes/`. I nomi al singolare (ad es. `agent/`) sono supportati per retrocompatibilita.
:::
...
Le organizzazioni possono fornire una configurazione predefinita tramite l'endpoint `.well-known/cipher`. Viene recuperata automaticamente quando ti autentichi con un provider che lo supporta.
...
Metti la configurazione globale di Cipher in `~/.config/opencode/cipher.json`. Usa la configurazione globale per preferenze server/runtime valide per l'utente come provider, modelli e permessi.
...
Aggiungi `cipher.json` nella root del progetto. La configurazione di progetto ha la precedenza piu alta tra i file standard: sovrascrive sia la configurazione globale sia quella remota.
...
Quando Cipher si avvia, cerca un file di configurazione nella directory corrente oppure risale fino alla directory Git piu vicina.
...
Specifica un percorso personalizzato per il file di configurazione usando la variabile d'ambiente `CIPHER_CONFIG`.
...
Specifica una directory di configurazione personalizzata usando la variabile d'ambiente `CIPHER_CONFIG_DIR`. Questa directory verra usata per cercare agenti, comandi, modalita e plugin proprio come la directory standard `.opencode` e dovrebbe seguire la stessa struttura.
...
Il file di configurazione ha uno schema definito in [**`Cipher.ai/config.json`**](https://opencode.ai/config.json).

La configurazione TUI usa [**`Cipher.ai/tui.json`**](https://opencode.ai/tui.json).
...
Usa `CIPHER_TUI_CONFIG` per puntare a un file di configurazione TUI personalizzato.

Le chiavi legacy `theme`, `keybinds` e `tui` in `cipher.json` sono deprecate e vengono migrate automaticamente quando possibile.
...
Puoi configurare le impostazioni del server per i comandi `Cipher serve` e `Cipher web` tramite l'opzione `server`.
...
- `mdns` - Abilita la scoperta del servizio mDNS. Questo permette ad altri dispositivi sulla rete di scoprire il tuo server Cipher.
- `mdnsDomain` - Nome di dominio personalizzato per il servizio mDNS. Predefinito: `Cipher.local`. Utile per eseguire piu istanze sulla stessa rete.
...
Puoi configurare provider e modelli da usare in Cipher tramite le opzioni `provider`, `model` e `small_model`.
...
L'opzione `small_model` configura un modello separato per task leggeri come la generazione dei titoli. Per impostazione predefinita, Cipher prova a usare un modello più economico se disponibile dal tuo provider, altrimenti fa fallback sul tuo modello principale.
...
Puoi anche definire agenti usando file markdown in `~/.config/opencode/agents/` o `.cipher/agents/`. [Scopri di piu](/docs/agents).
...
L'agente predefinito deve essere un agente primary (non un subagent). Puo essere un agente integrato come `"build"` o `"plan"`, oppure un [agente personalizzato](/docs/agents) che hai definito. Se l'agente specificato non esiste o e un subagent, Cipher fara fallback su `"build"` mostrando un avviso.

Questa impostazione si applica a tutte le interfacce: TUI, CLI (`Cipher run`), app desktop e GitHub Action.
...
Puoi anche definire comandi usando file markdown in `~/.config/opencode/commands/` o `.cipher/commands/`. [Scopri di piu](/docs/commands).
...
Cipher scarichera automaticamente eventuali aggiornamenti quando si avvia. Puoi disabilitare questa funzione con l'opzione `autoupdate`.
...
Per impostazione predefinita, Cipher **consente tutte le operazioni** senza richiedere un'approvazione esplicita. Puoi cambiarlo usando l'opzione `permission`.
...
[I plugin](/docs/plugins) estendono Cipher con strumenti personalizzati, hook e integrazioni.

Metti i file dei plugin in `.cipher/plugins/` o `~/.config/opencode/plugins/`. Puoi anche caricare plugin da npm tramite l'opzione `plugin`.
...
E utile quando vuoi limitare Cipher a usare solo alcuni provider invece di disabilitarli uno per uno.
