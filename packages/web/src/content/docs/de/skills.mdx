...
Agent Skills erlauben Cipher, wiederverwendbare Anweisungen aus deinem Repo oder Home-Verzeichnis zu finden.
...
Cipher sucht in folgenden Pfaden:
...
Bei projektlokalen Pfaden laeuft Cipher vom aktuellen Verzeichnis nach oben bis zum Git-Worktree.
Dabei werden passende `skills/*/SKILL.md` in `.cipher/` sowie passende Dateien unter `.claude/skills/*/SKILL.md` und `.agents/skills/*/SKILL.md` geladen.
...
Erstelle `.cipher/skills/git-release/SKILL.md` zum Beispiel so:
...
Cipher listet verfuegbare Skills in der Beschreibung des `skill`-Tools.
...
Steuere in `cipher.json` per Muster, auf welche Skills Agenten zugreifen duerfen:
...
**Fuer eingebaute Agenten** (in `cipher.json`):
