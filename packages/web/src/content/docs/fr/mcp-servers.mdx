...
Vous pouvez ajouter des outils externes à Cipher à l'aide du _Model Context Protocol_ ou MCP. Cipher prend en charge les serveurs locaux et distants.
...
Vous pouvez définir des serveurs MCP dans votre [Cipher Config](https://opencode.ai/docs/config/) sous `mcp`. Ajoutez chaque MCP avec un nom unique. Vous pouvez faire référence à ce MCP par son nom lorsque vous demandez le LLM.
...
Les organisations peuvent fournir des serveurs MCP par défaut via leur point de terminaison `.well-known/cipher`. Ces serveurs peuvent être désactivés par défaut, permettant aux utilisateurs de choisir ceux dont ils ont besoin.
...
Cipher gère automatiquement l'authentification OAuth pour les serveurs MCP distants. Lorsqu'un serveur nécessite une authentification, Cipher :
...
Si le serveur nécessite une authentification, Cipher vous demandera de vous authentifier lorsque vous essayez de l'utiliser pour la première fois. Sinon, vous pouvez [déclencher manuellement le flux](#authenticating) avec `Cipher mcp auth <server-name>`.
...
La commande `mcp auth` ouvrira votre navigateur pour autorisation. Après votre autorisation, Cipher stockera les jetons en toute sécurité dans `~/.local/share/opencode/mcp-auth.json`.
...
Vos MCP sont disponibles sous forme d'outils dans Cipher, aux côtés des outils intégrés. Vous pouvez donc les gérer via la configuration Cipher comme n'importe quel autre outil.
...
Cela ouvrira une fenêtre de navigateur pour terminer le flux OAuth et connecter Cipher à votre compte Sentry.
