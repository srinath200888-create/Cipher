...
Du kan legge til eksterne verktøy i Cipher ved å bruke _Model Context Protocol_ eller MCP. Cipher støtter både lokale og eksterne servere.
...
Du kan definere MCP-servere i [Cipher Config](https://opencode.ai/docs/config/) under `mcp`. Legg til hver MCP med et unikt navn. Du kan referere til den MCP ved navn når du spør LLM-en.

```jsonc title="cipher.jsonc" {6}
{
...
Organisasjoner kan tilby standard MCP-servere via deres `.well-known/cipher`-endepunkt. Disse serverne kan være deaktivert som standard, slik at brukere kan melde seg på de de trenger.
...
Cipher håndterer automatisk OAuth-autentisering for eksterne MCP-servere. Når en server krever autentisering, vil Cipher:
...
Hvis serveren krever autentisering, vil Cipher be deg om å autentisere når du prøver å bruke den for første gang. Hvis ikke, kan du [utløse flyten manuelt](#authenticating) med `Cipher mcp auth <server-name>`.
...
```bash
Cipher mcp auth my-oauth-server
```
...
```bash
Cipher mcp logout my-oauth-server
```

Kommandoen `mcp auth` åpner nettleseren din for autorisasjon. Etter at du har godkjent, vil Cipher lagre tokens sikkert i `~/.local/share/opencode/mcp-auth.json`.
...
Cipher mcp debug my-oauth-server
```
...
MCP-ene dine er tilgjengelige som verktøy i Cipher, sammen med innebygde verktøy. Så du kan administrere dem gjennom cipher-konfigurasjonen som alle andre verktøy.
...
```json title="cipher.json" {11, 14-18}
{
  "$schema": "https://opencode.ai/config.json",
  "mcp": {
    "my-mcp": {
...
```bash
Cipher mcp auth sentry
```

Dette åpner et nettleservindu for å fullføre OAuth-flyten og koble Cipher til Sentry-kontoen din.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` overskrifter i forespørslene dine. På Helicones Sessions-side vil du se hver cipher-samtale oppført som en separat økt.
