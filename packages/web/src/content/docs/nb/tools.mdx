...
Verktøy lar LLM utføre handlinger i kodebasen din. Cipher kommer med et sett med innebygde verktøy, men du kan utvide det med [egendefinerte verktøy](/docs/custom-tools) eller [MCP servere](/docs/mcp-servers).
...
Her er alle de innebygde verktøyene som er tilgjengelige i Cipher.
...
:::note
Dette verktøyet er bare tilgjengelig når `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (eller `CIPHER_EXPERIMENTAL=true`).
:::
...
:::note
Dette verktøyet er bare tilgjengelig når du bruker cipher-leverandøren eller når miljøvariabelen `CIPHER_ENABLE_EXA` er satt til en sann verdi (f.eks. `true` eller `1`).

Slik aktiverer du når du starter Cipher:
