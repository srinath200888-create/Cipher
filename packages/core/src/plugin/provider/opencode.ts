...
import type { IntegrationOAuthMethodRegistration } from "@cipher-ai/plugin/v2/effect/integration"
import { define } from "@cipher-ai/plugin/v2/effect/plugin"
...
const defaultServer = "https://console.cipher.ai"
const clientID = "cipher-cli"
...
function oauth(http: HttpClient.HttpClient) {
  return {
    integrationID: Integration.ID.make("cipher"),
    method: {
      id: methodID,
      type: "oauth",
      label: "cipher Console account",
    },
    authorize: () =>
      Effect.gen(function* () {
...
export const CipherPlugin = define<HttpClient.HttpClient | EventV2.Service | Scope.Scope>({
  id: "cipher",
  effect: Effect.fn(function* (ctx) {
    ...
    const load = Effect.fn("CipherPlugin.load")(function* () {
      const connection = yield* ctx.integration.connection.active("cipher")
      ...
      providers = credential
        ? yield* fetchProviders(http, credential).pipe(
            Effect.catch((cause) =>
              Effect.logWarning("failed to load cipher provider config", { cause }).pipe(Effect.as(undefined)),
            ),
          )
        : undefined
    })
    ...
    yield* ctx.integration.transform((draft) => {
      draft.update("cipher", (integration) => {
        integration.name = "cipher"
      })
      draft.method.update(oauth(http))
      draft.method.update({ integrationID: "cipher", method: { type: "key", label: "API key (service account)" } })
    })

    connected = (yield* ctx.integration.connection.active("cipher")) !== undefined
    yield* ctx.catalog.transform((catalog) => {
      for (const [providerID, item] of Object.entries(providers ?? {})) {
        catalog.provider.update(providerID, (provider) => {
          provider.integrationID = Integration.ID.make("cipher")
      ...
      const item = catalog.provider.get(ProviderV2.ID.cipher)
      if (!item) return
      const hasKey = Boolean(process.env.CIPHER_API_KEY || connected || item.provider.request.body.apiKey)
    ...
    yield* events.subscribe(Integration.Event.ConnectionUpdated).pipe(
      Stream.filter((event) => event.data.integrationID === Integration.ID.make("cipher")),
