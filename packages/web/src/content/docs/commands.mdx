...
Create `.cipher/commands/test.md`:
...
You can add custom commands through the Cipher config or by creating markdown files in the `commands/` directory.

---

### JSON

Use the `command` option in your Cipher [config](/docs/config):
...
Cipher includes several built-in commands like `/init`, `/undo`, `/redo`, `/share`, `/help`; [learn more](/docs/tui#commands).
