# Remove `packages/cipher/src/storage/db.ts`

## Goal

Remove all production usages of the legacy `packages/cipher/src/storage/db.ts` module.
...
This does not mean removing SQLite or Drizzle everywhere in one step. The smaller target is deleting the Cipher legacy wrapper by moving call sites onto deeper modules or onto the core/effect database adapter directly.

## Current Inventory

Production imports from `packages/cipher/src/storage/db.ts` are concentrated in 22 source files:

- `packages/cipher/src/account/repo.ts`
- `packages/cipher/src/cli/cmd/db.ts`
- `packages/cipher/src/cli/cmd/import.ts`
- `packages/cipher/src/cli/cmd/stats.ts`
- `packages/cipher/src/control-plane/workspace.ts`
- `packages/cipher/src/index.ts`
- `packages/cipher/src/node.ts`
- `packages/cipher/src/permission/index.ts`
- `packages/cipher/src/project/project.ts`
- `packages/cipher/src/server/projectors.ts`
- `packages/cipher/src/server/routes/instance/httpapi/handlers/sync.ts`
- `packages/cipher/src/server/shared/fence.ts`
- `packages/cipher/src/session/message-v2.ts`
- `packages/cipher/src/session/projectors.ts`
- `packages/cipher/src/session/prompt.ts`
- `packages/cipher/src/session/session.ts`
- `packages/cipher/src/session/todo.ts`
- `packages/cipher/src/share/share-next.ts`
- `packages/cipher/src/storage/db.ts`
- `packages/cipher/src/sync/index.ts`
- `packages/cipher/src/worktree/index.ts`
...
Status: Completed. Startup, the public node export, and database CLI tooling no longer import the legacy Cipher database wrapper; `packages/cipher/src/storage/db.ts` has been deleted.
...
- Move database path and client startup behind the core/effect database module rather than the Cipher wrapper.
...
Status: Completed. `SyncEvent` and the Cipher projector boundary were removed; session/message event projection now lives in core EventV2/projector infrastructure.
...
Status: Completed. These services no longer import the legacy Cipher database wrapper.
...
Status: Completed. Session/message reads and projector writes have moved off the legacy Cipher database wrapper.
...
Status: Completed. Remaining one-off CLI/admin reads and writes now use core database services or domain services instead of the legacy Cipher database wrapper.
...
All migration groups are complete or superseded. `packages/cipher/src/storage/db.ts` has been deleted.

## Superseded: Data Migrations

Status: Superseded. No Cipher data-migration group remains.

The previous Cipher `data-migration.ts` service only backfilled session usage from message rows. That work is now covered by core database migration `packages/core/src/database/migration/20260510033149_session_usage.ts`, so there is no separate Cipher data-migration group.
...
- Existing schema ownership remains in `packages/core/src/**/*.sql.ts`; do not move table definitions back into `packages/cipher`.

## Verification Commands

- `rg "@/storage/db|./storage/db|Database\.(use|transaction|effect|Client|getPath)|\bTxOrDb\b|\bTransaction\b" packages/cipher/src`
- `bun typecheck` from `packages/cipher`
- Relevant package tests from `packages/cipher`, not the repo root
