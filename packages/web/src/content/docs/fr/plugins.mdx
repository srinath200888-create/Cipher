---
title: Plugins
description: Écrivez vos propres plugins pour étendre Cipher.
---

Les plugins vous permettent d'étendre Cipher en vous connectant à divers événements et en personnalisant le comportement. Vous pouvez créer des plugins pour ajouter de nouvelles fonctionnalités, intégrer des services externes ou modifier le comportement par défaut de Cipher.
...
- `.cipher/plugins/` - Plugins au niveau du projet
...
1. Configuration globale (`~/.config/opencode/cipher.json`)
2. Configuration du projet (`cipher.json`)
3. Répertoire global des plugins (`~/.config/opencode/plugins/`)
4. Répertoire des plugins du projet (`.cipher/plugins/`)
...
Cipher exécute `bun install` au démarrage pour les installer. Vos plugins et outils peuvent ensuite les importer.
...
- `client` : un client SDK Cipher pour interagir avec l'IA.
...
Voici quelques exemples de plugins que vous pouvez utiliser pour étendre Cipher.
...
:::note
Si vous utilisez l'application de bureau Cipher, elle peut envoyer automatiquement des notifications système lorsqu'une réponse est prête ou en cas d'erreur de session.
:::
...
Empêchez Cipher de lire les fichiers `.env` :
...
Les plugins peuvent également ajouter des outils personnalisés à Cipher :
...
L'assistant `tool` crée un outil personnalisé que Cipher peut appeler. Il prend une fonction de schéma Zod et renvoie une définition d'outil avec :
...
Vos outils personnalisés seront disponibles pour Cipher aux côtés des outils intégrés.
