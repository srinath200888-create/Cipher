...
import {
  ...
  type TuiSlotPlugin,
  type TuiTheme,
} from "@cipher-ai/plugin/tui"
import path from "path"
import { fileURLToPath } from "url"
import { TuiConfig } from "@/config/tui"
import { AppNodeBuilder } from "@cipher-ai/core/effect/app-node-builder"
import { errorData, errorMessage } from "@cipher-ai/tui/util/error"
...
import { hasTheme, upsertTheme } from "@cipher-ai/tui/context/theme"
import { Global } from "@cipher-ai/core/global"
import { Filesystem } from "@/util/filesystem"
import { Process } from "@/util/process"
import { Flock } from "@cipher-ai/core/util/flock"
import { Flag } from "@cipher-ai/core/flag/flag"
import { internalTuiPlugins, type InternalTuiPlugin } from "./internal"
import type { HostPluginApi, HostSlots } from "@cipher-ai/tui/plugin/slots"
import { ConfigPlugin } from "@/config/plugin"
import { ConfigPluginV1 } from "@cipher-ai/core/v1/config/plugin"
import { createCommandShim } from "@cipher-ai/tui/plugin/command-shim"
import { RuntimeFlags } from "@/effect/runtime-flags"
import { Effect } from "effect"
import { createPluginRuntime, type PluginRuntime, type TuiPluginHost } from "@cipher-ai/tui/plugin/runtime"
...
function createThemeInstaller(
  ...
  return async (file) => {
    ...
    const local_dir =
      path.basename(source_dir) === ".cipher"
        ? path.join(source_dir, "themes")
        : path.join(source_dir, ".cipher", "themes")
...
function defaultPluginOrigin(state: RuntimeState, spec: string): ConfigPlugin.Origin {
  return {
    spec,
    scope: "local",
    source: state.api.state.path.config || path.join(state.directory, ".cipher", "tui.json"),
  }
}
...
async function load(input: {
  ...
  try {
    ...
    const records = Flag.CIPHER_PURE ? [] : pluginOrigins
    if (Flag.CIPHER_PURE && pluginOrigins.length) {
    }
