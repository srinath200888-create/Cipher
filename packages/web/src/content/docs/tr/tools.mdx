...
Araçlar, LLM'in kod tabanınızda eylem gerçekleştirmesini sağlar. Cipher yerleşik bir araç setiyle gelir, ancak bunu [özel araçlarla](/docs/custom-tools) veya [MCP sunucularıyla](/docs/mcp-servers) genişletebilirsiniz.
...
Cipher içinde bulunan tüm yerleşik araçlar aşağıdadır.
...
:::note
Bu araç yalnızca `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (veya `CIPHER_EXPERIMENTAL=true`) olduğunda kullanılabilir.
:::
...
:::note
Bu araç yalnızca Cipher provider kullanılırken veya `CIPHER_ENABLE_EXA` ortam değişkeni doğru değerlerden birine ayarlandığında (örneğin `true` ya da `1`) kullanılabilir.

Cipher başlatırken etkinleştirmek için:
