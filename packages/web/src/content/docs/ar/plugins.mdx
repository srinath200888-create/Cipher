---
title: الإضافات
description: اكتب إضافاتك الخاصة لتوسيع Cipher.
---

تتيح لك الإضافات توسيع Cipher عبر ربطها بأحداث مختلفة وتخصيص السلوك. يمكنك إنشاء إضافات لإضافة ميزات جديدة، أو التكامل مع خدمات خارجية، أو تعديل السلوك الافتراضي لـ Cipher.
...
- `.cipher/plugins/` - إضافات على مستوى المشروع
...
1. إعدادات عامة (`~/.config/opencode/cipher.json`)
2. إعدادات المشروع (`cipher.json`)
3. دليل الإضافات العام (`~/.config/opencode/plugins/`)
4. دليل إضافات المشروع (`.cipher/plugins/`)
...
يشغّل Cipher الأمر `bun install` عند بدء التشغيل لتثبيتها. بعد ذلك يمكن لإضافاتك وأدواتك استيرادها.
...
- `client`: عميل SDK لـ Cipher للتفاعل مع الذكاء الاصطناعي.
...
فيما يلي بعض أمثلة الإضافات التي يمكنك استخدامها لتوسيع Cipher.
...
:::note
إذا كنت تستخدم تطبيق Cipher لسطح المكتب، فيمكنه إرسال إشعارات النظام تلقائيا عند جاهزية الرد أو عند حدوث خطأ في جلسة.
:::
...
امنع Cipher من قراءة ملفات `.env`:
...
يمكن للإضافات أيضا إضافة أدوات مخصصة إلى Cipher:
...
تقوم الدالة المساعدة `tool` بإنشاء أداة مخصصة يمكن لـ Cipher استدعاؤها. تأخذ دالة مخطط Zod وتعيد تعريف أداة يتضمن:

- `description`: ما الذي تفعله الأداة
- `args`: مخطط Zod لوسائط الأداة
- `execute`: الدالة التي تُشغَّل عند استدعاء الأداة

ستكون أدواتك المخصصة متاحة لـ Cipher إلى جانب الأدوات المضمنة.
