---
title: Proveedores
description: Usando cualquier proveedor LLM en Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher usa el [AI SDK](https://ai-sdk.dev/) y [Models.dev](https://models.dev) para admitir **75+ proveedores LLM** y admite la ejecución de modelos locales.

Para agregar un proveedor necesita:

1. Agregue las claves API para el proveedor usando el comando `/connect`.
2. Configure el proveedor en su configuración Cipher.
...
Puedes personalizar los proveedores a través de la sección `provider` en tu Cipher
configuración.
...
Cipher Zen es una lista de modelos proporcionados por el equipo Cipher que han sido
probado y verificado para funcionar bien con Cipher. [Más información](/docs/zen).

:::tip
Si eres nuevo, te recomendamos comenzar con Cipher Zen.
:::

1. Ejecute el comando `/connect` en TUI, seleccione Cipher y diríjase a [Cipher.ai/auth](https://opencode.ai/auth).
...
Funciona como cualquier otro proveedor en Cipher y su uso es completamente opcional.

---

## Cipher Go

Cipher Go es un plan de suscripción de bajo costo que brinda acceso confiable a modelos de codificación abiertos populares proporcionados por el equipo de Cipher que han sido
probado y verificado para funcionar bien con Cipher.

1. Ejecute el comando `/connect` en TUI, seleccione `Cipher Go` y diríjase a [Cipher.ai/auth](https://opencode.ai/zen).
...
Para usar Amazon Bedrock con Cipher:
...
2. **Configure la autenticación** utilizando uno de los siguientes métodos:
   ...
   Establezca una de estas variables de entorno mientras ejecuta Cipher:
   ...
   Para una configuración persistente o específica del proyecto, utilice `cipher.json`:
...
:::info
El uso de su suscripción Claude Pro/Max en Cipher no está oficialmente respaldado por [Anthropic](https://anthropic.com).
:::
...
Puedes configurar Cipher para usar modelos locales mediante [Atomic Chat](https://atomic.chat), una aplicación de escritorio que ejecuta LLMs locales detrás de un servidor API compatible con OpenAI (endpoint por defecto `http://127.0.0.1:1337/v1`).
...
2. Vaya a [Azure AI Foundry](https://ai.azure.com/) e implemente un modelo.
   ...
   :::nota
   El nombre de la implementación debe coincidir con el nombre del modelo para que Cipher funcione correctamente.
   :::
...
5. Ejecute el comando `/models` para seleccionar un modelo.
   ...
   También puede agregar modelos a través de su configuración Cipher.
...
para almacenar el token en el almacenamiento de autenticación Cipher.
:::

##### Autohospedado GitLab

:::note[nota de cumplimiento]
Cipher utiliza un modelo pequeño para algunas tareas de IA, como generar el título de la sesión.
Está configurado para usar gpt-5-nano de forma predeterminada, alojado por Zen. Para bloquear Cipher
Para usar solo su propia instancia alojada en GitLab, agregue lo siguiente a su
Archivo `cipher.json`. También se recomienda desactivar el uso compartido de sesiones.
...
Más documentación en la página de inicio de [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).

##### Configuración

Personalizar a través de `cipher.json`:
...
Para utilizar su suscripción GitHub Copilot con Cipher:
...
Para utilizar Google Vertex AI con Cipher:
...
2. Establezca las variables de entorno requeridas:
   ...
   Configúrelos mientras ejecuta Cipher.
...
En caso de que vea una característica o modelo de Helicone que no esté configurado automáticamente a través de Cipher, siempre podrá configurarlo usted mismo.
...
La función [Sesiones](https://docs.helicone.ai/features/sessions) de Helicone le permite agrupar solicitudes LLM relacionadas. Utilice el complemento [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automáticamente cada conversación Cipher como una sesión en Helicone.
...
El complemento inyecta encabezados `Helicone-Session-Id` y `Helicone-Session-Name` en sus solicitudes. En la página Sesiones de Helicone, verá cada conversación Cipher enumerada como una sesión separada.
...
Puede configurar Cipher para usar modelos locales a través de la utilidad llama-server de [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Puede configurar Cipher para usar modelos locales a través de LM Studio.
...
Puedes configurar Cipher para usar modelos locales a través de Ollama.

:::tip
Ollama puede configurarse automáticamente para Cipher. Consulte los [documentos de integración de Ollama](https://docs.ollama.com/integrations/opencode) para obtener más detalles.
:::
...
Para usar Ollama Cloud con Cipher:
...
3. Copie la clave API para usarla en Cipher.
...
6. **Importante**: Antes de usar modelos en la nube en Cipher, debe obtener la información del modelo localmente:
...
Cipher Zen es una lista de modelos probados y verificados proporcionada por el equipo Cipher. [Más información](/docs/zen).

1. Inicie sesión en **<a href={console}>Cipher Zen</a>** y haga clic en **Crear API clave**.

2. Ejecute el comando `/connect` y busque **Cipher Zen**.

   ```txt
   /connect
   ```

3. Ingrese su clave Cipher API.
...
4. Muchos modelos de OpenRouter están precargados de forma predeterminada, ejecute el comando `/models` para seleccionar el que desee.
   ...
   También puede agregar modelos adicionales a través de su configuración Cipher.
...
5. También puedes personalizarlos a través de tu configuración Cipher. A continuación se muestra un ejemplo de cómo especificar un proveedor.
...
Para utilizar [API generativas de Scaleway](https://www.scaleway.com/en/docs/generative-apis/) con Cipher:
...
También puede personalizar modelos a través de su configuración Cipher. A continuación se muestra un ejemplo de cómo especificar el orden de enrutamiento del proveedor.
...
:::tip
Puede utilizar cualquier proveedor compatible con OpenAI con Cipher. La mayoría de los proveedores de IA modernos ofrecen API compatibles con OpenAI.
:::
...
4. Cree o actualice su archivo `cipher.json` en el directorio de su proyecto:
...
Los campos `limit` le permiten a Cipher comprender cuánto contexto le queda. Los proveedores estándar los extraen de models.dev automáticamente.
...
1. **Verifique la configuración de autenticación**: Ejecute `Cipher auth list` para ver si las credenciales
...
2. Para proveedores personalizados, verifique la configuración Cipher y:
   - Asegúrese de que el ID del proveedor utilizado en el comando `/connect` coincida con el ID en su configuración Cipher.
