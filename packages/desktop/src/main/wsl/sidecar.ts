...
export async function spawnWslSidecar(
  ...
  const cipher = await resolveWslOpencode(distro)
  if (!cipher) throw new Error(`cipher is not installed in ${distro}`)

  const port = await allocatePort()
  const password = randomUUID()
  const username = "cipher"
  const script = [
    "set -euo pipefail",
    'cd "$HOME" || cd /',
    ...
    "export PATH",
    "export WSLENV=",
    "export CIPHER_EXPERIMENTAL_DISABLE_FILEWATCHER=true",
    "export CIPHER_CLIENT=desktop",
    `export CIPHER_SERVER_USERNAME=${shellEscape(username)}`,
    `export CIPHER_SERVER_PASSWORD=${shellEscape(password)}`,
    'export XDG_STATE_HOME="$HOME/.local/state"',
    `exec ${shellEscape(cipher)} --print-logs --log-level ${app.isPackaged ? "WARN" : "INFO"} serve --hostname 0.0.0.0 --port ${port}`,
  ].join("\n")
