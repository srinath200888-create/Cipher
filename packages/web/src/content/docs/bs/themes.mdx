...
U Cipher mozete birati izmedu vise ugradenih tema, koristiti temu koja se prilagodava terminalu ili definisati vlastitu temu.

Po defaultu, Cipher koristi nasu `Cipher` temu.
...
Cipher dolazi sa vise ugradenih tema.
...
- Zele da Cipher odgovara izgledu njihovog terminala
...
Cipher podrzava fleksibilan sistem tema baziran na JSON-u koji olaksava kreiranje i prilagodavanje tema.
...
3. **Korijenski direktorij projekta** - `<project-root>/.cipher/themes/*.json`
4. **Trenutni radni direktorij** - `./.cipher/themes/*.json`
