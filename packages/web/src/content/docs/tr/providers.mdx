---
title: Sağlayıcılar
description: cipher'da herhangi bir LLM sağlayıcısını kullanma.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher, **75'ten fazla LLM sağlayıcısını** desteklemek için [AI SDK](https://ai-sdk.dev/) ve [Models.dev](https://models.dev) kullanır ve yerel modellerin çalıştırılmasını destekler.
...
2. Sağlayıcıyı Cipher yapılandırmanızda yapılandırın.
...
Sağlayıcıları cipher'unuzdaki `provider` bölümü aracılığıyla özelleştirebilirsiniz.
config.
...
Cipher Zen, Cipher ekibi tarafından sağlanan modellerin bir listesidir.
Cipher ile iyi çalıştığı test edilip doğrulandı. [Daha fazla bilgi](/docs/zen).

:::tip
Yeniyseniz Cipher Zen ile başlamanızı öneririz.
:::
...
cipher'daki diğer sağlayıcılar gibi çalışır ve kullanımı tamamen isteğe bağlıdır.

---

## Cipher Go

Cipher Go, Cipher ile iyi çalıştığı test edilmiş ve doğrulanmış, Cipher ekibi tarafından sağlanan popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir abonelik planıdır.

1. TUI'de `/connect` komutunu çalıştırın, `Cipher Go`'yu seçin ve [Cipher.ai/auth](https://opencode.ai/zen) adresine gidin.
...
Amazon Bedrock'u Cipher ile kullanmak için:
...
2. **Kimlik doğrulamayı yapılandırın**: Aşağıdaki yöntemlerden birini kullanın:
   ...
   cipher'u çalıştırırken bu ortam değişkenlerinden birini ayarlayın:
   ...
   Projeye özel veya kalıcı yapılandırma için `cipher.json` kullanın:
...
:::info
Claude Pro/Max aboneliğinizi cipher'da kullanmak [Anthropic](https://anthropic.com) tarafından resmi olarak desteklenmemektedir.
:::
...
cipher'u, yerel LLM'leri OpenAI uyumlu bir API sunucusunun arkasında çalıştıran bir masaüstü uygulaması olan [Atomic Chat](https://atomic.chat) aracılığıyla yerel modelleri kullanacak şekilde yapılandırabilirsiniz (varsayılan uç nokta `http://127.0.0.1:1337/v1`).
...
2. [Azure AI Foundry](https://ai.azure.com/)'a gidin ve bir model dağıtın.
   ...
   :::note
   cipher'un düzgün çalışması için dağıtım adının model adıyla eşleşmesi gerekir.
   :::
...
5. Bir model seçmek için `/models` komutunu çalıştırın.
   ...
   Cipher yapılandırmanız aracılığıyla da modeller ekleyebilirsiniz.
...
:::note[uyumluluk notu]
Cipher, oturum başlığını oluşturmak gibi bazı AI görevleri için küçük bir model kullanır.
Varsayılan olarak Zen tarafından barındırılan gpt-5-nano'yu kullanacak şekilde yapılandırılmıştır. cipher'u kilitlemek için
yalnızca kendi GitLab tarafından barındırılan örneğinizi kullanmak için aşağıdakileri ekleyin:
`cipher.json` dosyası. Ayrıca oturum paylaşımını devre dışı bırakmanız da önerilir.
...
Daha fazla belge [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) ana sayfasında.

##### Yapılandırma

`cipher.json` üzerinden özelleştirin:
...
GitHub Copilot aboneliğinizi cipher'la kullanmak için:
...
Google Vertex AI'yi Cipher ile kullanmak için:
...
2. Gerekli ortam değişkenlerini ayarlayın:
   ...
   Bunları Cipher çalıştırılırken ayarlayın.
...
Helicone'un Cipher aracılığıyla otomatik olarak yapılandırılmayan bir özelliği veya modelini görmeniz durumunda bunu her zaman kendiniz yapılandırabilirsiniz.
...
Helicone'un [Sessions](https://docs.helicone.ai/features/sessions) özelliği, ilgili LLM isteklerini birlikte gruplandırmanıza olanak tanır. Her Cipher görüşmesini Helicone'da bir oturum olarak otomatik olarak kaydetmek için [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) eklentisini kullanın.
...
Eklenti isteklerinize `Helicone-Session-Id` ve `Helicone-Session-Name` başlıklarını enjekte eder. Helicone'un Oturumlar sayfasında her Cipher görüşmesinin ayrı bir oturum olarak listelendiğini göreceksiniz.
...
cipher'u yerel modelleri kullanacak şekilde [llama.cpp's](https://github.com/ggml-org/llama.cpp) llama-server yardımcı programını kullanarak yapılandırabilirsiniz.
...
LM Studio aracılığıyla cipher'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.
...
Ollama aracılığıyla cipher'u yerel modelleri kullanacak şekilde yapılandırabilirsiniz.

:::tip
Ollama kendisini Cipher için otomatik olarak yapılandırabilir. Ayrıntılar için [Ollama integration docs](https://docs.ollama.com/integrations/opencode)'a bakın.
:::
...
Ollama Cloud'u Cipher ile kullanmak için:
...
3. cipher'da kullanmak için API anahtarını kopyalayın.
...
6. **Önemli**: cipher'da bulut modellerini kullanmadan önce model bilgilerini yerel olarak almanız gerekir:
...
Cipher Zen, Cipher ekibinin test edip doğruladığı modellerin bir listesidir. [Daha fazla bilgi](/docs/zen).

1. **<a href={console}>Cipher Zen</a>**'de oturum açın ve **API Anahtarı Oluştur**'u tıklayın.

2. `/connect` komutunu çalıştırın ve **Cipher Zen**'i arayın.

   ```txt
   /connect
   ```

3. Cipher API anahtarınızı girin.
...
4. Birçok OpenRouter modeli varsayılan olarak önceden yüklenmiştir; istediğiniz modeli seçmek için `/models` komutunu çalıştırın.
   ...
   Cipher yapılandırmanız aracılığıyla ek modeller de ekleyebilirsiniz.
...
5. Bunları ayrıca Cipher yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Burada bir sağlayıcı belirtmeye ilişkin bir örnek verilmiştir
...
[Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/)'yi Cipher ile kullanmak için:
...
Modelleri Cipher yapılandırmanız aracılığıyla da özelleştirebilirsiniz. Sağlayıcı yönlendirme sırasını belirtmeye ilişkin bir örneği burada bulabilirsiniz.
...
:::tip
OpenAI uyumlu herhangi bir sağlayıcıyı Cipher ile kullanabilirsiniz. Çoğu modern yapay zeka sağlayıcısı OpenAI uyumlu API'ler sunar.
:::
...
4. Proje dizininizde `cipher.json` dosyanızı oluşturun veya güncelleyin:
...
`limit` alanları cipher'un ne kadar bağlamınız kaldığını anlamasını sağlar. Standart sağlayıcılar bunları models.dev'den otomatik olarak alır.
...
1. **Kimlik doğrulama kurulumunu kontrol edin**: Kimlik bilgilerinin geçerli olup olmadığını görmek için `Cipher auth list` komutunu çalıştırın.
...
2. Özel sağlayıcılar için Cipher yapılandırmasını kontrol edin ve:
   - `/connect` komutunda kullanılan sağlayıcı kimliğinin, Cipher yapılandırmanızdaki kimlikle eşleştiğinden emin olun.
