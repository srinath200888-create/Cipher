---
title: Server
description: Nutze den cipher-Server ueber HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Der Befehl `Cipher serve` startet einen headless HTTP-Server.
Er stellt einen OpenAPI-Endpunkt bereit, den ein cipher-Client nutzen kann.
...
Setze `CIPHER_SERVER_PASSWORD`, um den Server mit HTTP Basic Auth zu schuetzen.
Der Benutzername ist standardmaessig `Cipher`, kann aber mit `CIPHER_SERVER_USERNAME` ueberschrieben werden. Das gilt fuer `Cipher serve` und `Cipher web`.
...
Wenn du `Cipher` startest, werden TUI und Server gestartet.
...
:::tip
Nutze den cipher-Server, um Cipher programmatisch anzusteuern.
:::

Diese Architektur erlaubt mehrere Clients und programmatische Nutzung.

Mit `Cipher serve` startest du einen eigenstaendigen Server.
Laeuft bereits eine TUI, startet `Cipher serve` trotzdem eine neue Serverinstanz.
...
Dieses Muster nutzen auch die cipher-[IDE](/docs/ide)-Plugins.
...
Der cipher-Server stellt folgende APIs bereit.
