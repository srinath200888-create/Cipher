---
title: Dodaci
description: Napišite vlastite dodatke za proširenje Cipher.
---

Dodaci vam omogućavaju da proširite Cipher spajanjem na različite događaje i prilagođavanjem ponašanja. Možete kreirati dodatke za dodavanje novih funkcija, integraciju sa eksternim uslugama ili izmenu zadanog ponašanja Cipher.
...
- `.cipher/plugins/` - Dodaci na nivou projekta
...
1. Globalna konfiguracija (`~/.config/opencode/cipher.json`)
2. Konfiguracija projekta (`cipher.json`)
3. Globalni direktorij dodataka (`~/.config/opencode/plugins/`)
4. Direktorij dodataka projekta (`.cipher/plugins/`)
...
Cipher pokreće `bun install` pri pokretanju da ih instalira. Vaši dodaci i alati ih zatim mogu uvesti.
...
- `client`: Cipher SDK klijent za interakciju sa AI.
...
Evo nekoliko primjera dodataka koje možete koristiti za proširenje Cipher.
...
Spriječite Cipher da čita `.env` fajlove:
...
Dodaci također mogu dodati prilagođene alate u Cipher:
...
Pomoćnik `tool` kreira prilagođeni alat koji Cipher može pozvati. Uzima funkciju Zod sheme i vraća definiciju alata sa:

- `description`: Šta alat radi
- `args`: Zod šema za argumente alata
- `execute`: Funkcija koja se pokreće kada se pozove alat
  Vaši prilagođeni alati će biti dostupni za Cipher zajedno sa ugrađenim alatima.
