...
Pour déboguer les problèmes avec Cipher, commencez par vérifier les journaux et les données locales qu'il stocke sur le disque.
...
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\cipher\log`
...
Vous pouvez définir le niveau de journalisation avec l'option de ligne de commande `--log-level` pour obtenir des informations de débogage plus détaillées. Par exemple, `Cipher --log-level DEBUG`.

---

## Stockage

Cipher stocke les données de session et autres données d'application sur le disque à l'emplacement :

- **macOS/Linux** : `~/.local/share/opencode/`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.local\share\Cipher`
...
Cipher Desktop exécute un serveur Cipher local (le side-car `cipher-cli`) en arrière-plan. La plupart des problèmes sont causés par un plugin qui se comporte mal, un cache corrompu ou un mauvais paramètre du serveur.
...
- macOS uniquement : menu `Cipher` -> **Recharger la vue Web** (aide si l'interface utilisateur est vide/gelée).
...
- **macOS/Linux** (anciennes installations) : `~/.local/share/opencode/cipher.jsonc`
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\cipher\cipher.jsonc`
...
Cipher peut également charger des plugins locaux à partir du disque. Écartez-les temporairement (ou renommez le dossier) et redémarrez l'application de bureau :

- **Plugins mondiaux**
  - **macOS/Linux** : `~/.config/opencode/plugins/`
  - **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.config\cipher\plugins`
...
Si la désactivation des plugins ne résout pas le problème (ou si l'installation d'un plugin est bloquée), videz le cache afin que Cipher puisse le reconstruire.

1. Quittez complètement Cipher Desktop.
2. Supprimez le répertoire cache :

- **macOS** : Finder -> `Cmd+Shift+G` -> coller `~/.cache/cipher`
- **Linux** : supprimez `~/.cache/cipher` (ou exécutez `rm -rf ~/.cache/cipher`)
- **Windows** : appuyez sur `WIN+R` et collez `%USERPROFILE%\.cache\Cipher`

3. Redémarrez le bureau Cipher.

---

### Résoudre les problèmes de connexion au serveur

Cipher Desktop peut soit démarrer son propre serveur local (par défaut), soit se connecter à un serveur URL que vous avez configuré.
...
Si votre `cipher.json(c)` contient une section `server`, supprimez-la temporairement et redémarrez l'application de bureau.

#### Vérifier les variables d'environnement

Si `CIPHER_PORT` est défini dans votre environnement, l'application de bureau tentera d'utiliser ce port pour le serveur local.

- Désactivez `CIPHER_PORT` (ou choisissez un port libre) et redémarrez.
...
Sur Windows, Cipher Desktop nécessite Microsoft Edge **WebView2 Runtime**. Si l'application s'ouvre sur une fenêtre vide ou ne démarre pas, installez/mettez à jour WebView2 et réessayez.

---

### Windows : problèmes de performances généraux

Si vous rencontrez des performances lentes, des problèmes d'accès aux fichiers ou des problèmes de terminal sur Windows, essayez d'utiliser [WSL (Windows Sous-système pour Linux)](/docs/windows-wsl). WSL fournit un environnement Linux qui fonctionne de manière plus transparente avec les fonctionnalités de Cipher.

---

### Les notifications ne s'affichent pas

Cipher Desktop affiche uniquement les notifications système lorsque :

- les notifications sont activées pour Cipher dans les paramètres de votre système d'exploitation, et
...
1. Quittez le bureau Cipher.
2. Recherchez et supprimez ces fichiers (ils se trouvent dans le répertoire de données de l'application Cipher Desktop) :

- `Cipher.settings.dat` (serveur de bureau par défaut URL)
- `Cipher.global.dat` et `Cipher.workspace.*.dat` (état de l'interface utilisateur comme les serveurs/projets récents)
...
Si vous rencontrez des problèmes avec Cipher :
...
3. Assurez-vous d'avoir la dernière version avec `Cipher upgrade`
...
Pour déterminer à quels modèles vous avez accès, exécutez `Cipher models`
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.local\share\Cipher`
...
Si vous rencontrez des erreurs d’appel API, cela peut être dû à des packages de fournisseurs obsolètes. Cipher installe dynamiquement les packages du fournisseur (OpenAI, Anthropic, Google, etc.) selon les besoins et les met en cache localement.
...
Sur Windows, appuyez sur `WIN+R` et supprimez : `%USERPROFILE%\.cache\Cipher`

2. Redémarrez Cipher pour réinstaller les derniers packages du fournisseur

Cela forcera Cipher à télécharger les versions les plus récentes des packages du fournisseur, ce qui résout souvent les problèmes de compatibilité avec les paramètres du modèle et les modifications de API.
...
Cipher détectera si vous utilisez Wayland et préférez `wl-clipboard`, sinon il essaiera de trouver les outils du presse-papiers dans l'ordre : `xclip` et `xsel`.
