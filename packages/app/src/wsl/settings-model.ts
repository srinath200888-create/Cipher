...
export type AddServerPrimaryButton = {
  ...
  label: AddServerText
  disabled: boolean
  action: "install-cipher" | "add" | null
  loading: boolean
  width: string | null
}
...
export function wslOpencodeAction(check?: WslOpencodeCheck) {
  if (!check) return
  if (!check.resolvedPath) return "Install cipher"
  if (check.matchesDesktop === false) return "Update cipher"
}
...
export function addServerViewModel(input: {
  ...
  const opencodeCheck = selectedDistro ? (state?.cipherChecks[selectedDistro] ?? null) : null
...
function addServerDistroStatus(input: {
  ...
  const check = input.state?.cipherChecks[input.name]
  if (!check) {
    if (input.probingAddable || (job?.kind === "probe-addable" && job.distros.includes(input.name))) {
      return checkingStatus()
    }
  ...
  if (!check.resolvedPath) return { label: { key: "wsl.onboarding.distroStatus.cipherMissing" }, tone: "warning" }
...
function addServerPrimaryButton(input: {
  ...
  const installingOpencode =
    input.state?.job?.kind === "install-cipher" && input.state.job.distro === input.selectedDistro
  ...
  if (!addServerOpencodeReady(input.cipherCheck)) {
    const update = !!input.cipherCheck?.resolvedPath && input.cipherCheck.matchesDesktop === false
    return {
      variant: "neutral",
...
function addServerSelectedDistroSettled(state: WslServersState | undefined, selectedDistro: string | null) {
  ...
  return !!state.cipherChecks[selectedDistro]
}
...
export function addableProbePlan(input: {
  ...
  const pending = ordered.flatMap((item) => {
    ...
    if (wslDistroReady(state, item.name) && !state.cipherChecks[item.name]) return [`cipher:${item.name}`]
    return []
  })
