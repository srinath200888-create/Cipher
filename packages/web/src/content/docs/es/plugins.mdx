---
title: Complementos
description: Escriba sus propios complementos para extender Cipher.
---

Los complementos le permiten extender Cipher al conectarse a varios eventos y personalizar el comportamiento. Puede crear complementos para agregar nuevas funciones, integrarlos con servicios externos o modificar el comportamiento predeterminado de Cipher.
...
- `.cipher/plugins/` - Complementos a nivel de proyecto
...
1. Configuración global (`~/.config/opencode/cipher.json`)
2. Configuración del proyecto (`cipher.json`)
3. Directorio global de complementos (`~/.config/opencode/plugins/`)
4. Directorio de complementos del proyecto (`.cipher/plugins/`)
...
Cipher ejecuta `bun install` al inicio para instalarlos. Luego, sus complementos y herramientas pueden importarlos.
...
- `client`: Un cliente SDK Cipher para interactuar con la IA.
...
A continuación se muestran algunos ejemplos de complementos que puede utilizar para ampliar Cipher.
...
:::note
Si está utilizando la aplicación de escritorio Cipher, puede enviar notificaciones del sistema automáticamente cuando una respuesta esté lista o cuando se produzca un error en una sesión.
:::
...
Evite que Cipher lea archivos `.env`:
...
Los complementos también pueden agregar herramientas personalizadas a Cipher:
...
El ayudante `tool` crea una herramienta personalizada a la que Cipher puede llamar. Toma una función de esquema Zod y devuelve una definición de herramienta con:
...
Sus herramientas personalizadas estarán disponibles para Cipher junto con las herramientas integradas.
