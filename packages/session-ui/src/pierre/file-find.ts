...
function clearHighlightFind() {
  ...
  if (!api) return
  api.delete("cipher-find")
  api.delete("cipher-find-current")
}
...
export function createFileFind(opts: CreateFileFindOptions) {
  ...
  const setHighlights = (ranges: Range[], currentIndex: number) => {
    ...
    if (active) api.set("cipher-find-current", new Highlight(active))

    const rest = ranges.filter((_, i) => i !== currentIndex)
    if (rest.length > 0) api.set("cipher-find", new Highlight(...rest))
    return true
  }
