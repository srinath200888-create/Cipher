...
describe("buildRequestParts", () => {
    ...
    expect(
      result.requestParts.some(
        (part) =>
          part.type === "text" &&
          part.synthetic &&
          part.metadata?.cipherComment &&
          (part.metadata.cipherComment as { comment?: string }).comment === "check this",
      ),
    ).toBe(true)
  ...
  test("preserves an external attachment source path for the model", () => {
    const result = buildRequestParts({
      ...
      images: [
        {
          type: "image",
          id: "img_external",
          filename: "cipher.global.dat",
          sourcePath: "C:\\Users\\Luke\\AppData\\Roaming\\ai.cipher.desktop.beta\\cipher.global.dat",
          mime: "text/plain",
      ...
      sessionDirectory: "C:\\Repos\\sst\\cipher",
    })

    expect(result.requestParts.find((part) => part.type === "file")?.filename).toBe(
      "C:\\Users\\Luke\\AppData\\Roaming\\ai.cipher.desktop.beta\\cipher.global.dat",
    )
  })
