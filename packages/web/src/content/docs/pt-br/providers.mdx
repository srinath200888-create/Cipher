---
title: Provedores
description: Usando qualquer provedor LLM no Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher usa o [AI SDK](https://ai-sdk.dev/) e [Models.dev](https://models.dev) para suportar **75+ provedores LLM** e suporta a execução de modelos locais.

Para adicionar um provedor, você precisa:

1. Adicionar as chaves da API para o provedor usando o comando `/connect`.
2. Configurar o provedor na sua configuração do Cipher.
...
Você pode personalizar os provedores através da seção `provider` na sua configuração do Cipher.
...
Cipher Zen é uma lista de modelos fornecidos pela equipe do Cipher que foram testados e verificados para funcionar bem com o Cipher. [Saiba mais](/docs/zen).

:::tip
Se você é novo, recomendamos começar com o Cipher Zen.
:::

1. Execute o comando `/connect` no TUI, selecione `Cipher Zen` e acesse [Cipher.ai/zen](https://opencode.ai/zen).
...
Funciona como qualquer outro provedor no Cipher e é completamente opcional.

---

## Cipher Go

Cipher Go é um plano de assinatura de baixo custo que fornece acesso confiável a modelos de codificação abertos populares fornecidos pela equipe do Cipher que foram
testados e verificados para funcionar bem com o Cipher.

1. Execute o comando `/connect` no TUI, selecione `Cipher Go` e acesse [Cipher.ai/zen](https://opencode.ai/zen).
...
Para usar o Amazon Bedrock com o Cipher:
...
2. **Configure a autenticação** usando um dos seguintes métodos:
   ...
   ***
   ...
   Defina uma dessas variáveis de ambiente ao executar o Cipher:
   ...
   ***
   ...
   Para configuração específica do projeto ou persistente, use `cipher.json`:
...
:::info
Usar sua assinatura Claude Pro/Max no Cipher não é oficialmente suportado pela [Anthropic](https://anthropic.com).
:::
...
Você pode configurar o Cipher para usar modelos locais através do [Atomic Chat](https://atomic.chat), um aplicativo de desktop que executa LLMs locais por trás de um servidor API compatível com OpenAI (endpoint padrão `http://127.0.0.1:1337/v1`).
...
2. Vá para [Azure AI Foundry](https://ai.azure.com/) e implante um modelo.
   ...
   :::note
   O nome da implantação deve corresponder ao nome do modelo para que o Cipher funcione corretamente.
   :::
...
5. Execute o comando `/models` para selecionar um modelo.
   ...
   Você também pode adicionar modelos através da sua configuração do Cipher.
...
armazenar o token no armazenamento de autenticação do Cipher.
:::

##### GitLab Auto-Hospedado

:::note[nota de conformidade]
Cipher usa um pequeno modelo para algumas tarefas de AI, como gerar o título da sessão.
Ele está configurado para usar gpt-5-nano por padrão, hospedado pela Zen. Para bloquear o Cipher
para usar apenas sua própria instância hospedada do GitLab, adicione o seguinte ao seu
arquivo `cipher.json`. Também é recomendado desativar o compartilhamento de sessões.
...
Mais documentação na página [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).

##### Configuração

Personalize através do `cipher.json`:
...
Para usar sua assinatura do GitHub Copilot com o Cipher:
...
Para usar o Google Vertex AI com o Cipher:
...
2. Defina as variáveis de ambiente necessárias:
   ...
   Defina-as ao executar o Cipher.
...
Caso você veja um recurso ou modelo do Helicone que não está configurado automaticamente através do Cipher, você sempre pode configurá-lo você mesmo.
...
O recurso [Sessões](https://docs.helicone.ai/features/sessions) do Helicone permite que você agrupe solicitações LLM relacionadas. Use o plugin [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) para registrar automaticamente cada conversa do Cipher como uma sessão no Helicone.
...
O plugin injeta os cabeçalhos `Helicone-Session-Id` e `Helicone-Session-Name` em suas solicitações. Na página de Sessões do Helicone, você verá cada conversa do Cipher listada como uma sessão separada.
...
Você pode configurar o Cipher para usar modelos locais através da utilidade llama-server do [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Você pode configurar o Cipher para usar modelos locais através do LM Studio.
...
Você pode configurar o Cipher para usar modelos locais através do Ollama.

:::tip
Ollama pode se configurar automaticamente para o Cipher. Consulte a [documentação de integração do Ollama](https://docs.ollama.com/integrations/opencode) para detalhes.
:::
...
Para usar o Ollama Cloud com o Cipher:
...
3. Copie a chave da API para uso no Cipher.
...
6. **Importante**: Antes de usar modelos em nuvem no Cipher, você deve puxar as informações do modelo localmente:
...
Cipher Zen é uma lista de modelos testados e verificados fornecidos pela equipe do Cipher. [Saiba mais](/docs/zen).

1. Faça login em **<a href={console}>Cipher Zen</a>** e clique em **Criar Chave da API**.

2. Execute o comando `/connect` e procure por **Cipher Zen**.

   ```txt
   /connect
   ```

3. Insira sua chave da API do Cipher.
...
4. Muitos modelos do OpenRouter são pré-carregados por padrão, execute o comando `/models` para selecionar o que você deseja.
   ...
   Você também pode adicionar modelos adicionais através da sua configuração do Cipher.
...
5. Você também pode personalizá-los através da sua configuração do Cipher. Aqui está um exemplo de especificar um provedor
...
Para usar [APIs Generativas Scaleway](https://www.scaleway.com/en/docs/generative-apis/) com o Cipher:
...
Você também pode personalizar modelos através da sua configuração do Cipher. Aqui está um exemplo de especificar a ordem de roteamento do provedor.
...
:::tip
Você pode usar qualquer provedor compatível com OpenAI com o Cipher. A maioria dos provedores de AI modernos oferece APIs compatíveis com OpenAI.
:::
...
3. Insira sua chave da API para o provedor.
   ...
   ┌  Adicionar credencial
   │
   ▲  Isso armazena apenas uma credencial para myprovider - você precisará configurá-lo no cipher.json, verifique a documentação para exemplos.
   │
...
4. Crie ou atualize seu arquivo `cipher.json` no diretório do seu projeto:
...
Os campos `limit` permitem que o Cipher entenda quanto contexto você tem disponível. Provedores padrão puxam isso automaticamente do models.dev.
...
1. **Verifique a configuração de autenticação**: Execute `Cipher auth list` para ver se as credenciais
...
2. Para provedores personalizados, verifique a configuração do Cipher e:
   - Certifique-se de que o ID do provedor usado no comando `/connect` corresponda ao ID na sua configuração do Cipher.
