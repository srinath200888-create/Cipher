...
export function fileLogger(file = path.join(Global.Path.log, "cipher.log"), id: string = runID) {
...
export function minimumLogLevel() {
  const value = process.env.CIPHER_LOG_LEVEL?.toUpperCase()
  const levels = {
    DEBUG: "Debug",
    INFO: "Info",
    WARN: "Warn",
...
export function loggers() {
  return process.env.CIPHER_PRINT_LOGS === "1" ? [fileLogger(), stderrLogger] : [fileLogger()]
}
