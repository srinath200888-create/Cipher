import type { TuiPluginApi } from "@cipher-ai/plugin/tui"
...
export function Tips(props: { api: TuiPluginApi; connected?: boolean }) {
  ...
  const shortcuts: Shortcuts = {
    ...
    statusView: useCommandShortcut("cipher.status"),
...
const TIPS: Tip[] = [
  ...
  "Run {highlight}/share{/highlight} to create a public cipher.ai link",
  ...
  "Create {highlight}cipher.json{/highlight} for server settings, and {highlight}tui.json{/highlight} for TUI",
  ...
  "Add {highlight}.md{/highlight} files to {highlight}.cipher/commands/{/highlight} for reusable prompts",
  "Use {highlight}$ARGUMENTS{/highlight}, {highlight}$1{/highlight}, {highlight}$2{/highlight} in custom commands for dynamic input",
  "Use backticks to inject shell output (e.g., {highlight}`git status`{/highlight})",
  "Add {highlight}.md{/highlight} files to {highlight}.cipher/agents/{/highlight} for specialized AI personas",
  ...
  "Create {highlight}.ts{/highlight} files in {highlight}.cipher/tools/{/highlight} to define new LLM tools",
  "Tool definitions can invoke scripts written in Python, Go, etc",
  "Add {highlight}.ts{/highlight} files to {highlight}.cipher/plugins/{/highlight} for event hooks",
  "Use plugins to send OS notifications when sessions complete",
  "Create a plugin to prevent cipher from reading sensitive files",
  "Use {highlight}cipher run{/highlight} for non-interactive scripting",
  "Use {highlight}cipher --continue{/highlight} to resume the last session",
  "Use {highlight}cipher run -f file.ts{/highlight} to attach files via CLI",
  "Use {highlight}--format json{/highlight} for machine-readable output in scripts",
  "Run {highlight}cipher serve{/highlight} for headless API access to cipher",
  "Use {highlight}cipher run --attach{/highlight} to connect to a running server",
  "Run {highlight}cipher upgrade{/highlight} to update to the latest version",
  "Run {highlight}cipher auth list{/highlight} to see all configured providers",
  "Run {highlight}cipher agent create{/highlight} for guided agent creation",
  "Use {highlight}/opencode{/highlight} in GitHub issues/PRs to trigger AI actions",
  "Run {highlight}cipher github install{/highlight} to set up the GitHub workflow",
  "Comment {highlight}/opencode fix this{/highlight} on issues to auto-create PRs",
  "Comment {highlight}/oc{/highlight} on PR code lines for targeted code reviews",
  'Use {highlight}"theme": "system"{/highlight} to match your terminal\'s colors',
  "Create JSON theme files in {highlight}.cipher/themes/{/highlight} directory",
  ...
  "Run {highlight}cipher debug config{/highlight} to troubleshoot configuration",
  ...
  "Use {highlight}/connect{/highlight} with cipher Zen for curated, tested models",
