import type { PermissionV1 } from "@cipher-ai/core/v1/permission"
import { FSUtil } from "@cipher-ai/core/fs-util"
// CLI entry point for `cipher run` and `cipher --mini`.
//
// Handles three modes:
//   1. Non-interactive (default): sends a single prompt, streams events to
//      stdout, and exits when the session goes idle.
//   2. Interactive local (`cipher --mini`): boots the split-footer direct mode
//      with an in-process server (no external HTTP).
//   3. Interactive attach (`cipher --mini --attach`): connects to a running
//      cipher server and runs interactive mode against it.
//
...
import { createCipherClient, type OpencodeClient, type ToolPart } from "@cipher-ai/sdk/v2"
...
export const RunCommand = effectCmd({
  command: "run [message..]",
  describe: "run cipher with a message",
  ...
  builder: (yargs: Argv) =>
    yargs
      ...
      .option("attach", {
        type: "string",
        describe: "attach to a running cipher server (e.g., http://localhost:4096)",
      })
      .option("password", {
        alias: ["p"],
        type: "string",
        describe: "basic auth password (defaults to CIPHER_SERVER_PASSWORD)",
      })
      .option("username", {
        alias: ["u"],
        type: "string",
        describe: "basic auth username (defaults to CIPHER_SERVER_USERNAME or 'cipher')",
      })
      .option("dir", {
        type: "string",
