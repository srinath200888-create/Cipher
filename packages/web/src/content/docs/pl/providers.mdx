---
title: Dostawcy
description: Korzystanie z dowolnego dostawcy LLM w Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher używa [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) do obsługi **ponad 75 dostawców LLM** i obsługuje uruchamianie modeli lokalnych.

Aby dodać dostawcę należy:

1. Dodaj klucze API dla dostawcy za pomocą komendy `/connect`.
2. Skonfiguruj dostawcę w konfiguracji Cipher.
...
Możesz dostosować dostawców za pomocą sekcji `provider` w swoim Cipher
config.
...
Cipher Zen to lista modeli dostarczonych przez zespół Cipher, które zostały
przetestowane i zweryfikowane, aby dobrze współpracować z Cipher. [Dowiedz się więcej](/docs/zen).

:::tip
Jeśli jesteś nowy, zalecamy rozpoczęcie od Cipher Zen.
:::

1. Uruchom polecenie `/connect` w TUI, wybierz `Cipher Zen` i przejdź do [Cipher.ai/auth](https://opencode.ai/zen).
...
Działa jak każdy inny dostawca w Cipher i jest całkowicie opcjonalny w użyciu.

---

## Cipher Go

Cipher Go to tani plan subskrypcji, który zapewnia niezawodny dostęp do popularnych modeli open coding dostarczanych przez zespół Cipher, które zostały przetestowane i zweryfikowane pod kątem dobrej współpracy z Cipher.

1. Uruchom polecenie `/connect` w TUI, wybierz `Cipher Go` i przejdź do [Cipher.ai/auth](https://opencode.ai/zen).
...
Aby używać Amazon Bedrock z Cipher:
...
2. **Skonfiguruj uwierzytelnianie** przy użyciu jednej z następujących metod:
   ...
   Ustaw jedną z tych zmiennych środowiskowych podczas uruchamiania Cipher:
   ...
   W przypadku konfiguracji specyficznej dla projektu lub trwałej użyj `cipher.json`:
...
:::info
Korzystanie z subskrypcji Claude Pro/Max w Cipher nie jest oficjalnie obsługiwane przez [Anthropic](https://anthropic.com).
:::
...
Możesz skonfigurować Cipher do korzystania z modeli lokalnych przez [Atomic Chat](https://atomic.chat) — aplikację desktopową uruchamiającą lokalne LLM-y za serwerem API zgodnym z OpenAI (domyślny endpoint `http://127.0.0.1:1337/v1`).
...
2. Przejdź do [Azure AI Foundry](https://ai.azure.com/) i wdróż model.
   ...
   :::note
   Aby kod Cipher działał poprawnie, nazwa wdrożenia musi być zgodna z nazwą modelu.
   :::
...
5. Uruchom komendę `/models`, aby wybrać model.
   ...
   Możesz także dodawać modele za pomocą konfiguracji Cipher.
...
Cipher używa małego modelu do niektórych zadań AI, takich jak generowanie tytułu sesji.
Domyślnie jest skonfigurowany do używania gpt-5-nano, hostowanego przez Zen. Aby zablokować Cipher
aby używać tylko własnej instancji hostowanej przez GitLab, dodaj następujące elementy do pliku
`cipher.json` plik. Zalecane jest również wyłączenie udostępniania sesji.
...
Więcej informacji znajdziesz na stronie [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).
...
Aby korzystać z subskrypcji GitHub Copilot z kodem Cipher:
...
Aby używać Google Vertex AI z Cipher:
...
2. Ustaw wymagane zmienne środowiskowe:
   ...
   Ustaw je podczas uruchamiania Cipher.
...
Jeśli zobaczysz funkcję lub model firmy Helicone, która nie jest konfigurowana automatycznie za pomocą Cipher, zawsze możesz ją skonfigurować samodzielnie.
...
Funkcja [Sesje](https://docs.helicone.ai/features/sessions) firmy Helicone umożliwia grupowanie powiązanych żądań LLM. Użyj wtyczki [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session), aby automatycznie rejestrować każdą konwersację Cipher jako sesję w Helicone.
...
Wtyczka wstawia nagłówki `Helicone-Session-Id` i `Helicone-Session-Name` do Twoich żądań. Na stronie Sesje Helicone każda konwersacja Cipher będzie wymieniona jako osobna sesja.
...
Możesz skonfigurować Cipher tak, aby korzystał z modeli lokalnych, za pomocą narzędzia llama-server [llama.cpp](https://github.com/ggml-org/llama.cpp)
...
Możesz skonfigurować Cipher tak, aby korzystał z modeli lokalnych poprzez LM Studio.
...
Możesz skonfigurować Cipher tak, aby korzystał z modeli lokalnych poprzez Ollamę.

:::tip
Ollama może automatycznie skonfigurować się pod kątem Cipher. Aby uzyskać szczegółowe informacje, zobacz [dokumentację integracji Ollama](https://docs.ollama.com/integrations/opencode).
:::
...
Aby korzystać z Ollama Cloud z Cipher:
...
3. Skopiuj klucz API do użycia w Cipher.
...
6. **Ważne**: Przed użyciem modeli chmurowych w Cipher musisz pobrać informacje o modelu lokalnie:
...
Cipher Zen to lista przetestowanych i zweryfikowanych modeli udostępniona przez zespół Cipher. [Dowiedz się więcej](/docs/zen).

1. Zaloguj się do **<a href={console}>Cipher Zen</a>** i kliknij **Utwórz klucz API**.

2. Uruchom polecenie `/connect` i wyszukaj **Cipher Zen**.

   ```txt
   /connect
   ```

3. Wprowadź klucz API Cipher.
...
4. Wiele modeli OpenRouter jest domyślnie załadowanych fabrycznie. Uruchom komendę `/models`, aby wybrać ten, który chcesz.
   ...
   Możesz także dodać dodatkowe modele za pomocą konfiguracji Cipher.
...
5. Możesz je także dostosować za pomocą konfiguracji Cipher. Oto przykład określenia dostawcy
...
Aby używać [generatywnych interfejsów API Scaleway](https://www.scaleway.com/en/docs/generative-apis/) z Cipher:
...
Możesz także dostosować modele za pomocą konfiguracji Cipher. Oto przykład określenia kolejności routingu dostawcy.
...
:::tip
Możesz użyć dowolnego dostawcy kompatybilnego z OpenAI z Cipher. Większość nowoczesnych dostawców sztucznej inteligencji oferuje interfejsy API kompatybilne z OpenAI.
:::
...
4. Utwórz lub zaktualizuj plik `cipher.json` w katalogu projektu:
...
Pola `limit` pozwalają Cipher zrozumieć, ile kontekstu pozostało. Standardowi dostawcy pobierają je automatycznie z models.dev.
...
1. **Sprawdź konfigurację uwierzytelniania**: Uruchom `Cipher auth list`, aby sprawdzić, czy poświadczenia
...
2. W przypadku dostawców niestandardowych sprawdź konfigurację Cipher i:
   - Upewnij się, że identyfikator dostawcy użyty w poleceniu `/connect` jest zgodny z identyfikatorem w konfiguracji Cipher.
