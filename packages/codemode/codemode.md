...
This is the living design and status document for `@cipher-ai/codemode` and its existing V2 Cipher adapter.
...
`packages/cipher/src/tool/code-mode.ts`, where nested MCP calls run the `tool.execute.before` and
