...
تتيح الأدوات لـ LLM تنفيذ إجراءات داخل قاعدة الشفرة الخاصة بك. يأتي Cipher مع مجموعة من الأدوات المدمجة، لكن يمكنك توسيعه عبر [أدوات مخصصة](/docs/custom-tools) أو [خوادم MCP](/docs/mcp-servers).
...
فيما يلي جميع الأدوات المدمجة المتاحة في Cipher.
...
:::note
هذه الأداة متاحة فقط عند ضبط `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (أو `CIPHER_EXPERIMENTAL=true`).
:::
...
:::note
هذه الأداة متاحة فقط عند استخدام مزود Cipher أو عند ضبط متغير البيئة `CIPHER_ENABLE_EXA` على أي قيمة منطقية صحيحة (مثل `true` أو `1`).

للتفعيل عند تشغيل Cipher:
