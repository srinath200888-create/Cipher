...
description: Créez des outils que le LLM peut appeler dans Cipher.
---

Les outils personnalisés sont des fonctions que vous créez et que le LLM peut appeler pendant les conversations. Ils fonctionnent avec les [outils intégrés](/docs/tools) de Cipher comme `read`, `write` et `bash`.
...
- Localement en les plaçant dans le répertoire `.cipher/tools/` de votre projet.
