...
Cipher Go je povoljna pretplata — **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno** — koja vam pruža pouzdan pristup popularnim otvorenim modelima za programiranje.

Go radi kao bilo koji drugi provajder u cipher-u. Pretplatite se na Cipher Go i
dobijete svoj API ključ. On je **potpuno opcionalan** i ne morate ga koristiti da
biste koristili Cipher.
...
:::tip
Testirali smo odabranu grupu modela i provajdera koji dobro rade sa cipher-om.
:::
...
Cipher Go vam daje pristup ovim modelima za **$5 za vaš prvi mjesec**, a zatim **$10/mjesečno**.

---

## Kako funkcioniše

Cipher Go radi kao bilo koji drugi provajder u cipher-u.

1. Prijavite se na **<a href={console}>Cipher Zen</a>**, pretplatite se na Go i
   kopirajte svoj API ključ.
2. Pokrenite komandu `/connect` u TUI-ju, odaberite `Cipher Go` i zalijepite
   svoj API ključ.
3. Pokrenite `/models` u TUI-ju da vidite listu modela dostupnih kroz Go.

:::note
Samo jedan član po radnom prostoru (workspace) može se pretplatiti na Cipher Go.
:::
...
Cipher Go uključuje sljedeća ograničenja:
...
[Model id](/docs/config/#models) u vašoj Cipher konfiguraciji
koristi format `cipher-go/<model-id>`. Na primjer, za Kimi K2.7 Code, koristili biste
`cipher-go/kimi-k2.7-code` u svojoj konfiguraciji.
...
Napravili smo Cipher Go da bismo:
...
4. Osigurali da **nema zaključavanja (no lock-in)**, omogućavajući vam da uz Cipher koristite i bilo kojeg drugog provajdera.
