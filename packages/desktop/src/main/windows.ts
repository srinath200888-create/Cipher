...
import { resolveThemeVariant } from "@cipher-ai/ui/theme/resolve"
import type { DesktopTheme } from "@cipher-ai/ui/theme/types"
...
function windowDataFile(id: string) {
  return `cipher.window.${id.replace(/[^a-zA-Z0-9._-]/g, "-")}.dat`
}
...
function wireWindowRecovery(win: BrowserWindow, name: string) {
  ...
  win.webContents.on("render-process-gone", (_event, details) => {
    ...
    void show(
      "cipher window terminated unexpectedly",
  ...
  win.on("unresponsive", () => {
    ...
    sampler.start()
    void show("cipher is not responding", "You can relaunch the app, open the logs, or keep waiting.", true)
  })
  win.on("responsive", () => {
