...
export const CIPHER_BASE_MODE = "base"
export const COMMAND_PALETTE_COMMAND = "command.palette.show"

const CIPHER_MODE_KEY = "cipher.mode"
...
export function createOpencodeModeStack(keymap: OpenTuiKeymap) {
  keymap.setData(CIPHER_MODE_KEY, CIPHER_BASE_MODE)

  const offFields = keymap.registerLayerFields({
    mode(value, ctx) {
      ctx.require(CIPHER_MODE_KEY, value)
    },
  })
  ...
  const update = () => {
    keymap.setData(CIPHER_MODE_KEY, stack.at(-1)?.mode ?? CIPHER_BASE_MODE)
  }

  const stackApi = {
    current() {
      return stack.at(-1)?.mode ?? CIPHER_BASE_MODE
    },
    push(mode: string) {
      if (disposed) return () => {}
    ...
    dispose() {
      ...
      stack.length = 0
      offFields()
      keymap.setData(CIPHER_MODE_KEY, undefined)
...
export function getOpencodeModeStack(keymap: OpenTuiKeymap) {
  ...
  if (!value) throw new Error("cipher mode stack is not registered for this keymap")
  return value
}
