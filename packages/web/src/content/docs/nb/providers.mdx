---
title: Leverandører
description: Bruke en hvilken som helst LLM-leverandør i Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher bruker [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) for å støtte **75+ LLM leverandører** og den støtter kjøring av lokale modeller.

For å legge til en leverandør må du:

1. Legg til API-nøkkelen for leverandøren ved å bruke kommandoen `/connect`.
2. Konfigurer leverandøren i cipher-konfigurasjonen.
...
Du kan tilpasse leverandørene gjennom `provider`-delen i Cipher
konfig.
...
Cipher Zen er en liste over modeller levert av cipher-teamet som har vært
testet og verifisert for å fungere godt med Cipher. [Finn ut mer](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi å starte med Cipher Zen.
:::

1. Kjør kommandoen `/connect` i TUI, velg `Cipher Zen` og gå til [Cipher.ai/auth](https://opencode.ai/zen).
...
Det fungerer som alle andre leverandører i Cipher og er helt valgfritt å bruke.

---

## Cipher Go

Cipher Go er en lavpris abonnementsplan som gir pålitelig tilgang til populære åpne kodemodeller levert av cipher-teamet som har vært
testet og verifisert for å fungere godt med Cipher.

1. Kjør kommandoen `/connect` i TUI, velg `Cipher Go`, og gå til [Cipher.ai/auth](https://opencode.ai/zen).
...
Slik bruker du Amazon Bedrock med Cipher:
...
2. **Konfigurer autentisering** ved å bruke en av følgende metoder:
   ...
   Angi en av disse miljøvariablene mens du kjører Cipher:
   ...
   For prosjektspesifikk eller vedvarende konfigurasjon, bruk `cipher.json`:
...
:::info
Å bruke Claude Pro/Max-abonnementet ditt i Cipher støttes ikke offisielt av [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Cipher til å bruke lokale modeller via [Atomic Chat](https://atomic.chat) — et skrivebordsprogram som kjører lokale LLM-er bak en OpenAI-kompatibel API-server (standard endepunkt `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en modell.
   ...
   :::note
   Distribusjonsnavnet må samsvare med modellnavnet for at Cipher skal fungere skikkelig.
   :::
...
5. Kjør kommandoen `/models` for å velge en modell.
   ...
   Du kan også legge til modeller gjennom cipher-konfigurasjonen.
...
for å lagre token i Cipher auth-lagring.
:::

##### Selvhostet GitLab

:::note[Merknad om samsvar]
Cipher bruker en liten modell for noen AI oppgaver som å generere økttittelen.
Den er konfigurert til å bruke gpt-5-nano som standard, hostet av Zen. For å låse Cipher
for kun å bruke din egen GitLab-hostede forekomst, legg til følgende i din
`cipher.json` fil. Det anbefales også å deaktivere øktdeling.
...
Mer dokumentasjon på [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) hjemmeside.

##### Konfigurasjon

Tilpass gjennom `cipher.json`:
...
Slik bruker du GitHub Copilot-abonnementet med Cipher:
...
Slik bruker du Google Vertex AI med Cipher:
...
2. Angi de nødvendige miljøvariablene:
   ...
   Sett dem mens du kjører Cipher.
...
I tilfelle du ser en funksjon eller modell fra Helicone som ikke konfigureres automatisk gjennom Cipher, kan du alltid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funksjon lar deg gruppere relaterte LLM forespørsler sammen. Bruk [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk å logge hver cipher-samtale som en økt i Helicone.
...
Programtillegget injiserer `Helicone-Session-Id` og `Helicone-Session-Name` headere i forespørslene dine. På Helicones Sessions-side vil du se hver cipher-samtale oppført som en separat økt.
...
Du kan konfigurere Cipher for å bruke lokale modeller gjennom [llama.cpp](https://github.com/ggml-org/llama.cpp) llama-server-verktøyet.
...
Du kan konfigurere Cipher for å bruke lokale modeller gjennom LM Studio.
...
Du kan konfigurere Cipher for å bruke lokale modeller gjennom Ollama.

:::tip
Ollama kan automatisk konfigurere seg selv for Cipher. Se [Ollama-integrasjonsdokumentene](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Slik bruker du Ollama Cloud med Cipher:
...
3. Kopier API-nøkkelen for bruk i Cipher.
...
6. **Viktig**: Før du bruker skymodeller i Cipher, må du hente modellinformasjonen lokalt:
...
Cipher Zen er en liste over testede og verifiserte modeller levert av cipher-teamet. [Finn ut mer](/docs/zen).

1. Logg på **<a href={console}>Cipher Zen</a>** og klikk på **Create API Key**.

2. Kjør kommandoen `/connect` og søk etter **Cipher Zen**.

   ```txt
   /connect
   ```

3. Skriv inn Cipher API-nøkkelen.
...
4. Mange OpenRouter-modeller er forhåndslastet som standard, kjør kommandoen `/models` for å velge den du ønsker.
   ...
   Du kan også legge til flere modeller gjennom cipher-konfigurasjonen.
...
5. Du kan også tilpasse dem gjennom cipher-konfigurasjonen. Her er et eksempel på å spesifisere en leverandør
...
Slik bruker du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Cipher:
...
Du kan også tilpasse modeller gjennom cipher-konfigurasjonen. Her er et eksempel på spesifisering av leverandørrutingsrekkefølge.
...
:::tip
Du kan bruke hvilken som helst OpenAI-kompatibel leverandør med Cipher. De fleste moderne AI-leverandører tilbyr OpenAI-kompatible APIer.
:::
...
4. Opprett eller oppdater `cipher.json`-filen i prosjektkatalogen:
...
`limit`-feltene lar Cipher forstå hvor mye kontekst du har igjen. Standardleverandører henter disse automatisk fra models.dev.
...
1. **Sjekk autentiseringsoppsettet**: Kjør `Cipher auth list` for å se om legitimasjonen
...
2. For tilpassede leverandører, sjekk cipher-konfigurasjonen og:
   - Sørg for at leverandør-ID-en som brukes i `/connect`-kommandoen samsvarer med ID i cipher-konfigurasjonen.
