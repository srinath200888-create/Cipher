...
Aby debugować problemy z Cipher, zacznij od sprawdzenia dzienników i danych lokalnych przechowywanych na dysku.
...
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\cipher\log`
...
Możesz ustawić poziom dziennika za pomocą opcji wiersza poleceń `--log-level`, aby uzyskać bardziej szczegółowe informacje debugowania. Na przykład `Cipher --log-level DEBUG`.

---

## Przechowywanie danych

Cipher przechowuje dane sesji i inne dane aplikacji na dysku pod adresem:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.local\share\Cipher`
...
Cipher Desktop uruchamia w tle lokalny serwer Cipher (sidecar `cipher-cli`). Większość problemów jest spowodowana nieprawidłowo działającą wtyczką, uszkodzoną pamięcią podręczną lub złymi ustawieniami serwera.
...
- Tylko macOS: menu `Cipher` -> **Załaduj ponownie przeglądarkę internetową** (pomaga, jeśli interfejs użytkownika jest pusty/zawieszony).
...
- **macOS/Linux**: `~/.config/opencode/cipher.jsonc` (lub `~/.config/opencode/cipher.json`)
- **macOS/Linux** (starsze instalacje): `~/.local/share/opencode/cipher.jsonc`
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\cipher\cipher.jsonc`
...
Cipher może także ładować lokalne wtyczki z dysku. Tymczasowo usuń je (lub zmień nazwę folderu) i uruchom ponownie aplikację komputerową:

- **Wtyczki globalne**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.config\cipher\plugins`
...
Jeśli wyłączenie wtyczek nie pomoże (lub instalacja wtyczki utknęła), wyczyść pamięć podręczną, aby Cipher mógł ją odbudować.

1. Całkowicie zamknij Cipher Desktop.
2. Usuń katalog pamięci podręcznej:

- **macOS**: Finder -> `Cmd+Shift+G` -> paste `~/.cache/cipher`
- **Linux**: usuń `~/.cache/cipher` (lub uruchom `rm -rf ~/.cache/cipher`)
- **Windows**: Naciśnij `WIN+R` i wklej `%USERPROFILE%\.cache\Cipher`

3. Uruchom ponownie Cipher Desktop.

---

### Napraw problemy z połączeniem z serwerem

Cipher Desktop może uruchomić własny serwer lokalny (domyślnie) lub połączyć się ze skonfigurowanym adresem URL serwera.
...
Jeśli Twój `cipher.json(c)` zawiera sekcję `server`, tymczasowo usuń ją i uruchom ponownie aplikację komputerową.

#### Sprawdź zmienne środowiskowe

Jeśli w swoim środowisku masz ustawiony `CIPHER_PORT`, aplikacja komputerowa spróbuje użyć tego portu dla serwera lokalnego.

- usuń `CIPHER_PORT` (lub wybierz wolny port) i uruchom ponownie.
...
W systemie Windows Cipher Desktop wymaga Microsoft Edge **WebView2 Runtime**. Jeśli aplikacja otwiera się w pustym oknie lub nie uruchamia się, zainstaluj/zaktualizuj WebView2 i spróbuj ponownie.

---

### Windows: Ogólne problemy z wydajnością

Jeśli doświadczasz niskiej wydajności, problemów z dostępem do plików lub problemów z terminalem w systemie Windows, spróbuj użyć [WSL (podsystem Windows dla systemu Linux) (./windows-wsl). WSL zapewnia środowisko Linux, które płynniej współpracuje z funkcjami Cipher.

---

### Brak powiadomień

Cipher Desktop pokazuje powiadomienia systemowe tylko wtedy, gdy:

- powiadomienia są włączone dla Cipher w ustawieniach systemu operacyjnego, oraz
...
1. Zamknij całkowicie Cipher Desktop.
2. Znajdź i usuń te pliki (znajdują się w katalogu danych aplikacji Cipher Desktop):

- `Cipher.settings.dat` (domyślny adres URL serwera na komputerze stacjonarnym)
- `Cipher.global.dat` i `Cipher.workspace.*.dat` (stan interfejsu użytkownika, taki jak najnowsze serwery/projekty)
...
3. Upewnij się, że masz najnowszą wersję z `Cipher upgrade`
...
Aby dowiedzieć się, do jakich modeli masz dostęp, uruchom `Cipher models`
...
2. Jeśli problem będzie się powtarzał, spróbuj wyczyścić zapisaną konfigurację:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.local\share\Cipher`
...
Jeśli napotkasz błędy wywołań API, może to wynikać z nieaktualnych pakietów dostawców. Cipher dynamicznie instaluje pakiety dostawców (OpenAI, Anthropic, Google itp.) w razie potrzeby i przechowuje je lokalnie w pamięci podręcznej.
...
1. Wyczyść pamięć podręczną pakietu dostawcy:
   ...
   W systemie Windows naciśnij `WIN+R` i usuń: `%USERPROFILE%\.cache\Cipher`

2. Uruchom ponownie kod Cipher, aby ponownie zainstalować najnowsze pakiety dostawców

Zmusi to Cipher do pobrania najnowszych wersji pakietów dostawców, co często rozwiązuje problemy ze zgodnością z parametrami modelu i zmianami API.
...
Cipher wykryje, czy używasz Waylanda i wolisz `wl-clipboard`, w przeciwnym razie spróbuje znaleźć narzędzia schowka w kolejności: `xclip` i `xsel`.
