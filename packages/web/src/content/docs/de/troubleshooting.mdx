...
Wenn Cipher Probleme macht, starte mit Logs und lokal gespeicherten Daten auf der Festplatte.
...
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\cipher\log`

Dateinamen enthalten Zeitstempel (z. B. `2025-01-09T123456.log`) und es bleiben die letzten 10 Logs erhalten.

Mit `--log-level` bekommst du detailliertere Diagnoseinfos, z. B. `Cipher --log-level DEBUG`.

---

## Speicher

Cipher speichert Sitzungs- und App-Daten auf der Festplatte unter:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.local\share\Cipher`
...
Cipher Desktop startet im Hintergrund einen lokalen cipher-Server (`cipher-cli`-Sidecar).
...
- Nur macOS: `Cipher`-Menue -> **Reload Webview** (hilft bei leerer/eingefrorener UI)
...
- **macOS/Linux**: `~/.config/opencode/cipher.jsonc` (oder `~/.config/opencode/cipher.json`)
- **macOS/Linux** (aeltere Installationen): `~/.local/share/opencode/cipher.jsonc`
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\cipher\cipher.jsonc`
...
Cipher kann auch lokale Plugins von der Festplatte laden. Verschiebe diese voruebergehend (oder benenne den Ordner um) und starte die Desktop-App neu:

- **Globale Plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.config\cipher\plugins`
...
Wenn das Deaktivieren von Plugins nicht hilft (oder eine Plugin-Installation haengt), leere den Cache, damit Cipher ihn neu aufbauen kann.

1. Beende Cipher Desktop komplett.
2. Loesche das Cache-Verzeichnis:

- **macOS**: Finder -> `Cmd+Shift+G` -> einfuegen: `~/.cache/cipher`
- **Linux**: loesche `~/.cache/cipher` (oder fuehre aus: `rm -rf ~/.cache/cipher`)
- **Windows**: Druecke `WIN+R` und fuege ein: `%USERPROFILE%\.cache\Cipher`

3. Starte Cipher Desktop neu.

---

### Server-Verbindungsprobleme beheben

Cipher Desktop kann entweder einen eigenen lokalen Server starten (Standard) oder sich mit einer konfigurierten Server-URL verbinden.
...
Wenn deine `cipher.json(c)` einen `server`-Abschnitt enthaelt, entferne ihn voruebergehend und starte die Desktop-App neu.

#### Umgebungsvariablen prüfen

Wenn du `CIPHER_PORT` in deiner Umgebung gesetzt hast, versucht die Desktop-App diesen Port fuer den lokalen Server zu nutzen.

- Setze `CIPHER_PORT` zurueck (oder waehle einen freien Port) und starte neu.
...
Unter Windows benoetigt Cipher Desktop die Microsoft Edge **WebView2 Runtime**. Wenn die App ein leeres Fenster zeigt oder nicht startet, installiere/aktualisiere WebView2 und versuche es erneut.

---

### Windows: Allgemeine Performance-Probleme

Wenn du langsame Performance, Dateizugriffsprobleme oder Terminal-Probleme unter Windows hast, versuche [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL bietet eine Linux-Umgebung, die nahtloser mit cipher-Features funktioniert.

---

### Benachrichtigungen werden nicht angezeigt

Cipher Desktop zeigt Systembenachrichtigungen nur wenn:

- Benachrichtigungen fuer Cipher in den OS-Einstellungen aktiviert sind, und
...
1. Beende Cipher Desktop.
2. Finde und loesche diese Dateien (im App-Data-Verzeichnis von Cipher Desktop):

- `Cipher.settings.dat` (desktop default server URL)
- `Cipher.global.dat` und `Cipher.workspace.*.dat` (UI state like recent servers/projects)
...
Wenn du Probleme mit Cipher hast:
...
Um zu sehen, auf welche Modelle du Zugriff hast, fuehre `Cipher models` aus.
...
2. Wenn das Problem besteht, versuche deine gespeicherte Konfiguration zu loeschen:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.local\share\Cipher`
...
Wenn du API-Call-Fehler erhaeltst, kann das an veralteten Provider-Paketen liegen. Cipher installiert Provider-Pakete (OpenAI, Anthropic, Google, etc.) dynamisch bei Bedarf und cached sie lokal.
...
1. Leere den Provider-Paket-Cache:
   ...
   Unter Windows druecke `WIN+R` und loesche: `%USERPROFILE%\.cache\Cipher`

2. Starte Cipher neu, um die neuesten Provider-Pakete zu installieren

Dies zwingt Cipher, die neuesten Versionen der Provider-Pakete herunterzuladen, was oft Kompatibilitaetsprobleme mit Modellparametern und API-Aenderungen loest.
...
Cipher erkennt, ob du Wayland nutzt und bevorzugt `wl-clipboard`, sonst versucht es `xclip` und `xsel` (in dieser Reihenfolge).
