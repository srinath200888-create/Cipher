---
title: Plugins
description: Skriv dine egne plugins for at udvide Cipher.
---

Plugins giver dig mulighed for at udvide Cipher ved at tilslutte sig forskellige begivenheder og tilpasse adfærd. Du kan oprette plugins for at tilføje nye funktioner, integrere med eksterne tjenester eller ændre ciphers standardadfærd.
...
- `.cipher/plugins/` - Plugins på projektniveau
...
1. Global konfiguration (`~/.config/opencode/cipher.json`)
2. Projektkonfiguration (`cipher.json`)
3. Global plugin-mappe (`~/.config/opencode/plugins/`)
4. Projekt plugin bibliotek (`.cipher/plugins/`)
...
Cipher kører `bun install` ved opstart for at installere disse. Dine plugins og værktøjer kan derefter importere dem.
...
- `client`: En Cipher SDK klient til interaktion med AI.
...
Her er nogle eksempler på plugins, du kan bruge til at udvide Cipher.
...
:::note
Hvis du bruger Cipher desktop-appen, kan den sende systemmeddelelser automatisk, når et svar er klar, eller når en session fejler.
:::
...
Undgå Cipher i at læse `.env` filer:
...
Plugins kan også tilføje brugerdefinerede værktøjer til Cipher:
...
`tool`-hjælperen opretter et brugerdefineret værktøj, som Cipher kan kalde. Det tager en Zod-skemafunktion og returnerer en værktøjsdefinition med:

- `description`: Hvad værktøjet gør
- `args`: Zod-skema for værktøjets argumenter
- `execute`: Funktion, der kører, når værktøjet kaldes

Dine tilpassede værktøjer vil være tilgængelige for Cipher sammen med indbyggede værktøjer.
