...
Cipher ile ilgili bir sorunu ayiklamak icin once loglari ve diskte tuttugu yerel verileri kontrol edin.
...
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\cipher\log` yapistirin

Log dosyalari zaman damgasiyla adlandirilir (ornegin `2025-01-09T123456.log`) ve en yeni 10 log dosyasi tutulur.

Daha ayrintili ayiklama bilgisi icin `--log-level` komut satiri secenegini kullanabilirsiniz. Ornek: `Cipher --log-level DEBUG`.

---

## Depolama

Cipher, oturum verilerini ve diger uygulama verilerini diskte su konumda saklar:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.local\share\Cipher` yapistirin
...
Cipher Desktop arka planda yerel bir Cipher sunucusu (`cipher-cli` sidecar) calistirir. Sorunlarin cogu bozuk bir plugin, hasarli cache veya hatali sunucu ayarindan kaynaklanir.

### Hızlı kontroller

- Uygulamayi tamamen kapatip yeniden acin
- Uygulama hata ekrani gosteriyorsa **Restart**'a tiklayip hata detaylarini kopyalayin
- Yalnizca macOS: `Cipher` menusu -> **Reload Webview** (arayuz bos/donukse yardimci olur)
...
- **macOS/Linux**: `~/.config/opencode/cipher.jsonc` (veya `~/.config/opencode/cipher.json`)
- **macOS/Linux** (eski kurulumlar): `~/.local/share/opencode/cipher.jsonc`
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\cipher\cipher.jsonc` yapistirin
...
Cipher diskten yerel plugin de yukleyebilir. Bu dizinleri gecici olarak tasiyin (veya klasoru yeniden adlandirin) ve uygulamayi yeniden baslatin:

- **Global plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.config\cipher\plugins` yapistirin
...
Pluginleri kapatmak ise yaramazsa (veya plugin kurulumu takili kaldiysa), cipher'un cache'i yeniden olusturmasi icin cache'i temizleyin.

1. Cipher Desktop'u tamamen kapatin
2. Cache dizinini silin:

- **macOS**: Finder -> `Cmd+Shift+G` -> `~/.cache/cipher`
- **Linux**: `~/.cache/cipher` dizinini silin (veya `rm -rf ~/.cache/cipher` calistirin)
- **Windows**: `WIN+R` tuslayip `%USERPROFILE%\.cache\Cipher` yapistirin

3. Cipher Desktop'u yeniden baslatin

---

### Sunucu bağlantı sorunlarını düzeltin

Cipher Desktop ya kendi yerel sunucusunu baslatir (varsayilan) ya da sizin tanimladiginiz bir sunucu URL'sine baglanir.
...
`cipher.json(c)` dosyanizda `server` bolumu varsa gecici olarak kaldirin ve desktop uygulamasini yeniden baslatin.

#### Ortam degiskenlerini kontrol edin

Ortamda `CIPHER_PORT` ayarliysa desktop uygulamasi yerel sunucu icin o portu kullanmaya calisir.

- `CIPHER_PORT` degerini kaldirin (veya bos bir port secin) ve yeniden baslatin
...
Windows'ta Cipher Desktop, Microsoft Edge **WebView2 Runtime** gerektirir. Uygulama bos pencereyle aciliyorsa veya hic baslamiyorsa WebView2'yi kurup/guncelleyip tekrar deneyin.

---

### Windows: Genel performans sorunları

Windows'ta yavaslik, dosya erisim sorunlari veya terminal problemleri yasiyorsaniz [WSL (Windows Subsystem for Linux)](/docs/windows-wsl) kullanmayi deneyin. WSL, Cipher ozellikleriyle daha sorunsuz calisan bir Linux ortami saglar.

---

### Bildirimler görünmüyor

Cipher Desktop sistem bildirimlerini yalnizca su durumlarda gosterir:

- Isletim sistemi ayarlarinizda Cipher icin bildirimler etkinse
...
1. Cipher Desktop'u kapatin
2. Su dosyalari bulun ve silin (Cipher Desktop uygulama veri dizininde yer alirlar):

- `Cipher.settings.dat` (desktop varsayilan sunucu URL'si)
- `Cipher.global.dat` ve `Cipher.workspace.*.dat` (son sunucular/projeler gibi UI durumu)
...
Cipher ile ilgili bir sorun yasiyorsaniz:
...
3. `Cipher upgrade` ile en guncel surumu kullandiginizdan emin olun
...
Erisiminiz olan modelleri gormek icin `Cipher models` calistirin.
...
2. Sorun surerse kayitli konfigurasyonu temizlemeyi deneyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.local\share\Cipher`
...
API cagrisi hatalari aliyorsaniz bunun nedeni guncel olmayan provider paketleri olabilir. Cipher, provider paketlerini (OpenAI, Anthropic, Google vb.) gerektikce dinamik yukler ve yerelde onbellekler.
...
1. Provider paket cache'ini temizleyin:
   ...
   Windows'ta `WIN+R` tuslayip su konumu silin: `%USERPROFILE%\.cache\Cipher`

2. En guncel provider paketlerini yeniden kurmak icin cipher'u yeniden baslatin

Bu, cipher'un en yeni provider paket surumlerini indirmesini zorlar ve model parametresi/API degisikliklerinden kaynakli uyumsuzluklari cogu zaman cozer.
...
Cipher, Wayland kullandiginizi algilarsa `wl-clipboard` tercihi yapar. Aksi halde sirayla `xclip` ve `xsel` araclarini arar.
