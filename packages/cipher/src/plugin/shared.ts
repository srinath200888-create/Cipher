...
export const DEPRECATED_PLUGIN_PACKAGES = ["cipher-openai-codex-auth", "cipher-copilot-auth"]
...
export async function checkPluginCompatibility(target: string, opencodeVersion: string, pkg?: PluginPackage) {
  ...
  const range = engines.cipher
  if (typeof range !== "string") return
  if (!semver.satisfies(opencodeVersion, range)) {
    throw new Error(`Plugin requires cipher ${range} but running ${opencodeVersion}`)
  }
}
