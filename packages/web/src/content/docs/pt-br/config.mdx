---
title: Configuração
description: Usando a configuração JSON do Cipher.
---

Você pode configurar o Cipher usando um arquivo de configuração JSON.

---

## Formato

O Cipher suporta os formatos **JSON** e **JSONC** (JSON com Comentários).
...
1. **Configuração remota** (de `.well-known/cipher`) - padrões organizacionais
2. **Configuração global** (`~/.config/opencode/cipher.json`) - preferências do usuário
3. **Configuração personalizada** (`CIPHER_CONFIG` var de ambiente) - substituições personalizadas
4. **Configuração do projeto** (`cipher.json` no projeto) - configurações específicas do projeto
5. **Diretórios `.opencode`** - agentes, comandos, plugins
6. **Configuração inline** (`CIPHER_CONFIG_CONTENT` var de ambiente) - substituições em tempo de execução
...
:::note
Os diretórios `.opencode` e `~/.config/cipher` usam **nomes no plural** para subdiretórios: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, e `themes/`. Nomes no singular (por exemplo, `agent/`) também são suportados para compatibilidade retroativa.
:::
...
As organizações podem fornecer configuração padrão através do endpoint `.well-known/cipher`. Isso é buscado automaticamente quando você se autentica com um provedor que o suporta.
...
Coloque sua configuração global do Cipher em `~/.config/opencode/cipher.json`. Use a configuração global para preferências de todo o usuário, como provedores, modelos e permissões.
...
Adicione `cipher.json` na raiz do seu projeto. A configuração do projeto tem a maior precedência entre os arquivos de configuração padrão - ela substitui tanto as configurações globais quanto as remotas.
...
Quando o Cipher é iniciado, ele procura um arquivo de configuração no diretório atual ou sobe até o diretório Git mais próximo.
...
Especifique um caminho de arquivo de configuração personalizado usando a variável de ambiente `CIPHER_CONFIG`.
...
Especifique um diretório de configuração personalizado usando a variável de ambiente `CIPHER_CONFIG_DIR`. Este diretório será pesquisado por agentes, comandos, modos e plugins, assim como o diretório padrão `.opencode`, e deve seguir a mesma estrutura.
...
O esquema de configuração do servidor/tempo de execução é definido em [**`Cipher.ai/config.json`**](https://opencode.ai/config.json).

A configuração do TUI usa [**`Cipher.ai/tui.json`**](https://opencode.ai/tui.json).
...
Use `CIPHER_TUI_CONFIG` para apontar para um arquivo de configuração TUI personalizado.

Chaves legadas `theme`, `keybinds` e `tui` em `cipher.json` estão obsoletas e são migradas automaticamente quando possível.
...
Você pode configurar as configurações do servidor para os comandos `Cipher serve` e `Cipher web` através da opção `server`.
...
- `mdns` - Habilitar descoberta de serviço mDNS. Isso permite que outros dispositivos na rede descubram seu servidor Cipher.
- `mdnsDomain` - Nome de domínio personalizado para o serviço mDNS. O padrão é `Cipher.local`. Útil para executar várias instâncias na mesma rede.
...
Você pode configurar os provedores e modelos que deseja usar em sua configuração do Cipher através das opções `provider`, `model` e `small_model`.
...
A opção `small_model` configura um modelo separado para tarefas leves, como geração de títulos. Por padrão, o Cipher tenta usar um modelo mais barato se um estiver disponível do seu provedor, caso contrário, ele recua para seu modelo principal.
...
Você também pode definir agentes usando arquivos markdown em `~/.config/opencode/agents/` ou `.cipher/agents/`. [Saiba mais aqui](/docs/agents).
...
O agente padrão deve ser um agente primário (não um subagente). Isso pode ser um agente embutido como `"build"` ou `"plan"`, ou um [agente personalizado](/docs/agents) que você definiu. Se o agente especificado não existir ou for um subagente, o Cipher recuará para `"build"` com um aviso.

Essa configuração se aplica a todas as interfaces: TUI, CLI (`Cipher run`), aplicativo desktop e GitHub Action.
...
Você também pode definir comandos usando arquivos markdown em `~/.config/opencode/commands/` ou `.cipher/commands/`. [Saiba mais aqui](/docs/commands).
...
O Cipher fará o download automaticamente de quaisquer novas atualizações quando for iniciado. Você pode desabilitar isso com a opção `autoupdate`.
...
Por padrão, o Cipher **permite todas as operações** sem exigir aprovação explícita. Você pode mudar isso usando a opção `permission`.
...
[Plugins](/docs/plugins) estendem o Cipher com ferramentas, hooks e integrações personalizadas.

Coloque arquivos de plugin em `.cipher/plugins/` ou `~/.config/opencode/plugins/`. Você também pode carregar plugins do npm através da opção `plugin`.
...
Isso é útil quando você deseja restringir o Cipher para usar apenas provedores específicos, em vez de desabilitá-los um a um.
