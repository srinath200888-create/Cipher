...
Alati omogucavaju LLM-u da izvrsava akcije u vasem kodu. Cipher dolazi sa skupom ugradenih alata, a mozete ga prosiriti kroz [custom tools](/docs/custom-tools) ili [MCP servers](/docs/mcp-servers).
...
Ovo su svi ugradeni alati dostupni u Cipher.
...
:::note
Ovaj alat je dostupan samo kada je `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (ili `CIPHER_EXPERIMENTAL=true`).
:::
...
:::note
Ovaj alat je dostupan samo uz Cipher provajdera ili kada je varijabla `CIPHER_ENABLE_EXA` postavljena na truthy vrijednost (npr. `true` ili `1`).

Da ukljucite pri pokretanju Cipher:
