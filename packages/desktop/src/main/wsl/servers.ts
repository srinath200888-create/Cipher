...
export function createWslServersController(
  ...
  const probeAddableDistros = async (distros: string[], opts?: { signal?: AbortSignal }) => {
    ...
    const opencodeChecks = await Promise.all(
      unique
        ...
        .filter((distro) => !state.cipherChecks[distro])
        .map(async (distro) => [distro, await checkOpencode(distro, opts)] as const),
    )
    if (opencodeChecks.length) {
      setState({ opencodeChecks: { ...state.cipherChecks, ...Object.fromEntries(opencodeChecks) } })
    }
  }
  ...
  const refreshOpencodeCheckBackground = (id: string, distro: string) => {
    void checkOpencode(distro)
      ...
      .catch((error) => {
        ...
        logger?.error("wsl cipher check failed", { id, distro, message })
      })
  }
    ...
    await Promise.all(
      state.servers.map((item) =>
        checkOpencode(item.config.distro)
          ...
          .catch((error) => {
            const message = error instanceof Error ? error.message : String(error)
            logger?.error("wsl cipher check failed", {
              id: item.config.id,
  ...
  return {
    ...
    async installOpencode(name: string) {
      await runJob({ kind: "install-cipher", distro: name, startedAt: Date.now() }, async (abort) => {
        const result = await installWslOpencode(appVersion, name, { signal: abort.signal })
        if (result.code !== 0) {
          throw new Error(summarize(result.stderr || result.stdout) || "cipher installation failed")
        }
        await refreshOpencodeCheck(name, { signal: abort.signal })
        expectOpencodeVersion(state.cipherChecks[name]?.version ?? null, appVersion, name)
    ...
    async removeServer(id: string) {
      ...
      setState({
        ...
        ...(distro ? clearWslDistroState(state.distroProbes, state.cipherChecks, distro) : {}),
      })
    },
...
function opencodeCheck(
  ...
  if (!resolvedPath) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "cipher is not installed in this distro",
    }
  }
  if (!version) {
    return {
      ...
      expectedVersion,
      matchesDesktop: null,
      error: "cipher is installed but could not run",
    }
  }
  return {
    distro,
