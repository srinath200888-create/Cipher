...
For at fejlfinde problemer med Cipher, start med at tjekke logfilerne og de lokale data, den gemmer på disken.
...
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\cipher\log`

Logfiler navngives med tidsstempler (f.eks. `2025-01-09T123456.log`) og de sidste 10 logfiler beholdes.

Du kan angive logniveauet med kommandolinjeflaget `--log-level` for at få mere detaljeret fejlfindingsinformation. For eksempel `Cipher --log-level DEBUG`.

---

## Lagring

Cipher gemmer sessionsdata og andre applikationsdata på disken på:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.local\share\Cipher`
...
Cipher Desktop kører en lokal cipher-server (`cipher-cli` sidevognen) i baggrunden. De fleste problemer er forårsaget af et plugin, der fungerer dårligt, en ødelagt cache eller en dårlig serverindstilling.

### Hurtige tjek

- Afslut og start appen på ny.
- Hvis appen viser en fejlskærm, klikker du på **Start på ny** og kopierer fejldetaljerne.
- Kun macOS: `Cipher`-menu -> **Genindlæs webvisning** (hjælper hvis UI er tom/frosset).
...
- **macOS/Linux**: `~/.config/opencode/cipher.jsonc` (eller `~/.config/opencode/cipher.json`)
- **macOS/Linux** (ældre installationer): `~/.local/share/opencode/cipher.jsonc`
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\cipher\cipher.jsonc`
...
Cipher kan også indlæse lokale plugins fra disken. Flyt disse midlertidigt væk (eller giv mappen nyt navn) og start skrivebordsappen på ny:

- **Globale plugins**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.config\cipher\plugins`
...
Hvis deaktivering af plugins ikke hjælper (eller en plugin-installation sidder fast), tøm cachen så Cipher kan genopbygge den.

1. Afslut Cipher Desktop helt.
2. Slet cache-mappen:

- **macOS**: Finder -> `Cmd+Shift+G` -> indsæt `~/.cache/cipher`
- **Linux**: slet `~/.cache/cipher` (eller kør `rm -rf ~/.cache/cipher`)
- **Windows**: Tryk `WIN+R` og indsæt `%USERPROFILE%\.cache\Cipher`

3. Start Cipher Desktop på ny.

---

### Løs problemer med serverforbindelse

Cipher Desktop kan enten starte sin egen lokale server (standard) eller forbinde til en server URL, du har konfigureret.
...
Hvis `cipher.json(c)` indeholder en `server`-del, fjern den midlertidigt og start skrivebordsappen på ny.

#### Tjek miljøvariabler

Hvis du har `CIPHER_PORT` sat i dit miljø, vil skrivebordsappen prøve at bruge den port for den lokale server.

- Deaktiver `CIPHER_PORT` (eller vælg en ledig port) og start på ny.
...
På Windows kræver Cipher Desktop Microsoft Edge **WebView2 Runtime**. Hvis appen åbnes i et tomt vindue eller ikke starter, installer/opdater WebView2 og prøv igen.

---

### Windows: Generelle ydeevneproblemer

Hvis du oplever langsom ydeevne, problemer med filadgang eller terminalproblemer på Windows, kan du prøve at bruge [WSL (Windows Subsystem for Linux)](/docs/windows-wsl). WSL giver et Linux-miljø som fungerer mere sømløst med funktionerne i Cipher.

---

### Meddelelser vises ikke

Cipher Desktop viser kun systemvarsler når:

- varsler er aktiveret for Cipher i dine OS-indstillinger, og
...
1. Afslut Cipher Desktop.
2. Find og slet disse filer (de findes i Cipher Desktop-appens datamappe):

- `Cipher.settings.dat` (skrivebordsstandardserver URL)
- `Cipher.global.dat` og `Cipher.workspace.*.dat` (UI tilstand som nylige servere/projekter)
...
Hvis du oplever problemer med Cipher:
...
For at finde ud af hvilke modeller du har adgang til, kør `Cipher models`
...
2. Hvis problemet vedvarer, prøv at tømme den gemte konfiguration:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.local\share\Cipher`
...
Hvis du støder på API-kaldsfejl, kan dette skyldes forældede udbyderpakker. Cipher installerer dynamisk udbyderpakker (OpenAI, Anthropic, Google, etc.) efter behov og cacher dem lokalt.
...
1. Tøm udbyderens pakkecache:
   ...
   På Windows, tryk `WIN+R` og slet: `%USERPROFILE%\.cache\Cipher`

2. Start Cipher på ny for at installere de nyeste udbyderpakker på ny

Dette vil tvinge Cipher til at downloade de nyeste versioner af udbyderpakkerne, som ofte løser kompatibilitetsproblemer med modelparametre og API-ændringer.
...
Cipher vil opdage om du bruger Wayland og foretrække `wl-clipboard`, ellers vil den prøve at finde udklipsholderværktøjer i rækkefølgen: `xclip` og `xsel`.
