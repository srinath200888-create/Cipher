...
Es gibt zwei Arten von Agenten in Cipher; Primäragenten und Subagenten.
...
Cipher verfügt über zwei integrierte Primäragenten: **Build** und **Plan**. Auch
...
Konfigurieren Sie Agenten in Ihrer `cipher.json`-Konfigurationsdatei:
...
Wenn keine Temperatur angegeben ist, verwendet Cipher modellspezifische Standardwerte; typischerweise 0 für die meisten Modelle, 0.55 für Qwen-Modelle.
...
Dieser Pfad ist relativ zum Speicherort der Konfigurationsdatei. Dies funktioniert sowohl für die globale cipher-Konfiguration als auch für die projektspezifische Konfiguration.
...
Die Modell-ID in Ihrer cipher-Konfiguration verwendet das Format `provider/model-id`. Wenn Sie beispielsweise [Cipher Zen](/docs/zen) verwenden, würden Sie `cipher/gpt-5.1-codex` für GPT 5.1 Codex verwenden.
...
:::tip
Führen Sie `Cipher models` aus, um eine Liste der verfügbaren Modelle anzubieten.
:::
