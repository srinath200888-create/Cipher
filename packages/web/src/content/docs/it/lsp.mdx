---
title: Server LSP
description: Cipher si integra con i tuoi server LSP.
---

Cipher può integrarsi con server Language Server Protocol (LSP) per usare la diagnostica come feedback per l'agente.

---

## Integrati

Cipher include diversi server LSP integrati per linguaggi popolari:
...
:::note
Puoi disabilitare il download automatico dei server LSP impostando la variabile d'ambiente `CIPHER_DISABLE_LSP_DOWNLOAD` a `true`.
:::
...
Quando LSP è abilitato e Cipher apre un file:
...
Puoi abilitare e personalizzare i server LSP tramite la sezione `lsp` nella configurazione di Cipher.
