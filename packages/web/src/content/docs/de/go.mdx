...
Cipher Go ist ein kostengünstiges Abonnement — **5 $ für deinen ersten Monat**, danach **10 $/Monat** —, das dir zuverlässigen Zugriff auf beliebte offene Coding-Modelle bietet.

Go funktioniert wie jeder andere Provider in Cipher. Du abonnierst Cipher Go und
...
Cipher Go bietet dir Zugriff auf diese Modelle für **5 $ im ersten Monat**, danach **10 $/Monat**.

---

## Wie es funktioniert

Cipher Go funktioniert wie jeder andere Provider in Cipher.

1. Du meldest dich bei **<a href={console}>Cipher Zen</a>** an, abonnierst Go und kopierst deinen API-Key.
2. Du führst den Befehl `/connect` in der TUI aus, wählst `Cipher Go` und fügst deinen API-Key ein.
3. Führe `/models` in der TUI aus, um die Liste der über Go verfügbaren Modelle zu sehen.

:::note
Nur ein Mitglied pro Workspace kann Cipher Go abonnieren.
:::
...
Cipher Go beinhaltet die folgenden Limits:
...
Die [Modell-ID](/docs/config/#models) in deiner Cipher Config verwendet das Format `cipher-go/<model-id>`. Für Kimi K2.7 Code würdest du beispielsweise `cipher-go/kimi-k2.7-code` in deiner Config verwenden.
...
Wir haben Cipher Go entwickelt, um:
...
4. **Keinen Lock-in-Effekt** zu haben, indem wir dir ermöglichen, auch jeden anderen Provider mit Cipher zu nutzen.
