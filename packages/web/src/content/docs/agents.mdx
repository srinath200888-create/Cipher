...
There are two types of agents in Cipher; primary agents and subagents.
...
Cipher comes with two built-in primary agents, **Build** and **Plan**. We'll
look at these below.
...
Cipher comes with three built-in subagents, **General**, **Explore**, and **Scout**. We'll look at this below.

---

## Built-in

Cipher comes with two built-in primary agents and three built-in subagents.
...
A read-only agent for external docs and dependency research. Use this when you need to clone a dependency repository into cipher's managed cache, inspect library source, or cross-reference local code against upstream implementations without modifying your workspace.
...
Configure agents in your `cipher.json` config file:
...
If no temperature is specified, Cipher uses model-specific defaults; typically 0 for most models, 0.55 for Qwen models.
...
This path is relative to where the config file is located. So this works for both the global Cipher config and the project specific config.
...
The model ID in your Cipher config uses the format `provider/model-id`. For example, if you're using [Cipher Zen](/docs/zen), you would use `cipher/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Run `Cipher models` to see a list of the available models.
:::
