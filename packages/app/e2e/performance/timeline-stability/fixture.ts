...
import { SessionStatusEvent } from "@cipher-ai/schema/session-status-event"
import { SessionV1 } from "@cipher-ai/schema/session-v1"
import type {
  AssistantMessage,
  GlobalEvent,
  Message,
...
export const model = { providerID: "cipher", modelID: "claude-opus-4-6", variant: "max" }
...
export async function setupTimeline(
  ...
  if (input.locale) {
    await page.addInitScript((locale) => {
      localStorage.setItem("cipher.global.dat:language", JSON.stringify({ locale }))
    }, input.locale)
  }
