...
import { AbsolutePath, Agent, Location, Model, cipher, Prompt, Session, SessionMessage } from "../src/effect"
...
test("sessions.get returns the decoded Effect projection", async () => {
  ...
  const result = await Effect.gen(function* () {
    const client = yield* cipher.make({ baseUrl: "http://localhost:3000" })
