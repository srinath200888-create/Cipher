...
import { LLMEvent, ToolFailure } from "@cipher-ai/llm"
import { LLMClient, RequestExecutor, WebSocketExecutor, type LLMClientShape } from "@cipher-ai/llm/route"
...
describe("session.llm-native.request", () => {
  ...
  test("selects native request routes for provider packages", () => {
    ...
    const compatible = LLMNative.model({
      model: {
        ...baseModel,
        providerID: ProviderV2.ID.make("cipher"),
  ...
  test("only enables native runtime for supported OpenAI API-key models", () => {
    ...
    expect(
      LLMNativeRuntime.status({
        model: { ...baseModel, providerID: ProviderV2.ID.make("cipher") },
        provider: { ...providerInfo, id: ProviderV2.ID.make("cipher") },
        auth: undefined,
      }),
    ).toMatchObject({
      type: "supported",
