...
Da biste otklonili probleme s Cipher, počnite provjeravanjem dnevnika i lokalnih podataka koje pohranjuje na disku.
...
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\cipher\log`

Datoteke evidencije se imenuju vremenskim oznakama (npr. `2025-01-09T123456.log`) i čuvaju se najnovijih 10 datoteka dnevnika.

Možete postaviti nivo dnevnika pomoću opcije komandne linije `--log-level` da biste dobili detaljnije informacije o otklanjanju grešaka. Na primjer, `Cipher --log-level DEBUG`.

---

## Pohrana

Cipher pohranjuje podatke o sesiji i druge podatke aplikacije na disku na:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.local\share\Cipher`
...
Cipher Desktop pokreće lokalni Cipher server (`cipher-cli` sidecar) u pozadini. Većina problema je uzrokovana nedostatkom dodatka, oštećenom keš memorijom ili lošim postavkama servera.
...
- samo za macOS: `Cipher` meni -> **Ponovo učitaj Webview** (pomaže ako je korisnički interfejs prazan/zamrznut).
...
- **macOS/Linux**: `~/.config/opencode/cipher.jsonc` (ili `~/.config/opencode/cipher.json`)
- **macOS/Linux** (starije instalacije): `~/.local/share/opencode/cipher.jsonc`
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\cipher\cipher.jsonc`
...
Cipher također može učitati lokalne dodatke s diska. Privremeno ih maknite s puta (ili preimenujte folder) i ponovo pokrenite desktop aplikaciju:

- **Globalni dodaci**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.config\cipher\plugins`
...
Ako onemogućavanje dodataka ne pomogne (ili se instalacija dodatka zaglavila), obrišite keš memoriju kako bi ga Cipher mogao ponovo izgraditi.

1. Potpuno zatvorite Cipher Desktop.
2. Izbrišite keš direktorij:

- **macOS**: Finder -> `Cmd+Shift+G` -> zalijepi `~/.cache/cipher`
- **Linux**: obrišite `~/.cache/cipher` (ili pokrenite `rm -rf ~/.cache/cipher`)
- **Windows**: Pritisnite `WIN+R` i zalijepite `%USERPROFILE%\.cache\Cipher`

3. Ponovo pokrenite Cipher Desktop.

---

### Rješavanje problema sa vezom na serveru

Cipher Desktop može ili pokrenuti svoj lokalni server (podrazumevano) ili se povezati na URL servera koji ste konfigurisali.
...
Ako vaš `cipher.json(c)` sadrži odjeljak `server`, privremeno ga uklonite i ponovo pokrenite desktop aplikaciju.

#### Provjerite varijable okruženja

Ako ste postavili `CIPHER_PORT` u svom okruženju, desktop aplikacija će pokušati da koristi taj port za lokalni server.

- Poništite `CIPHER_PORT` (ili odaberite slobodan port) i ponovo pokrenite.
...
Na Windows-u, Cipher Desktop zahtijeva Microsoft Edge **WebView2 Runtime**. Ako se aplikacija otvori u praznom prozoru ili se ne pokrene, instalirajte/ažurirajte WebView2 i pokušajte ponovo.

---

### Windows: Opšti problemi sa performansama

Ako imate spore performanse, probleme s pristupom datotekama ili probleme s terminalom na Windows-u, pokušajte koristiti [WSL (Windows podsistem za Linux)](/docs/windows-wsl). WSL pruža Linux okruženje koje radi neprimetnije sa Cipher karakteristikama.

---

### Obavještenja se ne prikazuju

Cipher Desktop prikazuje sistemska obavještenja samo kada:

- obavještenja su omogućena za Cipher u postavkama vašeg OS-a, i
...
1. Zatvorite Cipher Desktop.
2. Pronađite i izbrišite ove datoteke (oni žive u direktoriju podataka Cipher Desktop aplikacije):

- `Cipher.settings.dat` (URL zadanog servera za desktop)
- `Cipher.global.dat` i `Cipher.workspace.*.dat` (stanje korisničkog interfejsa poput nedavnih servera/projekata)
...
Da saznate kojim modelima imate pristup, pokrenite `Cipher models`
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.local\share\Cipher`
...
Ako naiđete na greške API poziva, to može biti zbog zastarjelih paketa dobavljača. Cipher dinamički instalira pakete dobavljača (OpenAI, Anthropic, Google, itd.) po potrebi i kešira ih lokalno.
...
Na Windows-u pritisnite `WIN+R` i izbrišite: `%USERPROFILE%\.cache\Cipher`

2. Ponovo pokrenite Cipher da ponovo instalirate najnovije pakete dobavljača

Ovo će prisiliti Cipher da preuzme najnovije verzije paketa dobavljača, što često rješava probleme kompatibilnosti s parametrima modela i promjenama API-ja.
...
Cipher će otkriti da li koristite Wayland i preferirate `wl-clipboard`, u suprotnom će pokušati pronaći alate međuspremnika po redoslijedu: `xclip` i `xsel`.
