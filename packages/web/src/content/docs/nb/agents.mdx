...
Det er to typer agenter i Cipher; primære agenter og underagenter.
...
Cipher kommer med to innebygde primære agenter, **Build** og **Plan**. Vi ser nærmere på disse nedenfor.
...
Cipher kommer med tre innebygde underagenter, **General**, **Explore** og **Scout**. Vi skal se på dette nedenfor.

---

## Innebygd

Cipher kommer med to innebygde primære agenter og tre innebygde underagenter.
...
En skrivebeskyttet agent for ekstern dokumentasjon og forskning på avhengigheter. Bruk denne når du trenger å klone et avhengighetsrepo inn i Cipher sin administrerte cache, inspisere kildekoden til et bibliotek eller kryssjekke lokal kode mot upstream-implementasjoner uten å endre arbeidsområdet ditt.
...
Konfigurer agenter i konfigurasjonsfilen `cipher.json`:
...
Hvis ingen temperatur er spesifisert, bruker Cipher modellspesifikke standardinnstillinger; typisk 0 for de fleste modeller, 0,55 for Qwen-modeller.
...
Denne banen er i forhold til hvor konfigurasjonsfilen er plassert. Så dette fungerer for både den globale cipher-konfigurasjonen og den prosjektspesifikke konfigurasjonen.
...
Modellen ID i cipher-konfigurasjonen din bruker formatet `provider/model-id`. For eksempel, hvis du bruker [Cipher Zen](/docs/zen), vil du bruke `cipher/gpt-5.1-codex` for GPT 5.1 Codex.
...
:::tip
Kjør `Cipher models` for å se en liste over tilgjengelige modeller.
:::
