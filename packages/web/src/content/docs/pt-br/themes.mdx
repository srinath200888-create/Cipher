...
Com o Cipher, você pode selecionar um dos vários temas embutidos, usar um tema que se adapta ao tema do seu terminal ou definir seu próprio tema personalizado.

Por padrão, o Cipher usa nosso próprio tema `Cipher`.
...
O Cipher vem com vários temas embutidos.
...
- Querem que o Cipher corresponda à aparência do seu terminal
...
O Cipher suporta um sistema de temas flexível baseado em JSON que permite aos usuários criar e personalizar temas facilmente.
...
3. **Diretório raiz do projeto** - Definido em `<project-root>/.cipher/themes/*.json`
4. **Diretório de trabalho atual** - Definido em `./.cipher/themes/*.json`
