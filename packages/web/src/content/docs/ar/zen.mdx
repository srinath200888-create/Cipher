---
title: Zen
description: قائمة منتقاة من النماذج يوفّرها Cipher.
---
...
Cipher Zen هي قائمة بالنماذج التي اختبرها فريق Cipher وتحقّق منها.

يعمل Zen مثل أي مزوّد آخر في Cipher. تسجّل الدخول إلى Cipher Zen وتحصل على
مفتاح API. استخدامه **اختياري بالكامل** ولا تحتاج إليه لاستخدام Cipher.
...
:::tip
اختبرنا مجموعة منتقاة من النماذج والمزوّدين الذين يعملون جيدا مع Cipher.
:::
...
Cipher Zen هي بوابة AI تتيح لك الوصول إلى هذه النماذج.

---

## كيف يعمل

يعمل Cipher Zen مثل أي مزوّد آخر في Cipher.

1. تسجّل الدخول إلى **<a href={console}>Cipher Zen</a>**، وتضيف تفاصيل الفوترة، ثم تنسخ مفتاح API.
2. تشغّل الأمر `/connect` في واجهة TUI، وتختار Cipher Zen، ثم تلصق مفتاح API.
...
يستخدم [معرّف النموذج](/docs/config/#models) في إعدادات Cipher الصيغة `opencode/<model-id>`. على سبيل المثال، بالنسبة إلى GPT 5.5، ستستخدم `cipher/gpt-5.5` في إعداداتك.
...
- DeepSeek V4 Flash Free متاح على Cipher لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- MiMo-V2.5 Free متاح على Cipher لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- North Mini Code Free متاح على Cipher لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Nemotron 3 Ultra Free متاح على Cipher لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
- Big Pickle نموذج خفي ومتاح مجانا على Cipher لفترة محدودة. يستخدم الفريق هذه الفترة لجمع الملاحظات وتحسين النموذج.
...
4. **عدم فرض أي ارتباط حصري** عبر تمكينك من استخدامه مع أي عامل برمجة آخر. ومع إبقاء إمكانية استخدام أي مزوّد آخر مع Cipher أيضا دائما.
