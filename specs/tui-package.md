...
Move the canonical Cipher terminal application from
`packages/cipher/src/cli/cmd/tui` into a self-contained workspace package while
...
generic presentation libraries. It must not depend on `packages/cipher`,
`packages/cli`, or `@cipher-ai/core`.

The SDK is the TUI's Cipher boundary. Missing backend data or operations must
...
- Do not preserve private imports by creating aliases from `packages/tui` back
  into `packages/cipher`.
- Do not replace private `packages/cipher` imports with `@cipher-ai/core`
...
- Keep legacy CLI command parsing, server startup, worker management,
  authentication, and config discovery outside `@cipher-ai/tui`.
...
- Cipher domain data displayed by the TUI
...
```text
packages/cipher/src/cli/cmd/tui
```

Its private dependency on `packages/cipher` is primarily expressed through
the `@/*` TypeScript alias, which resolves to `packages/cipher/src/*`.
...
Status: Completed. The private `@cipher-ai/tui` workspace package now has an
...
- Add `packages/tui/package.json` with the name `@cipher-ai/tui`.
...
- The package has no dependency on `Cipher`, `@cipher-ai/cli`, or
  `@cipher-ai/core`.
...
primitives now live in `@cipher-ai/tui`. The legacy host consumes explicit
...
and build inputs through `@cipher-ai/tui/runtime`. Movable app, component,
route, and feature-plugin code no longer reads Cipher globals or process state;
...
Replace process-global Cipher state with resolved TUI inputs.
...
`@cipher-ai/tui/config` now owns schemas, defaults, keybind resolution, the
...
- Keep config path discovery, project/global precedence, migration, variable
  expansion, and plugin package installation in `packages/cipher` initially.
...
canonical in `@cipher-ai/tui`. Configured references resolve through the new
...
adapters rather than Cipher domain implementations.

Make the SDK the only Cipher domain boundary used by the TUI.
...
- Domain-facing TUI code imports Cipher data and operations only from
  `@cipher-ai/sdk`.
...
`@cipher-ai/tui`. The legacy host injects a narrow plugin host that retains
...
- Move plugin rendering tests to `packages/tui`; retain installation/loading
  integration tests in `packages/cipher`.
...
- The package has no imports from `packages/cipher`, `packages/cli`, or
  `@cipher-ai/core`.
...
public `@cipher-ai/tui` root while retaining worker/server/config/plugin and
...
- Keep the legacy yargs commands corresponding to current `thread.ts` and
  `attach.ts` in `packages/cipher`.
- Keep the legacy embedded worker and server startup in `packages/cipher`.
- Change those adapters to load config, create transport inputs, and call the
  public `@cipher-ai/tui` API.
...
- Remove obsolete `@tui/*` path mappings from `packages/cipher`.
- Remove stale test fixtures and update all imports to package exports.
- Narrow `@cipher-ai/tui` exports to intentional public entrypoints.
...
- No source under `packages/tui` imports `@/...`, `@cipher-ai/core`, or either
  executable package.
...
- TUI package imports do not reach into executable or backend implementation
  packages.
- SDK wire data is treated as the source of truth for Cipher domain state.
...
cd packages/cipher && bun typecheck
...
rg '@cipher-ai/core|packages/cipher|packages/cli' packages/tui
rg 'src/cli/cmd/tui|@tui/' packages/cipher/src packages/cipher/test
```
...
- Build the current-platform `packages/cipher` binary.
