...
Avec Cipher, vous pouvez choisir parmi plusieurs thèmes intégrés, utiliser un thème qui s'adapte au thème de votre terminal ou définir votre propre thème personnalisé.

Par défaut, Cipher utilise notre propre thème `Cipher`.
...
Cipher est livré avec plusieurs thèmes intégrés.
...
- Vous souhaitez que Cipher corresponde à l'apparence de leur terminal
...
Cipher prend en charge un système de thèmes flexible basé sur JSON qui permet aux utilisateurs de créer et de personnaliser facilement des thèmes.
...
3. **Répertoire racine du projet** - Défini dans `<project-root>/.cipher/themes/*.json`
4. **Répertoire de travail actuel** - Défini dans `./.cipher/themes/*.json`
...
```bash no-frame
mkdir -p .cipher/themes
vim .cipher/themes/my-theme.json
```
