...
Umiejętności agenta pozwalają Cipher odkryć instrukcje wielokrotnego użytku z repozytorium lub katalogu domowego.
...
Cipher przeszukuje te lokalizacje:

- Project config: `.cipher/skills/<name>/SKILL.md`
...
W przypadku ścieżek lokalnych projektu Cipher przechodzi od bieżącego katalogu roboczego, aż dotrze do drzewa roboczego git.
Ładuje po drodze dowolne pasujące `skills/*/SKILL.md` w `.cipher/` i dowolne pasujące `.claude/skills/*/SKILL.md` lub `.agents/skills/*/SKILL.md`.
...
Utwórz `.cipher/skills/git-release/SKILL.md` w ten sposób:
...
Cipher wymienia dostępne umiejętności w opisie narzędzia `skill`.
...
Kontroluj, do których umiejętności agenci mogą uzyskać dostęp, używając uprawnień opartych na wzorcach w `cipher.json`:
...
**Dla agentów wbudowanych** (w `cipher.json`):
