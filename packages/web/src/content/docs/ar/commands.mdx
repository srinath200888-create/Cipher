...
أنشئ الملف `.cipher/commands/test.md`:
...
يمكنك إضافة أوامر مخصصة عبر إعدادات Cipher أو بإنشاء ملفات Markdown داخل الدليل `commands/`.

---

### JSON

استخدم خيار `command` في [config](/docs/config) الخاص بـ Cipher:
...
يتضمن Cipher عدة أوامر مضمنة مثل `/init` و`/undo` و`/redo` و`/share` و`/help`؛ [اعرف المزيد](/docs/tui#commands).
