---
title: Pravila
description: Postavite prilagodena uputstva za Cipher.
---

Mozete dodati prilagodena uputstva za Cipher tako sto kreirate `AGENTS.md` datoteku. Ovo je slicno pravilima u Cursor-u. Sadrzi uputstva koja se ubacuju u LLM kontekst da prilagode ponasanje za vas projekat.

---

## Inicijalizacija

Da kreirate novu `AGENTS.md` datoteku, pokrenite `/init` komandu u Cipher.

:::tip
Preporuceno je da `AGENTS.md` iz projekta commitujete u Git.
:::

Ovo skenira projekat i njegov sadrzaj, razumije cemu projekat sluzi i generise `AGENTS.md`. Tako Cipher bolje navigira kroz kod.
...
Cipher podrzava citanje `AGENTS.md` datoteke sa vise lokacija. Svaka lokacija ima drugu svrhu.
...
Mozete imati i globalna pravila u `~/.config/opencode/AGENTS.md`. Ona se primjenjuju u svim Cipher sesijama.
...
Za korisnike koji prelaze sa Claude Code, Cipher podrzava i Claude konvencije datoteka kao rezervu:
...
Kada se Cipher pokrene, trazi datoteke pravila ovim redoslijedom:
...
Mozete navesti prilagodene datoteke uputstava u `cipher.json` ili globalnom `~/.config/opencode/cipher.json`. Tako vi i tim ponovo koristite postojeca pravila bez dupliranja u AGENTS.md.
...
Iako Cipher ne parsira automatski reference datoteka u `AGENTS.md`, slicno ponasanje mozete dobiti na dva nacina:

### Korištenje cipher.json

Preporuceni pristup je da koristite `instructions` polje u `cipher.json`:
...
Mozete nauciti Cipher da cita eksterne datoteke tako sto cete dati eksplicitna uputstva u `AGENTS.md`. Evo prakticnog primjera:
...
- Osigurate da Cipher ucitava datoteke samo kad su potrebne za konkretan zadatak

:::tip
Za monorepo projekte ili projekte sa zajednickim standardima, odrzivije je koristiti `cipher.json` sa glob obrascima (npr. `packages/*/AGENTS.md`) nego rucna uputstva.
:::
