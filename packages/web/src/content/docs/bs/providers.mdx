---
title: Provajderi
description: Korištenje bilo kojeg LLM provajdera u Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher koristi [AI SDK](https://ai-sdk.dev/) i [Models.dev](https://models.dev) za podršku **75+ LLM provajdera** i podržava pokretanje lokalnih modela.

Za dodavanje provajdera potrebno je:

1. Dodajte API ključeve za provajdera koristeći naredbu `/connect`.
2. Konfigurirajte dobavljača u vašoj Cipher konfiguraciji.
...
Možete prilagoditi dobavljače putem odjeljka `provider` u vašem Cipher
config.
...
Cipher Zen je lista modela koje je obezbedio Cipher tim koji su bili
testirano i potvrđeno da dobro radi sa Cipher. [Saznajte više](/docs/zen).

:::tip
Ako ste novi, preporučujemo da počnete sa Cipher Zen.
:::

1. Pokrenite naredbu `/connect` u TUI-u, odaberite Cipher i idite na [Cipher.ai/auth](https://opencode.ai/auth).
...
Radi kao i svaki drugi provajder u Cipher i potpuno je opcionalan za korištenje.

---

## Cipher Go

Cipher Go je jeftin plan pretplate koji pruža pouzdan pristup popularnim modelima otvorenog kodiranja koje pruža Cipher tim i koji su testirani i verificirani da dobro rade s cipher-om.

1. Pokrenite naredbu `/connect` u TUI-u, odaberite `Cipher Go` i idite na [Cipher.ai/auth](https://opencode.ai/zen).
...
Da biste koristili Amazon Bedrock s Cipher:
...
2. **Konfigurirajte autentifikaciju** koristeći jedan od sljedećih metoda:
   ...
   Postavite jednu od ovih varijabli okruženja dok pokrećete Cipher:
...
Za konfiguraciju specifičnu za projekat ili trajnu konfiguraciju, koristite `cipher.json`:
...
:::info
[Anthropic] (https://anthropic.com) službeno ne podržava korištenje vaše Claude Pro/Max pretplate u Cipher.
:::
...
Možete konfigurirati Cipher za korištenje lokalnih modela preko [Atomic Chata](https://atomic.chat) — desktop aplikacije koja pokreće lokalne LLM-ove iza OpenAI-kompatibilnog API servera (zadana krajnja tačka `http://127.0.0.1:1337/v1`).
...
2. Idite na [Azure AI Foundry](https://ai.azure.com/) i implementirajte model.
   ...
   :::note
   Ime implementacije mora odgovarati imenu modela da bi Cipher ispravno radio.
   :::
...
Također možete dodati modele kroz svoju Cipher konfiguraciju.
...
da pohrani token u Cipher auth memoriju.
:::

##### Samostalni GitLab

:::note[Napomena o usklađenosti]
Cipher koristi mali model za neke AI zadatke kao što je generiranje naslova sesije.
Podrazumevano je konfigurisan da koristi gpt-5-nano, a hostuje ga Zen. Da zaključate Cipher
da biste koristili samo svoju vlastitu instancu koju hostuje GitLab, dodajte sljedeće u svoju
`cipher.json` fajl. Također se preporučuje da onemogućite dijeljenje sesije.
...
Više dokumentacije na početnoj stranici [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth).

##### Konfiguracija

Prilagodite putem `cipher.json`:
...
Da biste koristili svoju GitHub Copilot pretplatu s Cipher:
...
Za korištenje Google Vertex AI s Cipher:
...
2. Postavite potrebne varijable okruženja:
   ...
   Postavite ih dok se pokreće Cipher.
...
U slučaju da vidite funkciju ili model iz Helicone-a koji nije automatski konfiguriran putem ciphera, uvijek ga možete sami konfigurirati.
...
Heliconeova funkcija [Sessions](https://docs.helicone.ai/features/sessions) vam omogućava da grupišete povezane LLM zahtjeve zajedno. Koristite dodatak [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) da automatski prijavite svaki Cipher razgovor kao sesiju u Helicone-u.
...
Dodatak ubacuje zaglavlja `Helicone-Session-Id` i `Helicone-Session-Name` u vaše zahtjeve. Na stranici Helicone Sessions, vidjet ćete svaki Cipher razgovor naveden kao zasebna sesija.
...
Možete konfigurirati Cipher za korištenje lokalnih modela putem [llama.cpp's](https://github.com/ggml-org/llama.cpp) uslužnog programa llama-server
...
Možete konfigurirati Cipher za korištenje lokalnih modela preko LM Studio.
...
Možete konfigurirati Cipher za korištenje lokalnih modela putem Ollame.

:::tip
Ollama se može automatski konfigurirati za Cipher. Pogledajte [Ollama integracijske dokumente](https://docs.ollama.com/integrations/opencode) za detalje.
:::
...
Da biste koristili Ollama Cloud s Cipher:
...
3. Kopirajte API ključ za korištenje u Cipher.
...
6. **Važno**: Prije upotrebe modela oblaka u Cipher, morate lokalno povući informacije o modelu:
...
Cipher Zen je lista testiranih i verifikovanih modela koju je obezbedio Cipher tim. [Saznajte više](/docs/zen).

1. Prijavite se na **<a href={console}>Cipher Zen</a>** i kliknite na **Kreiraj API ključ**.

2. Pokrenite naredbu `/connect` i potražite **Cipher Zen**.

```txt
   /connect
```

3. Unesite svoj Cipher API ključ.
...
Također možete dodati dodatne modele putem vaše Cipher konfiguracije.
...
5. Također ih možete prilagoditi putem vaše Cipher konfiguracije. Evo primjera navođenja provajdera
...
Da biste koristili [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) sa cipherom:
...
Također možete prilagoditi modele kroz svoju Cipher konfiguraciju. Evo primjera specificiranja redoslijeda usmjeravanja dobavljača.
...
:::tip
Možete koristiti bilo kojeg OpenAI kompatibilnog provajdera s cipher-om. Većina modernih AI provajdera nudi API-je kompatibilne sa OpenAI.
:::
...
4. Kreirajte ili ažurirajte svoju `cipher.json` datoteku u direktoriju projekta:
...
Polja `limit` omogućavaju Cipher da shvati koliko vam je konteksta ostalo. Standardni dobavljači ih automatski preuzimaju sa models.dev.
...
1. **Provjerite postavke autentifikacije**: Pokrenite `Cipher auth list` da vidite da li su vjerodajnice
...
2. Za prilagođene provajdere, provjerite Cipher konfiguraciju i:
   - Uvjerite se da ID provajdera korišten u naredbi `/connect` odgovara ID-u u vašoj Cipher konfiguraciji.
