...
Kit and Aidan want provider-specific LLM behavior to move out of cipher's AI
...
cipher's dynamic runtime should construct executable models at its app
...
- Effect AI: executable models carry provider requirements and can be resolved by
  an app boundary.
- LiteLLM/cipher config: dynamic `providerID/modelID` branching belongs at the
...
- [ ] Decide whether a tiny `Provider.define(...)` helper is warranted after two
      ...
      yet painful.
- [x] Update `packages/cipher/src/session/llm/native-request.ts` to construct
