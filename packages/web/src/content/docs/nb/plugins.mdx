...
description: Skriv dine egne programtillegg for å utvide Cipher.
---

Plugins lar deg utvide Cipher ved å koble til ulike hendelser og tilpasse atferd. Du kan lage plugins for å legge til nye funksjoner, integrere med eksterne tjenester eller endre standardoppførselen til Cipher.
...
- `.cipher/plugins/` - Programtillegg på prosjektnivå
...
1. Global konfigurasjon (`~/.config/opencode/cipher.json`)
2. Prosjektkonfigurasjon (`cipher.json`)
3. Global plugin-katalog (`~/.config/opencode/plugins/`)
4. Prosjektpluginkatalog (`.cipher/plugins/`)
...
Cipher kjører `bun install` ved oppstart for å installere disse. Programtilleggene og verktøyene dine kan deretter importere dem.
...
- `client`: En Cipher SDK klient for samhandling med AI.
...
Her er noen eksempler på plugins du kan bruke for å utvide Cipher.
...
:::note
Hvis du bruker cipher-skrivebordsappen, kan den sende systemvarsler automatisk når et svar er klart eller når en økt feiler.
:::
...
Hindre Cipher fra å lese `.env` filer:
...
Plugins kan også legge til egendefinerte verktøy til Cipher:
...
`tool`-hjelperen lager et tilpasset verktøy som Cipher kan kalle. Den tar en Zod-skjemafunksjon og returnerer en verktøydefinisjon med:

- `description`: Hva verktøyet gjør
- `args`: Zod-skjema for verktøyets argumenter
  - `execute`: Funksjon som kjører når verktøyet kalles

Dine egendefinerte verktøy vil være tilgjengelige for Cipher sammen med innebygde verktøy.
