---
title: Regeln
description: Lege eigene Anweisungen fuer Cipher fest.
---

Du kannst Cipher ueber eine `AGENTS.md`-Datei mit eigenen Anweisungen steuern.
...
Um eine neue `AGENTS.md` zu erstellen, fuehre in Cipher den Befehl `/init` aus.
...
So kann Cipher sich in deinem Code besser orientieren.

Wenn bereits eine `AGENTS.md` existiert, versucht Cipher sie zu erweitern.
...
Cipher kann `AGENTS.md` aus mehreren Orten lesen.
...
Diese gelten in allen cipher-Sitzungen.
...
Wenn du von Claude Code kommst, unterstuetzt Cipher dessen Dateikonventionen als Fallback:
...
Beim Start sucht Cipher in dieser Reihenfolge nach Regeldateien:
...
Du kannst zusaetzliche Anweisungsdateien in deiner `cipher.json` oder in `~/.config/opencode/cipher.json` angeben.
...
Cipher parst Datei-Referenzen in `AGENTS.md` nicht automatisch.
Du erreichst aehnliches Verhalten auf zwei Wegen.

### Mit cipher.json

Empfohlen ist das `instructions`-Feld in der `cipher.json`:
...
Du kannst Cipher explizit anweisen, externe Dateien zu lesen.
...
:::tip
Fuer Monorepos oder gemeinsame Standards ist `cipher.json` mit Glob-Mustern (z. B. `packages/*/AGENTS.md`) oft wartbarer als manuelle Anweisungen.
:::
