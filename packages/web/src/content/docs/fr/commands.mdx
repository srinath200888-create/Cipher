...
Vous pouvez ajouter des commandes personnalisées via la configuration Cipher ou en créant des fichiers markdown dans le répertoire `commands/`.

---

### JSON

Utilisez l'option `command` dans votre Cipher [config](/docs/config) :
...
Cipher comprend plusieurs commandes intégrées comme `/init`, `/undo`, `/redo`, `/share`, `/help` ; [en savoir plus](/docs/tui#commands).
