---
title: Cipher Zen
description: Cipher によって提供されるモデルの厳選されたリスト。
---
...
Cipher Zen は、Cipher チームが提供する、テスト済みかつ検証済みのモデルの一覧です。

Zen は Cipher のほかのプロバイダーと同じように動作します。Cipher Zen にログインして API キーを取得できます。これは **完全にオプション** であり、Cipher を使うために必須ではありません。
...
:::tip
Cipher でうまく動作する、厳選したモデルとプロバイダーの組み合わせをテストしました。
:::
...
Cipher Zen は、これらのモデルにアクセスできる AI ゲートウェイです。

---

## 仕組み

Cipher Zen は、Cipher のほかのプロバイダーと同じように動作します。

1. **<a href={console}>Cipher Zen</a>** にサインインし、請求情報を追加して、API キーをコピーします。
2. TUI で `/connect` コマンドを実行し、Cipher Zen を選んで API キーを貼り付けます。
...
Cipher 設定で使う [model id](/docs/config/#models) は `opencode/<model-id>` 形式です。たとえば、GPT 5.5 では設定に `cipher/gpt-5.5` を使用します。
...
- DeepSeek V4 Flash Free は期間限定で Cipher で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- MiMo-V2.5 Free は期間限定で Cipher で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- North Mini Code Free は期間限定で Cipher で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Nemotron 3 Ultra Free は期間限定で Cipher で利用できます。チームはこの期間中にフィードバックを集め、モデルを改善しています。
- Big Pickle はステルスモデルで、期間限定で Cipher で無料提供されています。チームはこの期間中にフィードバックを集め、モデルを改善しています。
...
私たちが Cipher Zen を作った目的は次のとおりです。
...
4. ほかのコーディングエージェントでも使えるようにして **no lock-in** を実現し、さらに Cipher では常にほかのプロバイダーも使えるようにすること。
