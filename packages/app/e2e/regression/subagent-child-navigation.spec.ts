...
test("navigates to a subagent child session missing from the session list", async ({ page }) => {
  ...
  const titlebarRight = page.locator("#cipher-titlebar-right")
  ...
  return [
    {
      info: {
        ...
        agent: "build",
        model: { providerID: "cipher", modelID: "claude-opus-4-6" },
      },
      parts: [
        {
  ...
  await page.addInitScript(
    ({ directory, server, sessionId }) => {
      ...
      localStorage.setItem(
        "cipher.global.dat:server",
        JSON.stringify({
          projects: { local: [{ worktree: directory, expanded: true }] },
          lastProject: { local: directory },
        }),
      )
      localStorage.setItem("cipher.window.browser.dat:tabs", JSON.stringify([{ type: "session", server, sessionId }]))
    },
