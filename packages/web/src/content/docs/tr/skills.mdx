...
Ajan becerileri, cipher'un deponuzdan veya ev dizininizden tekrar kullanilabilir talimatlari kesfetmesini saglar.
...
Cipher su konumlari tarar:

- Proje konfigurasyonu: `.cipher/skills/<name>/SKILL.md`
...
Proje yerel yollarinda Cipher, mevcut calisma dizininizden baslayip git worktree kokune kadar yukari cikar.
Bu sirada `skills/*/SKILL.md` altindaki eslesen `.cipher/` dosyalarini ve `.claude/skills/*/SKILL.md` ya da `.agents/skills/*/SKILL.md` dosyalarini yukler.
...
`.cipher/skills/git-release/SKILL.md` dosyasini soyle olusturun:
...
Cipher, `skill` araci aciklamasinda mevcut becerileri listeler.
...
`cipher.json` icinde pattern tabanli izinlerle ajanlarin hangi becerilere erisecegini kontrol edin:
...
**Yerlesik ajanlar icin** (`cipher.json` icinde):
