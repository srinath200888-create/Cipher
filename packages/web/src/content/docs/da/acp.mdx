...
description: Brug Cipher i enhver ACP-kompatibel editor.
---

Cipher understøtter [Agent Client Protocol](https://agentclientprotocol.com) eller (ACP), så du kan bruge det direkte i kompatible editorer og IDE'er.
...
For at bruge Cipher via ACP, konfigurer din editor til at køre kommandoen `Cipher acp`.

Kommandoen starter Cipher som en ACP-kompatibel underproces, der kommunikerer med din editor over JSON-RPC via stdio.
...
For at åbne den skal du bruge den nye 'cipher'-agent i AI Chat-agentvælgeren.
...
For at bruge Cipher som en ACP-agent i [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), skal du tilføje følgende til din Neovim-konfiguration:
...
Denne konfiguration sætter CodeCompanion til at bruge Cipher som ACP-agent til chat.

Hvis du har brug for at sende miljøvariabler (som `CIPHER_API_KEY`), henvises til [Konfiguration af adaptere: Miljøvariabler](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) i CodeCompanion.nvim-dokumentationen for alle detaljer.

## Understøttelse

Cipher fungerer på samme måde via ACP som i terminalen. Alle funktioner understøtter:
...
- MCP-servere konfigureret i din cipher-konfiguration
