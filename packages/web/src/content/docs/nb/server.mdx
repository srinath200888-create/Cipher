---
title: Tjener
description: Samhandle med cipher-tjener over HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Kommandoen `Cipher serve` kjører en hodeløs HTTP-server som eksponerer et OpenAPI-endepunkt som en cipher-klient kan bruke.
...
| `--mdns-domain` | Egendefinert domenenavn for mDNS-tjeneste     | `Cipher.local` |
...
Sett `CIPHER_SERVER_PASSWORD` for å beskytte serveren med HTTP grunnleggende autentisering. Brukernavnet er satt til `Cipher` som standard, eller sett `CIPHER_SERVER_USERNAME` for å overstyre det. Dette gjelder både `Cipher serve` og `Cipher web`.
...
Når du kjører `Cipher` starter den en TUI og en server. Der TUI er
...
:::tip
Bruk cipher-serveren til å samhandle med Cipher programmatisk.
:::

Denne arkitekturen lar Cipher støtte flere klienter og lar deg samhandle med Cipher programmatisk.

Du kan kjøre `Cipher serve` for å starte en frittstående server. Hvis du har
Cipher TUI kjører, vil `Cipher serve` starte en ny server.
...
[`/tui`](#tui) endepunktet kan brukes til å kjøre TUI gjennom serveren. Du kan for eksempel forhåndsutfylle eller kjøre en forespørsel. Dette oppsettet brukes av Cipher [IDE](/docs/ide) plugins.
...
cipher-serveren viser følgende APIer.
