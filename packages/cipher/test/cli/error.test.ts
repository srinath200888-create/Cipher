...
describe("cli.error", () => {
  test("formats legacy and tagged config errors the same way", () => {
    const cases = [
      {
        tag: "ConfigJsonError",
        data: { path: "/tmp/cipher.jsonc", message: "Unexpected token" },
        expected: "Config file at /tmp/cipher.jsonc is not valid JSON(C): Unexpected token",
      },
      {
        tag: "ConfigDirectoryTypoError",
        data: { path: "/tmp/cipher.jsonc", dir: ".cipher", suggestion: "cipher" },
        expected:
          'Directory ".cipher" in /tmp/cipher.jsonc is not valid. Rename the directory to "cipher" or remove it. This is a common typo.',
      },
      {
      ...
      },
      {
        ...
        data: {
          path: "/tmp/cipher.jsonc",
          message: "schema mismatch",
          issues: [{ message: "Expected string", path: ["provider", "id"] }],
        },
        expected: "Configuration is invalid at /tmp/cipher.jsonc: schema mismatch\n↳ Expected string provider.id",
      },
    ]
  ...
  test("formats legacy and tagged provider model errors the same way", () => {
    ...
    const expected = [
      ...
      "Or check your config (cipher.json) provider/model names",
    ].join("\n")
