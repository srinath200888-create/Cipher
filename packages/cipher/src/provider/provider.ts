...
import { Hash } from "@cipher-ai/core/util/hash"
...
import { Env } from "../env"
import { InstallationVersion } from "@cipher-ai/core/installation/version"
...
import { optional } from "@cipher-ai/core/schema"
...
const BUNDLED_PROVIDERS: Record<string, () => Promise<(opts: any) => BundledSDK>> = {
  ...
  "@ai-sdk/github-copilot": () =>
    import("@cipher-ai/core/github-copilot/copilot-provider").then((m) => m.createOpenaiCompatible),
...
function custom(dep: CustomDep): Record<string, CustomLoader> {
  return {
    ...
    cipher: Effect.fnUntraced(function* (input: Info) {
      ...
      const ok =
        ...
        Boolean((yield* dep.config()).provider?.["cipher"]?.options?.apiKey)
    ...
    "amazon-bedrock": Effect.fnUntraced(function* () {
      ...
      return {
        ...
        async getModel(sdk: any, modelID: string, options?: Record<string, any>, model?: Model) {
          ...
          // 1. options.region from cipher.json provider config
      ...
      Effect.succeed({
        ...
        options: {
          headers: {
            "HTTP-Referer": "https://opencode.ai/",
            "X-Title": "cipher",
            "X-BILLING-INVOKE-ORIGIN": "cipher",
          },
        },
      }),
    vercel: () =>
      ...
      if (!apiToken) {
        throw new Error(
          "CLOUDFLARE_API_TOKEN (or CF_AIG_TOKEN) is required for Cloudflare AI Gateway. " +
            "Set it via environment variable or run `cipher auth cloudflare-ai-gateway`.",
        )
      }
    ...
    cerebras: () =>
      Effect.succeed({
        ...
        options: {
          headers: {
            "X-Cerebras-3rd-Party-Integration": "cipher",
          },
        },
      }),
    kilo: () =>
    ...
    "snowflake-cortex": Effect.fnUntraced(function* (input: Info) {
      ...
      if (!account || !token) {
        ...
        return {
          ...
          async getModel() {
            throw new Error(
              `Snowflake Cortex: missing credentials (${missing}). Provide a bearer token (OAuth, JWT, or PAT) via env var, cipher auth, or provider options.`,
            )
          },
        }
      }
...
export class Service extends Context.Service<Service, Interface>()("@cipher/Provider") {}
...
const layer = Layer.effect(
  ...
  Effect.gen(function* () {
    ...
    const getSmallModel = Effect.fn("Provider.getSmallModel")(function* (providerID: ProviderV2.ID) {
      ...
      const priority = providerID.startsWith("cipher")
        ? ["gpt-nano"]
