...
//   - CIPHER_CONFIG_CONTENT      : provider config inline, no files to find
//   - CIPHER_TEST_HOME           : pins os.homedir() → tmpdir
//   - CIPHER_DISABLE_PROJECT_CONFIG : skip walking up for cipher.json
//   - CIPHER_PURE                : skip external plugin discovery + install
//   - CIPHER_DISABLE_AUTOUPDATE / AUTOCOMPACT / MODELS_FETCH : no background work
...
function isolatedEnv(home: string, configJson: string): Record<string, string> {
  return {
    CIPHER_TEST_HOME: home,
    HOME: home,
    XDG_CONFIG_HOME: path.join(home, ".config"),
    XDG_DATA_HOME: path.join(home, ".local/share"),
    XDG_STATE_HOME: path.join(home, ".local/state"),
    XDG_CACHE_HOME: path.join(home, ".cache"),
    CIPHER_CONFIG_CONTENT: configJson,
    CIPHER_DISABLE_PROJECT_CONFIG: "1",
    CIPHER_PURE: "1",
    CIPHER_DISABLE_AUTOUPDATE: "1",
    CIPHER_DISABLE_AUTOCOMPACT: "1",
    CIPHER_DISABLE_MODELS_FETCH: "1",
    CIPHER_AUTH_CONTENT: "{}",
  }
}
  ...
  return Effect.gen(function* () {
    ...
    const runOpts = (opts?: RunOpts): SpawnOpts | undefined => {
      ...
      return {
        ...
        env: {
          ...opts.env,
          CIPHER_CONFIG_CONTENT: JSON.stringify({
...
function expectExit(result: RunResult, expected: number, label = "cipher") {
