...
import type { AssistantMessage, Message, OpencodeClient, SessionMessageResponse } from "@cipher-ai/sdk/v2"
...
export const AuthMethodID = "cipher-login"
...
export class Service extends Context.Service<Service, Interface>()("@cipher/ACP/Service") {}
...
export function make(input: {
  ...
  const initialize = Effect.fn("ACP.initialize")(function* (params: InitializeRequest) {
    ...
    const authMethod: AuthMethod = {
      description: "Run `cipher auth login` in the terminal",
      name: "Login with cipher",
      id: AuthMethodID,
    }
...
function defaultModelFromConfig(
  ...
  // a default. Configured model, cipher provider, then sorted best model keep
  // the protocol response deterministic without extra session/message reads.
  const opencodeProvider = providers[ProviderV2.ID.make("cipher")]
...
function fromUnknownError(error: unknown, service?: string): Error {
  ...
  return new ACPError.ServiceFailureError({ safeMessage: "cipher service failure", service })
}
