...
- Canonical tagged tool file sources in `@cipher-ai/llm`.
...
- Spill oversized model-facing tool text into globally unique files under cipher's shared tool-output directory.
...
- Canonicalize ambient instruction traversal boundaries, honor `CIPHER_DISABLE_PROJECT_CONFIG`, and make non-empty aggregate updates explicitly supersede previously loaded instructions.
