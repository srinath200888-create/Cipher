...
async function fix(pr: PR, files: string[], prs: PR[], applied: number[], idx: number) {
  console.log(`  Trying to auto-resolve ${files.length} conflict(s) with cipher...`)
  ...
  try {
    await $`cipher run -m ${model} ${prompt}`
  } catch (err) {
    console.log(`  cipher failed: ${err}`)
    return false
  }
  ...
  console.log("  Conflicts resolved with cipher")
  return true
}
...
async function smoke(prs: PR[], applied: number[]) {
  ...
  console.log("\nTrying to fix final smoke check with cipher...")
