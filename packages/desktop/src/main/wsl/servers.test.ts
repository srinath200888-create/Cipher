...
test("rejects an update that did not install the desktop version", () => {
  ...
  expect(() => expectOpencodeVersion("1.14.35", "1.16.2")).toThrow(
    "cipher update finished but Debian still reports 1.14.35; expected 1.16.2",
  )
})

test("restarts an existing distro server after updating cipher", () => {
  expect(
    wslServerIdToRestart(
      [
        {
  ...
  expect(
    clearWslDistroState(
      { Debian: { name: "Debian", canExecute: true, hasBash: true, hasCurl: true, error: null } },
      {
        Debian: {
          distro: "Debian",
          resolvedPath: "/home/luke/.cipher/bin/opencode",
          version: "1.16.2",
...
test("ignores stale background cipher checks after removing a WSL server", async () => {
  persistedServers = []
  ...
  expect(controller.getState().cipherChecks).toEqual({})
})

test("ignores stale startup cipher checks after removing a WSL server", async () => {
...
test("probes addable distros in parallel before checking cipher", async () => {
  persistedServers = []
  ...
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...
    resolveOpencode: async (distro) => {
      cipher.push(distro)
      return "/home/me/.cipher/bin/opencode"
    },
  })
  ...
  expect(cipher).toEqual(["Debian", "Ubuntu"])
  expect(Object.keys(controller.getState().cipherChecks)).toEqual(["Debian", "Ubuntu"])
})

test("does not check cipher in addable distros that cannot execute commands", async () => {
  persistedServers = []
  const cipher: string[] = []
  const controller = createWslServersController("1.16.2", async () => new Promise<never>(() => undefined), {
    ...testControllerOptions(),
  ...
  expect(Object.keys(controller.getState().distroProbes)).toEqual(["Debian", "Ubuntu"])
  expect(cipher).toEqual(["Debian"])
  expect(Object.keys(controller.getState().cipherChecks)).toEqual(["Debian"])
})
