...
description: Cree herramientas que LLM pueda llamar en Cipher.
---

Las herramientas personalizadas son funciones que usted crea y que el LLM puede llamar durante las conversaciones. Trabajan junto con las [herramientas integradas](/docs/tools) de Cipher como `read`, `write` y `bash`.
...
- Localmente colocándolos en el directorio `.cipher/tools/` de tu proyecto.
...
```ts title=".cipher/tools/database.ts" {1}
...
```ts title=".cipher/tools/project.ts" {8}
import { tool } from "@cipher-ai/plugin"
...
```python title=".cipher/tools/add.py"
import sys
...
```ts title=".cipher/tools/python-add.ts" {10}
import { tool } from "@cipher-ai/plugin"
...
export default tool({
  ...
  async execute(args, context) {
    const script = path.join(context.worktree, ".cipher/tools/add.py")
