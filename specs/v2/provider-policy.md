...
Both `action` and `resource` use cipher's existing wildcard matching behavior.
...
Plugins must not be allowed to add, remove, or override policy statements. Plugins can contribute functionality or configured providers; policy determines whether Cipher permits an operation through its managed execution paths.
