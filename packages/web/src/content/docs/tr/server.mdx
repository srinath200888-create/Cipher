---
title: Sunucu
description: Cipher sunucusuyla HTTP uzerinden etkilesin.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

`Cipher serve` komutu, Cipher istemcisinin kullanabileceği bir OpenAPI endpoint'i açan headless bir HTTP sunucusu çalıştırır.
...
| `--mdns-domain` | mDNS hizmeti için özel alan adı                | `Cipher.local` |
...
Sunucuyu HTTP temel kimlik doğrulama (basic auth) ile korumak için `CIPHER_SERVER_PASSWORD` ayarlayın. Kullanıcı adı varsayılan olarak `Cipher` değeridir; değiştirmek isterseniz `CIPHER_SERVER_USERNAME` ayarlayabilirsiniz. Bu ayar hem `Cipher serve` hem de `Cipher web` için geçerlidir.
...
`Cipher` çalıştırdığınızda hem TUI hem de sunucu başlar. TUI, sunucuyla konuşan istemci tarafıdır.
Sunucu bir OpenAPI 3.1 spesifikasyon uç noktası açar. Bu uç nokta [SDK](/docs/sdk) üretiminde de kullanılır.

:::tip
Cipher ile programatik etkileşim için Cipher sunucusunu kullanın.
:::

Bu mimari, cipher'un birden fazla istemciyi desteklemesini ve programatik kullanımları mümkün kılmasını sağlar.

`Cipher serve` ile bağımsız bir sunucu başlatabilirsiniz. Cipher TUI açıksa, `Cipher serve` yeni bir sunucu başlatır.
...
[`/tui`](#tui) uç noktası sunucu üzerinden TUI'yi sürmek için kullanılabilir. Örneğin bir istemi önceden doldurabilir veya çalıştırabilirsiniz. Bu kurulum Cipher [IDE](/docs/ide) eklentileri tarafından kullanılır.
...
Cipher sunucusu aşağıdaki API'leri sunar.
