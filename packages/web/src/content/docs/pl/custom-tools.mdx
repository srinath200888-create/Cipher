...
Narzędzia specjalistyczne to funkcje, które tworzysz i które LLM może wywoływać podczas rozmów. Współpracują one z [wbudowanymi narzędziami](/docs/tools) Cipher, takimi jak `read`, `write` i `bash`.
...
- Lokalnie, umieszczając je w katalogu `.cipher/tools/` swojego projektu.
