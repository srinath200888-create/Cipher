...
Las herramientas permiten que LLM realice acciones en su código base. Cipher viene con un conjunto de herramientas integradas, pero puede ampliarlo con [herramientas personalizadas](/docs/custom-tools) o [servidores MCP](/docs/mcp-servers).
...
Aquí están todas las herramientas integradas disponibles en Cipher.
...
:::note
Esta herramienta solo está disponible cuando `CIPHER_EXPERIMENTAL_LSP_TOOL=true` (o `CIPHER_EXPERIMENTAL=true`).
:::
...
:::note
Esta herramienta solo está disponible cuando se utiliza el proveedor Cipher o cuando la variable de entorno `CIPHER_ENABLE_EXA` está configurada en cualquier valor verdadero (por ejemplo, `true` o `1`).

Para habilitar al iniciar Cipher:

```bash
CIPHER_ENABLE_EXA=1 Cipher
```
