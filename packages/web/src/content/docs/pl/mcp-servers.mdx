...
Możesz dodać dodatkowe informacje do Cipher za pomocą _Model Context Protocol_, w skrócie MCP. Cipher obsługuje zarówno serwery lokalne, jak i zdalne.
...
Dostępny serwery MCP w [Cipher Config](https://opencode.ai/docs/config/) w `mcp`. Dodaj każdy MCP z unikalną nazwą. Możesz zgłosić się do tego MCP po nazwie, pytając LLM.
...
Organizacje mogą udostępniać serwery MCP za pośrednictwem punktu końcowego `.well-known/cipher`. Serwer może być podłączony, dzięki czemu użytkownicy mogą wybrać te, których.
...
Cipher automatyczne uwierzytelnianie OAuth dla zdalnych serwerów MCP. Gdy serwer wymaga uwierzytelnienia, Cipher:
...
Jeśli serwer wymaga uwierzytelnienia, Cipher poprosi Cię o uwierzytelnienie przy próbie jego użycia. Jeśli nie, możesz [ręcznie podłączyć przepływ] (#authenticating) za pomocą `Cipher mcp auth <server-name>`.
...
Komenda `mcp auth` została udostępniona w celu autoryzacji. Po autoryzacji Cipher bezpieczne przechowa tokeny w `~/.local/share/opencode/mcp-auth.json`.
...
Twoje MCP są dostępne jako narzędzie w Cipher, obok narzędzi dodatkowych. Można to zrobić poprzez podłączenie Cipher, jak również w innym przypadku.
...
Spowoduje otwarcie okna konfiguracji, w którym można zakończyć proces OAuth i połączyć Cipher z kontem Sentry.
