...
In Cipher esistono due tipi di agenti: agenti primari e subagenti.
...
Cipher include due agenti primari integrati: **Build** e **Plan**. Li vediamo sotto.
...
Cipher include tre subagenti integrati: **General**, **Explore** e **Scout**. Li vediamo sotto.

---

## Integrati

Cipher include due agenti primari integrati e tre subagenti integrati.
...
Un agente in sola lettura per la ricerca su documentazione esterna e dipendenze. Usalo quando devi clonare il repository di una dipendenza nella cache gestita di Cipher, ispezionare il codice sorgente di una libreria o confrontare il codice locale con implementazioni upstream senza modificare il tuo workspace.
...
Configura gli agenti nel file `cipher.json`:
...
Se non specifichi una temperature, Cipher usa i default specifici del modello; tipicamente 0 per la maggior parte dei modelli e 0.55 per i modelli Qwen.
...
Questo path è relativo alla posizione del file di configurazione. Quindi funziona sia per la config globale di Cipher sia per la config specifica del progetto.
...
L'ID modello nella configurazione Cipher usa il formato `provider/model-id`. Per esempio, se stai usando [Cipher Zen](/docs/zen), useresti `cipher/gpt-5.1-codex` per GPT 5.1 Codex.
...
:::tip
Esegui `Cipher models` per vedere la lista dei modelli disponibili.
:::
