---
title: Konfiguracija
description: Korištenje Cipher JSON konfiguracije.
---

Možete konfigurirati Cipher koristeći JSON konfiguracijski fajl.

---

## Format

Cipher podržava i **JSON** i **JSONC** (JSON sa komentarima) formate.
...
1. **Udaljena konfiguracija** (od `.well-known/cipher`) - organizacijske postavke
2. **Globalna konfiguracija** (`~/.config/opencode/cipher.json`) - korisničke preferencije
3. **Prilagođena konfiguracija** (`CIPHER_CONFIG` env var) - prilagođena preinačenja
4. **Konfiguracija projekta** (`cipher.json` u projektu) - postavke specifične za projekat
5. **`.opencode` direktoriji** - agenti, komande, dodaci
6. **Inline konfiguracija** (`CIPHER_CONFIG_CONTENT` env var) - runtime preinačenja
...
:::note
Direktoriji `.opencode` i `~/.config/cipher` koriste **imena u množini** za poddirektorije: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/` i `themes/`. Pojedinačna imena (npr. `agent/`) su također podržana za kompatibilnost unatrag.
:::
...
Organizacije mogu pružiti zadanu konfiguraciju preko `.well-known/cipher` krajnje tačke. Ovo se automatski preuzima kada se autentifikujete kod provajdera koji to podržava.
...
Postavite svoju globalnu Cipher konfiguraciju u `~/.config/opencode/cipher.json`. Koristite globalnu konfiguraciju za korisničke preferencije kao što su provajderi, modeli i dozvole.
...
Dodajte `cipher.json` u korijen projekta. Konfiguracija projekta ima najveći prioritet među standardnim konfiguracijskim datotekama - ona nadjačava globalne i udaljene konfiguracije.
...
Kada se Cipher pokrene, traži konfiguracijsku datoteku u trenutnom direktoriju ili prelazi do najbližeg Git direktorija.
...
Navedite prilagođenu putanju konfiguracijske datoteke koristeći varijablu okruženja `CIPHER_CONFIG`.
...
Navedite prilagođeni konfiguracijski direktorij koristeći `CIPHER_CONFIG_DIR` varijablu okruženja. U ovom direktoriju će se tražiti agenti, komande, modovi i dodaci baš kao standardni `.opencode` direktorij, i trebali bi pratiti istu strukturu.
...
Konfiguracijski fajl ima šemu koja je definirana u [**`Cipher.ai/config.json`**](https://opencode.ai/config.json).

TUI konfiguracija koristi [**`Cipher.ai/tui.json`**](https://opencode.ai/tui.json).
...
Koristite `CIPHER_TUI_CONFIG` da pokažete na prilagođeni TUI konfiguracijski fajl.

Stari `theme`, `keybinds`, i `tui` ključevi u `cipher.json` su zastarjeli i automatski će se migrirati kada je to moguće.
...
Možete konfigurirati postavke servera za naredbe `Cipher serve` i `Cipher web` putem opcije `server`.
...
- `mdns` - Omogući mDNS otkrivanje servisa. Ovo omogućava drugim uređajima na mreži da otkriju vaš Cipher server.
- `mdnsDomain` - Prilagođeno ime domene za mDNS servis. Zadano je `Cipher.local`. Korisno za pokretanje više instanci na istoj mreži.
...
Možete konfigurirati dobavljače i modele koje želite koristiti u svojoj Cipher konfiguraciji kroz opcije `provider`, `model` i `small_model`.
...
Opcija `small_model` konfigurira poseban model za lagane zadatke poput generiranja naslova. Podrazumevano, Cipher pokušava da koristi jeftiniji model ako je dostupan od vašeg provajdera, inače se vraća na vaš glavni model.
...
Također možete definirati agente koristeći markdown datoteke u `~/.config/opencode/agents/` ili `.cipher/agents/`. [Saznajte više ovdje](/docs/agents).
...
Zadani agent mora biti primarni agent (ne podagent). Ovo može biti ugrađeni agent kao što je `"build"` ili `"plan"`, ili [prilagođeni agent](/docs/agents) koji ste definirali. Ako navedeni agent ne postoji ili je podagent, Cipher će se vratiti na `"build"` s upozorenjem.

Ova postavka se primjenjuje na sva sučelja: TUI, CLI (`Cipher run`), desktop aplikaciju i GitHub Action.
...
Također možete definirati naredbe koristeći markdown fajlove u `~/.config/opencode/commands/` ili `.cipher/commands/`. [Saznajte više ovdje](/docs/commands).
...
Cipher će automatski preuzeti sva nova ažuriranja kada se pokrene. Ovo možete onemogućiti opcijom `autoupdate`.
...
Prema zadanim postavkama, Cipher **dopušta sve operacije** bez potrebe za eksplicitnim dopuštenjem. Ovo možete promijeniti koristeći opciju `permission`.
...
[Plugins](/docs/plugins) proširuju Cipher sa prilagođenim alatima, kukicama i integracijama.

Postavite datoteke dodataka u `.cipher/plugins/` ili `~/.config/opencode/plugins/`. Također možete učitati dodatke iz npm-a preko opcije `plugin`.
...
Ovo je korisno kada želite da ograničite Cipher da koristi samo određene provajdere umjesto da ih onemogućavate jednog po jednog.
