---
title: Serwer
description: Interakcja z serwerem Cipher za pośrednictwem protokołu HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

Komenda `Cipher serve` uruchamia bezgłowy serwer HTTP, który udostępnia punkt końcowy OpenAPI, z którego może korzystać klient Cipher.
...
| `--mdns-domain` | Niestandardowa nazwa domeny dla usługi mDNS  | `Cipher.local` |
...
Ustaw `CIPHER_SERVER_PASSWORD`, aby chronić serwer za pomocą podstawowego uwierzytelniania HTTP. Domyślną nazwą użytkownika jest `Cipher` lub ustaw `CIPHER_SERVER_USERNAME`, aby ją zastąpić. Dotyczy to zarówno `Cipher serve`, jak i `Cipher web`.
...
Kiedy uruchomisz `Cipher`, uruchomi się TUI i serwer. Gdzie jest TUI
...
:::tip
Użyj serwera Cipher do programowej interakcji z Cipher.
:::

Ta architektura umożliwia obsługę wielu klientów przez Cipher i programową interakcję z kodem otwartym.

Możesz uruchomić `Cipher serve`, aby uruchomić samodzielny serwer. Jeśli masz
Cipher TUI działa, `Cipher serve` uruchomi nowy serwer.
...
Punktu końcowego [`/tui`](#TUI) można użyć do sterowania TUI przez serwer. Można na przykład wstępnie wypełnić lub uruchomić monit. Ta konfiguracja jest używana przez wtyczki Cipher [IDE](/docs/ide).
...
Serwer Cipher udostępnia następujące interfejsy API.
