...
export function FormatError(input: unknown): string | undefined {
  ...
  if (NamedError.hasName(input, "MCPFailed")) {
    ...
    return `MCP server "${data}" failed. Note, cipher does not support MCP authentication yet.`
  }
  ...
  if (providerModelNotFound) {
    ...
    return [
      ...
      `Try: \`cipher models\` to list available models`,
  ...
  if (remoteAuth) {
    ...
    return [
      ...
      ...(url ? [`Run \`cipher auth login ${url}\` to re-authenticate.`] : []),
    ].join("\n")
  }
