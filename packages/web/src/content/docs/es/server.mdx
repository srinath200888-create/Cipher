---
title: Servidor
description: Interactuar con el servidor Cipher a través de HTTP.
---

import config from "../../../../config.mjs"
export const typesUrl = `${config.github}/blob/dev/packages/sdk/js/src/gen/types.gen.ts`

El comando `Cipher serve` ejecuta un servidor HTTP sin cabeza que expone un punto final OpenAPI que un cliente Cipher puede usar.
...
| `--mdns-domain` | Nombre de dominio personalizado para el servicio mDNS | `Cipher.local` |
...
Configure `CIPHER_SERVER_PASSWORD` para proteger el servidor con autenticación básica HTTP. El nombre de usuario predeterminado es `Cipher`, o configure `CIPHER_SERVER_USERNAME` para anularlo. Esto se aplica tanto a `Cipher serve` como a `Cipher web`.
...
Cuando ejecuta `Cipher`, inicia un TUI y un servidor. Donde el TUI es el
...
:::tip
Utilice el servidor Cipher para interactuar con Cipher mediante programación.
:::

Esta arquitectura permite que Cipher admita múltiples clientes y le permite interactuar con Cipher mediante programación.

Puede ejecutar `Cipher serve` para iniciar un servidor independiente. Si tienes el
Cipher TUI ejecutándose, `Cipher serve` iniciará un nuevo servidor.
...
El punto final [`/tui`](#tui) se puede utilizar para conducir el TUI a través del servidor. Por ejemplo, puede completar previamente o ejecutar un mensaje. Esta configuración es utilizada por los complementos Cipher [IDE](/docs/ide).
...
El servidor Cipher expone las siguientes API.
