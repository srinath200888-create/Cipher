...
import { PromptInput } from "@cipher-ai/schema/prompt-input"
import { Session } from "@cipher-ai/schema/session"
import { Project } from "@cipher-ai/schema/project"
import { AbsolutePath, NonNegativeInt, PositiveInt, RelativePath, statics } from "@cipher-ai/schema/schema"
...
import { SessionEvent } from "@cipher-ai/schema/session-event"
    ...
    .add(
      HttpApiEndpoint.get("session.active", "/api/session/active", {
        ...
        OpenApi.annotations({
          ...
          description:
            "Retrieve foreground Session drains currently owned by this cipher process. Sessions absent from the result are inactive.",
        }),
      ),
    )
    .add(
      ...
      HttpApiEndpoint.post("session.interrupt", "/api/session/:sessionID/interrupt", {
        ...
        .annotateMerge(
          OpenApi.annotations({
            ...
            description: "Interrupt active execution owned by this cipher process. Idle interruption is a no-op.",
          }),
        ),
    )
    .add(
