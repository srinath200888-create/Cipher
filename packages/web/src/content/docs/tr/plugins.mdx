---
title: Eklentiler
description: cipher'u genişletmek için kendi eklentilerinizi yazın.
---

Eklentiler, çeşitli olaylara bağlanarak ve davranışı özelleştirerek cipher'u genişletmenize olanak tanır. Yeni özellikler eklemek, harici hizmetlerle entegrasyon sağlamak veya cipher'un varsayılan davranışını değiştirmek için eklentiler oluşturabilirsiniz.
...
- `.cipher/plugins/` - Proje düzeyinde eklentiler
...
1. Global config (`~/.config/opencode/cipher.json`)
2. Project config (`cipher.json`)
3. Global eklenti dizini (`~/.config/opencode/plugins/`)
4. Proje eklenti dizini (`.cipher/plugins/`)
...
Cipher bunları yüklemek için başlangıçta `bun install` komutunu çalıştırır. Eklentileriniz ve araçlarınız daha sonra bunları içe aktarabilir.
...
- `client`: Yapay zeka ile etkileşim kurmak için bir Cipher SDK istemcisi.
...
cipher'u genişletmek için kullanabileceğiniz bazı eklenti örneklerini burada bulabilirsiniz.
...
:::note
Cipher masaüstü uygulamasını kullanıyorsanız yanıt hazır olduğunda veya oturum hataları oluştuğunda otomatik olarak sistem bildirimleri gönderebilir.
:::
...
cipher'un `.env` dosyalarını okumasını önleyin:
...
Eklentiler ayrıca cipher'a özel araçlar da ekleyebilir:
...
`tool` yardımcısı, cipher'un çağırabileceği özel bir araç oluşturur. Bir Zod şeması işlevini alır ve aşağıdakileri içeren bir araç tanımı döndürür:

- `description`: Araç ne yapar?
- `args`: Aracın argümanları için Zod şeması
- `execute`: Araç çağrıldığında çalışan fonksiyon

Özel araçlarınız, yerleşik araçların yanı sıra cipher'a da hazır olacaktır.
