...
import { type Session } from "@cipher-ai/sdk/v2/client"
import {
  childSessionOnPath,
  closeHomeProject,
  displayName,
...
describe("layout deep links", () => {
  test("parses open-project deep links", () => {
    expect(parseDeepLink("cipher://open-project?directory=/tmp/demo")).toBe("/tmp/demo")
  })

  test("ignores non-project deep links", () => {
    expect(parseDeepLink("cipher://other?directory=/tmp/demo")).toBeUndefined()
    expect(parseDeepLink("https://example.com")).toBeUndefined()
  })

  test("ignores malformed deep links safely", () => {
    expect(() => parseDeepLink("cipher://open-project/%E0%A4%A%")).not.toThrow()
    expect(parseDeepLink("cipher://open-project/%E0%A4%A%")).toBeUndefined()
  })
  ...
  test("ignores open-project deep links without directory", () => {
    expect(parseDeepLink("cipher://open-project")).toBeUndefined()
    expect(parseDeepLink("cipher://open-project?directory=")).toBeUndefined()
  })

  test("collects only valid open-project directories", () => {
    const result = collectOpenProjectDeepLinks([
      "cipher://open-project?directory=/a",
      "cipher://other?directory=/b",
      "cipher://open-project?directory=/c",
    ])
    expect(result).toEqual(["/a", "/c"])
  })

  test("parses new-session deep links with optional prompt", () => {
    expect(parseNewSessionDeepLink("cipher://new-session?directory=/tmp/demo")).toEqual({ directory: "/tmp/demo" })
    expect(parseNewSessionDeepLink("cipher://new-session?directory=/tmp/demo&prompt=hello%20world")).toEqual({
      directory: "/tmp/demo",
      prompt: "hello world",
    })
  })
  ...
  test("ignores new-session deep links without directory", () => {
    expect(parseNewSessionDeepLink("cipher://new-session")).toBeUndefined()
    expect(parseNewSessionDeepLink("cipher://new-session?directory=")).toBeUndefined()
  })

  test("collects only valid new-session deep links", () => {
    const result = collectNewSessionDeepLinks([
      "cipher://new-session?directory=/a",
      "cipher://open-project?directory=/b",
      "cipher://new-session?directory=/c&prompt=ship%20it",
    ])
  ...
  test("drains global deep links once", () => {
    const target = {
      __CIPHER__: {
        deepLinks: ["cipher://open-project?directory=/a"],
      },
    } as unknown as Window & { __CIPHER__?: { deepLinks?: string[] } }

    expect(drainPendingDeepLinks(target)).toEqual(["cipher://open-project?directory=/a"])
