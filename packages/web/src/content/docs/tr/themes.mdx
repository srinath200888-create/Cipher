...
Cipher ile birden fazla yerleşik tema arasından seçim yapabilir, terminal temanıza uyum sağlayan bir tema kullanabilir veya kendi özel temanızı tanımlayabilirsiniz.

Varsayılan olarak Cipher kendi `Cipher` temasını kullanır.
...
Cipher birden fazla yerleşik temayla gelir.
...
- cipher'un terminal görünümüyle birebir uyumlu olmasını isteyenler
...
Cipher, kullanıcıların kolayca tema oluşturup özelleştirebilmesi için esnek bir JSON tabanlı tema sistemi sunar.
...
3. **Proje kök dizini** - `<project-root>/.cipher/themes/*.json`
4. **Geçerli çalışma dizini** - `./.cipher/themes/*.json`
