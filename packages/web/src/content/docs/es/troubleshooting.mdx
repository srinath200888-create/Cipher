...
Para depurar problemas con Cipher, comience verificando los registros y los datos locales que almacena en el disco.
...
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\cipher\log`
...
Puede configurar el nivel de registro con la opción de línea de comandos `--log-level` para obtener información de depuración más detallada. Por ejemplo, `Cipher --log-level DEBUG`.

---

## Almacenamiento

Cipher almacena datos de sesión y otros datos de aplicaciones en el disco en:

- **macOS/Linux**: `~/.local/share/opencode/`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.local\share\Cipher`
...
Cipher Desktop ejecuta un servidor Cipher local (el sidecar `cipher-cli`) en segundo plano. La mayoría de los problemas se deben a un complemento que no funciona correctamente, una memoria caché dañada o una mala configuración del servidor.
...
- Solo macOS: menú `Cipher` -> **Recargar vista web** (ayuda si la interfaz de usuario está en blanco/congelada).
...
- **macOS/Linux**: `~/.config/opencode/cipher.jsonc` (o `~/.config/opencode/cipher.json`)
- **macOS/Linux** (instalaciones anteriores): `~/.local/share/opencode/cipher.jsonc`
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\cipher\cipher.jsonc`
...
Cipher también puede cargar complementos locales desde el disco. Quítelos temporalmente del camino (o cambie el nombre de la carpeta) y reinicie la aplicación de escritorio:

- **Complementos globales**
  - **macOS/Linux**: `~/.config/opencode/plugins/`
  - **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.config\cipher\plugins`
- **Complementos de proyecto** (solo si usas la configuración por proyecto)
  - `<your-project>/.cipher/plugins/`
...
Si deshabilitar los complementos no ayuda (o la instalación de un complemento está bloqueada), borre el caché para que Cipher pueda reconstruirlo.

1. Salga de Cipher Desktop por completo.
2. Elimine el directorio de caché:

- **macOS**: Buscador -> `Cmd+Shift+G` -> pegar `~/.cache/cipher`
- **Linux**: eliminar `~/.cache/cipher` (o ejecutar `rm -rf ~/.cache/cipher`)
- **Windows**: Presione `WIN+R` y pegue `%USERPROFILE%\.cache\Cipher`

3. Reinicie el escritorio Cipher.

---

### Solucionar problemas de conexión del servidor

Cipher Desktop puede iniciar su propio servidor local (predeterminado) o conectarse a la URL de un servidor que haya configurado.
...
Si su `cipher.json(c)` contiene una sección `server`, elimínela temporalmente y reinicie la aplicación de escritorio.

#### Verificar variables de entorno

Si tiene `CIPHER_PORT` configurado en su entorno, la aplicación de escritorio intentará usar ese puerto para el servidor local.

- Desarme `CIPHER_PORT` (o elija un puerto libre) y reinicie.
...
En Windows, el escritorio Cipher requiere Microsoft Edge **WebView2 Runtime**. Si la aplicación se abre en una ventana en blanco o no se inicia, instale/actualice WebView2 e inténtelo nuevamente.

---

### Windows: Problemas generales de rendimiento

Si tiene un rendimiento lento, problemas de acceso a archivos o problemas de terminal en Windows, intente usar [WSL (Windows Subsistema para Linux)](/docs/windows-wsl). WSL proporciona un entorno Linux que funciona de manera más fluida con las funciones de Cipher.

---

### Notificaciones que no se muestran

Cipher Desktop solo muestra notificaciones del sistema cuando:

- las notificaciones están habilitadas para Cipher en la configuración de su sistema operativo, y
...
1. Salga del escritorio Cipher.
2. Busque y elimine estos archivos (se encuentran en el directorio de datos de la aplicación de escritorio Cipher):

- `Cipher.settings.dat` (URL del servidor predeterminado de escritorio)
- `Cipher.global.dat` y `Cipher.workspace.*.dat` (estado de la interfaz de usuario como servidores/proyectos recientes)
...
2. **Únete a nuestro Discord**
   ...
   [**Cipher.ai/discord**](https://opencode.ai/discord)
...
3. Asegúrese de tener la última versión con `Cipher upgrade`
...
Para saber a qué modelos tiene acceso, ejecute `Cipher models`
...
2. Si el problema persiste, intente borrar la configuración almacenada:
   ...
   ```bash
   rm -rf ~/.local/share/cipher
   ```

   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.local\share\Cipher`
...
Si encuentra errores de llamada API, esto puede deberse a paquetes de proveedores obsoletos. Cipher instala dinámicamente paquetes de proveedores (OpenAI, Anthropic, Google, etc.) según sea necesario y los almacena en caché localmente.
...
1. Borre la caché del paquete del proveedor:
   ...
   En Windows, presione `WIN+R` y elimine: `%USERPROFILE%\.cache\Cipher`

2. Reinicie Cipher para reinstalar los paquetes del proveedor más recientes.

Esto obligará a Cipher a descargar las versiones más recientes de los paquetes del proveedor, lo que a menudo resuelve problemas de compatibilidad con los parámetros del modelo y los cambios de API.
...
Cipher detectará si estás usando Wayland y prefieres `wl-clipboard`; de lo contrario, intentará encontrar herramientas del portapapeles en el orden de: `xclip` y `xsel`.
