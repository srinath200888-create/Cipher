---
title: Supporto ACP
description: Usa Cipher in qualunque editor compatibile con ACP.
---

Cipher supporta [Agent Client Protocol](https://agentclientprotocol.com) (ACP), permettendoti di usarlo direttamente in editor e IDE compatibili.
...
Per usare Cipher via ACP, configura l'editor in modo che esegua il comando `Cipher acp`.

Il comando avvia Cipher come sottoprocesso compatibile con ACP che comunica con l'editor via JSON-RPC su stdio.
...
Per aprirlo, seleziona il nuovo agente 'cipher' nel selettore agenti di AI Chat.
...
Per usare Cipher come agente ACP in [CodeCompanion.nvim](https://github.com/olimorris/codecompanion.nvim), aggiungi quanto segue alla config di Neovim:
...
Questa config imposta CodeCompanion per usare Cipher come agente ACP per la chat.

Se devi passare variabili d'ambiente (come `CIPHER_API_KEY`), vedi [Configuring Adapters: Environment Variables](https://codecompanion.olimorris.dev/getting-started#setting-an-api-key) nella documentazione di CodeCompanion.nvim.

## Supporto

Cipher funziona via ACP come nel terminale. Tutte le funzionalita sono supportate:
:::note
...
- Server MCP configurati nella tua configurazione Cipher
