        ...
        Effect.tryPromise({
          try: () =>
            createCliRenderer({
              ...
              useMouse: !Flag.CIPHER_DISABLE_MOUSE && input.config.mouse,
              consoleOptions: {
                ...
                <ErrorBoundary fallback={(error, reset) => <ErrorComponent error={error} reset={reset} mode={mode} />}>
                  ...
                  >
                    ...
                    >
                      <TuiStartupProvider
                        value={{
                          initialRoute: process.env.CIPHER_ROUTE ? JSON.parse(process.env.CIPHER_ROUTE) : undefined,
                          skipInitialLoading: Boolean(process.env.CIPHER_FAST_BOOT),
                        }}
                      >
                        <ClipboardProvider>
...
function App(props: { onSnapshot?: () => Promise<string[]>; pluginHost: TuiPluginHost }) {
  ...
  const offSelectionKeys = keymap.intercept(
    "key",
    ({ event }) => {
      if (!Flag.CIPHER_EXPERIMENTAL_DISABLE_COPY_ON_SELECT) return
  ...
  createEffect(() => {
    if (!terminalTitleEnabled() || Flag.CIPHER_DISABLE_TERMINAL_TITLE) return

    if (route.data.type === "home") {
      renderer.setTerminalTitle("cipher")
      return
    }
  ...
  const appCommands = createMemo(() =>
    [
      ...
      },
      {
        ...
        category: "Workspace",
        hidden: !Flag.CIPHER_EXPERIMENTAL_WORKSPACES,
  ...
  event.on("installation.update-available", async (evt) => {
    ...
    await DialogAlert.show(
      dialog,
      "Update Complete",
      `Successfully updated to cipher v${result.data.version}. Please restart the application.`,
    )
  ...
  return (
    <box
      ...
      onMouseUp={
        !Flag.CIPHER_EXPERIMENTAL_DISABLE_COPY_ON_SELECT
          ? () => Selection.copy(renderer, toast, clipboard)
          : undefined
      }
    >
      <Show when={Flag.CIPHER_SHOW_TTFD}>
        <TimeToFirstDraw />
      </Show>
      <Show when={ready()}>
