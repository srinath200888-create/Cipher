---
title: Wtyczki
description: Napisz własną wtyczkę, aby rozszerzyć Cipher.
---

Wtyczki umożliwiające wyłączenie Cipher poprzez podjęcie różnych zdarzeń i zachowanie. Można utworzyć wtyczkę, aby sprawdzić nowe funkcje, integrować się z usługami lub sprawdzaniem zachowania Cipher.
...
- `.cipher/plugins/` - Wtyczki na poziomie projektu
...
1. Konfiguracja globalna (`~/.config/opencode/cipher.json`)
2. Konfiguracja projektu (`cipher.json`)
3. Globalny katalog wtyczek (`~/.config/opencode/plugins/`)
4. Katalog wtyczek projektu (`.cipher/plugins/`)
...
Cipher uruchomienie `bun install` przy uruchomieniu, aby je uruchomić. Twoje dodatkowe funkcje będą dostępne po zaimportowaniu.
...
- `client`: Klient SDK z Cipher do sztucznej inteligencji.
...
Oto kilka dostępnych wtyczek, które można udostępnić do Cipher.
...
:::note
Jeśli korzystasz z aplikacji komputerowej Cipher, może ona automatycznie wysłać wiadomość systemową, gdy odpowiedź jest gotowa lub gdy wystąpią błędy sesji.
:::
...
Zablokuj Cipher czytanie plików `.env`:
...
Wtyczki mogą również dodawać niestandardowe narzędzia do Cipher:
...
Pomocnik `tool` tworzy narzędzie użytkowe, które może wywołać kod Cipher. Pobierane schematu Zoda i narzędzie wyszukiwania za pomocą:
