...
import { SessionV1 } from "@cipher-ai/core/v1/session"
import { ModelsDev } from "@cipher-ai/core/models-dev"
...
import { LLMEvent, LLMResponse } from "@cipher-ai/llm"
import { RequestExecutor } from "@cipher-ai/llm/route"
...
function decodeRecordOpenAIOAuth() {
  const value = process.env.CIPHER_RECORD_OPENAI_AUTH
...
const RECORDED_SCENARIOS = [
  {
    ...
    tags: ["cipher", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CIPHER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.openai,
        name: "OpenAI",
        ...
        model,
        options: {
          apiKey: envValue("CIPHER_RECORD_OPENAI_API_KEY", "OPENAI_API_KEY") ?? "fixture-openai-key",
  ...
  },
  {
    ...
    protocol: "openai-responses",
    tags: ["cipher", "native", "oauth", "tool-loop"],
  ...
  },
  {
    id: "opencode-proxy",
    name: "OpenCode proxy",
    providerID: ProviderV2.ID.cipher,
    modelID: "gpt-5.2-codex",
    cassette: "session/native-zen-tool-loop",
    protocol: "openai-responses",
    tags: ["cipher", "zen", "native", "tool-loop"],
    canRecord: () => Boolean(process.env.CIPHER_RECORD_CONSOLE_TOKEN && process.env.CIPHER_RECORD_ZEN_ORG_ID),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.cipher,
        name: "OpenCode Zen",
        env: ["CIPHER_CONSOLE_TOKEN"],
        npm: "@ai-sdk/openai-compatible",
        api: zenURL(process.env.CIPHER_RECORD_ZEN_CONNECTION ?? "fixture"),
        model,
        options: {
          apiKey: process.env.CIPHER_RECORD_CONSOLE_TOKEN ?? "fixture-console-token",
          headers: { "x-org-id": process.env.CIPHER_RECORD_ZEN_ORG_ID ?? "fixture-org" },
        },
      }),
  },
  {
    ...
    tags: ["cipher", "native", "tool-loop"],
    canRecord: () => Boolean(envValue("CIPHER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY")),
    config: (model) =>
      providerConfig({
        providerID: ProviderV2.ID.anthropic,
        name: "Anthropic",
        ...
        model,
        options: {
          apiKey: envValue("CIPHER_RECORD_ANTHROPIC_API_KEY", "ANTHROPIC_API_KEY") ?? "fixture-anthropic-key",
...
const selectedScenarios = new Set(
  (envValue("CIPHER_RECORDED_SCENARIO", "RECORDED_PROVIDER") ?? "")
    .split(",")
...
const recordError = (scenario: RecordedScenario) =>
  scenario.id === "openai-oauth"
    ? "Set CIPHER_RECORD_OPENAI_AUTH to an OAuth auth JSON object in the recording environment."
...
const writeConfig = (directory: string, scenario: RecordedScenario, model: ModelsDev.Provider["models"][string]) =>
  Effect.promise(() =>
    Bun.write(
      path.join(directory, "cipher.json"),
