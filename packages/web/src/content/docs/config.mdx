---
title: Config
description: Using the Cipher JSON config.
---

You can configure Cipher using a JSON config file.

---

## Format

Cipher supports both **JSON** and **JSONC** (JSON with Comments) formats.

```jsonc title="cipher.jsonc"
{
...
1. **Remote config** (from `.well-known/cipher`) - organizational defaults
2. **Global config** (`~/.config/opencode/cipher.json`) - user preferences
3. **Custom config** (`CIPHER_CONFIG` env var) - custom overrides
4. **Project config** (`cipher.json` in project) - project-specific settings
5. **`.opencode` directories** - agents, commands, plugins
6. **Inline config** (`CIPHER_CONFIG_CONTENT` env var) - runtime overrides
...
:::note
The `.opencode` and `~/.config/cipher` directories use **plural names** for subdirectories: `agents/`, `commands/`, `modes/`, `plugins/`, `skills/`, `tools/`, and `themes/`. Singular names (e.g., `agent/`) are also supported for backwards compatibility.
:::
...
Organizations can provide default configuration via the `.well-known/cipher` endpoint. This is fetched automatically when you authenticate with a provider that supports it.
...
Place your global Cipher config in `~/.config/opencode/cipher.json`. Use global config for user-wide server/runtime preferences like providers, models, and permissions.
...
Add `cipher.json` in your project root. Project config has the highest precedence among standard config files - it overrides both global and remote configs.
...
When Cipher starts up, it first looks for a config file in the current directory, then traverses up to the nearest Git directory.
...
Specify a custom config file path using the `CIPHER_CONFIG` environment variable.

```bash
export CIPHER_CONFIG=/path/to/my/custom-config.json
Cipher run "Hello world"
```
...
Specify a custom config directory using the `CIPHER_CONFIG_DIR`
...
```bash
export CIPHER_CONFIG_DIR=/path/to/my/config-directory
Cipher run "Hello world"
```
...
Drop an `cipher.json` or `cipher.jsonc` file in the system managed config directory:

| Platform | Path                                     |
| -------- | ---------------------------------------- |
| macOS    | `/Library/Application Support/opencode/` |
| Linux    | `/etc/opencode/`                         |
| Windows  | `%ProgramData%\Cipher`                 |

These directories require admin/root access to write, so users cannot modify them.

#### macOS managed preferences

On macOS, Cipher reads managed preferences from the `ai.opencode.managed` preference domain. Deploy a `.mobileconfig` via MDM (Jamf, Kandji, FleetDM) and the settings are enforced automatically.

Cipher checks these paths:

1. `/Library/Managed Preferences/<user>/ai.opencode.managed.plist`
2. `/Library/Managed Preferences/ai.opencode.managed.plist`

The plist keys map directly to `cipher.json` fields. MDM metadata keys (`PayloadUUID`, `PayloadType`, etc.) are stripped automatically.

**Creating a `.mobileconfig`**

Use the `ai.opencode.managed` PayloadType. The Cipher config keys go directly in the payload dict:
...
The server/runtime config schema is defined in [**`Cipher.ai/config.json`**](https://opencode.ai/config.json).

TUI config uses [**`Cipher.ai/tui.json`**](https://opencode.ai/tui.json).
...
Use `CIPHER_TUI_CONFIG` to point to a custom TUI config file.

Set `attention.enabled` to turn on TUI desktop notifications and sounds. See [TUI attention](/docs/tui#attention).

Legacy `theme`, `keybinds`, and `tui` keys in `cipher.json` are deprecated and automatically migrated when possible.

---

### Server

You can configure server settings for the `Cipher serve` and `Cipher web` commands through the `server` option.
...
- `mdns` - Enable mDNS service discovery. This allows other devices on the network to discover your Cipher server.
- `mdnsDomain` - Custom domain name for mDNS service. Defaults to `Cipher.local`. Useful for running multiple instances on the same network.
...
If not specified, Cipher will automatically discover and use a sensible default based on your operating system (e.g. `pwsh` or `cmd.exe` on Windows, `/bin/zsh` or `/bin/bash` on macOS/Linux). You can provide an absolute path or a short name.
...
You can configure the providers and models you want to use in your Cipher config through the `provider`, `model` and `small_model` options.
...
The `small_model` option configures a separate model for lightweight tasks like title generation. By default, Cipher tries to use a cheaper model if one is available from your provider, otherwise it falls back to your main model.
...
Use the `experimental.policies` option to allow or deny Cipher actions on configured resources. Currently, policies can control which providers Cipher may use.
...
Cipher normalizes image attachments before sending them to the model. By default, images are resized when they exceed `2000x2000` pixels or `5242880` base64 bytes.
...
If an image still cannot fit after resizing, Cipher omits oversized tool-result images or fails oversized user-provided images with an image size error.
...
You can also define agents using markdown files in `~/.config/opencode/agents/` or `.cipher/agents/`. [Learn more here](/docs/agents).
...
The default agent must be a primary agent (not a subagent). This can be a built-in agent like `"build"` or `"plan"`, or a [custom agent](/docs/agents) you've defined. If the specified agent doesn't exist or is a subagent, Cipher will fall back to `"build"` with a warning.

This setting applies across all interfaces: TUI, CLI (`Cipher run`), desktop app, and GitHub Action.
...
You can also define commands using markdown files in `~/.config/opencode/commands/` or `.cipher/commands/`. [Learn more here](/docs/commands).
...
Cipher uses snapshots to track file changes during agent operations, enabling you to undo and revert changes within a session. Snapshots are enabled by default.
...
Cipher will automatically download any new updates when it starts up. You can disable this with the `autoupdate` option.
...
By default, Cipher **allows all operations** without requiring explicit approval. You can change this using the `permission` option.
...
[Plugins](/docs/plugins) extend Cipher with custom tools, hooks, and integrations.

Place plugin files in `.cipher/plugins/` or `~/.config/opencode/plugins/`. You can also load plugins from npm through the `plugin` option.

```json title="cipher.json"
{
  "$schema": "https://opencode.ai/config.json",
  "plugin": ["cipher-helicone-session", "@my-org/custom-plugin"]
}
```
...
This is useful when you want to restrict Cipher to only use specific providers rather than disabling them one by one.
