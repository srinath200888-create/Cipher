...
import { SessionMessageTable } from "@cipher-ai/core/session/sql"
...
import { Shell } from "@cipher-ai/core/shell"
...
import { LocationServiceMap, locationServiceMapLayer } from "@cipher-ai/core/location-services"
...
it.instance("does not loop empty assistant turns for a simple reply", () =>
  Effect.gen(function* () {
    ...
    yield* llm.text("packages/cipher/src/session/processor.ts")
  ...
  () =>
    Effect.gen(function* () {
      ...
      const other = yield* prompt.prompt({
        sessionID: session.id,
        agent: "build",
        model: { providerID: ProviderV2.ID.make("cipher"), modelID: ModelV2.ID.make("kimi-k2.5-free") },
        noReply: true,
        parts: [{ type: "text", text: "hello" }],
      })
