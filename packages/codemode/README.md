...
```ts
import { CodeMode, Tool } from "@cipher-ai/codemode"
...
```ts
import { CodeMode, OpenAPI } from "@cipher-ai/codemode"
...
const runtime = CodeMode.make({ tools: { Cipher: api.tools } })
...
The catalog-entry budget defaults to 2,000 estimated tokens (characters / 4, the same heuristic Cipher uses). It applies only to full tool entries shown in the catalog; fixed instructions and namespace summaries are not counted. Override it when constructing a runtime:
...
```ts
import { toolError } from "@cipher-ai/codemode"
