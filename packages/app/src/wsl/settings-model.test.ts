...
describe("WSL server settings presentation", () => {
  ...
  test("offers install and update only when cipher needs attention", () => {
  ...
  test("does not report ready when cipher is present but cannot run", () => {
    const model = addServerViewModel({
      state: {
        ...readyWslState,
        installed: [{ name: "Debian", version: 2, isDefault: true }],
        ...
        opencodeChecks: {
          Debian: {
            distro: "Debian",
            resolvedPath: "/home/me/.cipher/bin/opencode",
            version: null,
            expectedVersion: "1.2.3",
            matchesDesktop: null,
    ...
    expect(model.primaryButton.action).toBe("install-cipher")
  })
