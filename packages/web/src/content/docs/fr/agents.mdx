...
Il existe deux types d'agents dans Cipher ; agents primaires et sous-agents.
...
Cipher est livré avec deux agents primaires intégrés, **Build** et **Plan**.
...
Cipher est livré avec trois sous-agents intégrés, **General**, **Explore** et **Scout**. Nous les verrons ci-dessous.

---

## Agents intégrés

Cipher est livré avec deux agents primaires intégrés et trois sous-agents intégrés.
...
Configurez les agents dans votre fichier de configuration `cipher.json` :
...
Si aucune température n'est spécifiée, Cipher utilise les valeurs par défaut spécifiques au modèle ; généralement 0 pour la plupart des modèles, 0,55 pour les modèles Qwen.
...
Ce chemin est relatif à l'emplacement du fichier de configuration. Cela fonctionne donc à la fois pour la configuration globale Cipher et pour la configuration spécifique au projet.
...
L'ID de modèle dans votre configuration Cipher utilise le format `provider/model-id`. Par exemple, si vous utilisez [Cipher Zen](/docs/zen), vous utiliserez `cipher/gpt-5.1-codex` pour GPT 5.1 Codex.
...
:::tip
Exécutez `Cipher models` pour voir une liste des modèles disponibles.
:::
