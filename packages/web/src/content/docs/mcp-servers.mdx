...
You can add external tools to Cipher using the _Model Context Protocol_, or MCP. Cipher supports both local and remote servers.
...
You can define MCP servers in your [Cipher Config](https://opencode.ai/docs/config/) under `mcp`. Add each MCP with a unique name. You can refer to that MCP by name when prompting the LLM.
...
Organizations can provide default MCP servers via their `.well-known/cipher` endpoint. These servers may be disabled by default, allowing users to opt-in to the ones they need.
...
Cipher automatically handles OAuth authentication for remote MCP servers. When a server requires authentication, Cipher will:
...
If the server requires authentication, Cipher will prompt you to authenticate when you first try to use it. If not, you can [manually trigger the flow](#authenticating) with `Cipher mcp auth <server-name>`.
...
The `mcp auth` command will open your browser for authorization. After you authorize, Cipher will store the tokens securely in `~/.local/share/opencode/mcp-auth.json`.
...
Your MCPs are available as tools in Cipher, alongside built-in tools. So you can manage them through the Cipher config like any other tool.
...
This will open a browser window to complete the OAuth flow and connect Cipher to your Sentry account.
