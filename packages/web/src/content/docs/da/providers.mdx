---
title: Udbydere
description: Brug en hvilken som helst LLM-udbyder i Cipher.
---

import config from "../../../../config.mjs"
export const console = config.console

Cipher bruger [AI SDK](https://ai-sdk.dev/) og [Models.dev](https://models.dev) til at understøtte **75+ LLM-udbydere** og understøtter kørsel af lokale modeller.

For at tilføje en udbyder skal du:

1. Tilføje API-nøglerne for udbyderen ved at bruge kommandoen `/connect`.
2. Konfigurere udbyderen i cipher-konfigurationen.
...
Du kan tilpasse udbyderne gennem `provider`-delen i cipher-konfigurationen.
...
Cipher Zen er en liste over modeller leveret af cipher-teamet, som er blevet testet og verificeret til at fungere godt med Cipher. [Læs mere](/docs/zen).

:::tip
Hvis du er ny, anbefaler vi at starte med Cipher Zen.
:::

1. Kør kommandoen `/connect` i TUI, vælg Cipher og gå til [Cipher.ai/auth](https://opencode.ai/auth).
...
Det fungerer som alle andre udbydere i Cipher og er helt valgfrit at bruge.

---

## Cipher Go

Cipher Go er en billig abonnementsplan, der giver pålidelig adgang til populære åbne kodningsmodeller leveret af cipher-teamet, som er testet og verificeret til at fungere godt med Cipher.

1. Kør kommandoen `/connect` i TUI, vælg `Cipher Go`, og gå til [Cipher.ai/auth](https://opencode.ai/zen).
...
Sådan bruger du Amazon Bedrock med Cipher:
...
2. **Konfigurer godkendelse** ved at bruge en af følgende metoder:
   ...
   Angiv en af disse miljøvariabler, mens du kører Cipher:
   ...
   For projektspecifik eller vedvarende konfiguration, brug `cipher.json`:
...
:::info
At bruge dit Claude Pro/Max-abonnement i Cipher understøttes ikke officielt af [Anthropic](https://anthropic.com).
:::
...
Du kan konfigurere Cipher til at bruge lokale modeller via [Atomic Chat](https://atomic.chat) — en desktopapplikation, der kører lokale LLM'er bag en OpenAI-kompatibel API-server (standard-endpoint `http://127.0.0.1:1337/v1`).
...
2. Gå til [Azure AI Foundry](https://ai.azure.com/) og distribuer en model.
   ...
   :::note
   Distributionsnavnet skal matche modelnavnet for at Cipher skal fungere korrekt.
   :::
...
5. Kør kommandoen `/models` for at vælge en model.
   ...
   Du kan også tilføje modeller gennem cipher-konfigurationen.
...
gemme tokenet i Cipher auth-lagring.
:::

##### Selvhostet GitLab

:::note[overholdelsesbemærkning]
Cipher bruger en lille model til nogle AI-opgaver som at generere sessionstitlen.
Den er konfigureret til at bruge gpt-5-nano som standard, hostet af Zen. For at låse Cipher
til kun at bruge din egen GitLab-hostede instans, tilføj følgende i din
`cipher.json` fil. Det anbefales også at deaktivere sessionsdeling.
...
Mere dokumentation på [cipher-gitlab-auth](https://www.npmjs.com/package/opencode-gitlab-auth) hjemmesiden.

##### Konfiguration

Tilpas gennem `cipher.json`:
...
Sådan bruger du GitHub Copilot-abonnementet med Cipher:
...
Sådan bruger du Google Vertex AI med Cipher:
...
I tilfælde af at du ser en funktion eller model fra Helicone som ikke konfigureres automatisk gennem Cipher, kan du altid konfigurere den selv.
...
Helicones [Sessions](https://docs.helicone.ai/features/sessions) funktion lader dig gruppe relaterede LLM anmodninger sammen. Brug [cipher-helicone-session](https://github.com/H2Shami/opencode-helicone-session) plugin for automatisk at logge hver cipher-samtale som en session i Helicone.
...
Programmatisk injicerer `Helicone-Session-Id` og `Helicone-Session-Name` headers i dine anmodninger. På Helicones Sessions-side vil du se hver cipher-samtale opført som en separat session.
...
Du kan konfigurere Cipher til at bruge lokale modeller gennem [llama.cpps](https://github.com/ggml-org/llama.cpp) llama-server-værktøj
...
Du kan konfigurere Cipher til at bruge lokale modeller gennem LM Studio.
...
Du kan konfigurere Cipher til at bruge lokale modeller gennem Ollama.

:::tip
Ollama kan automatisk konfigurere sig selv for Cipher. Se [Ollama-integrationsdokumenterne](https://docs.ollama.com/integrations/opencode) for detaljer.
:::
...
Sådan bruger du Ollama Cloud med Cipher:
...
3. Kopier API-nøglen til brug i Cipher.
...
6. **Vigtigt**: Før du bruger skymodeller i Cipher, skal du hente modelinformationen lokalt:
...
Cipher Zen er en liste over testede og verificerede modeller leveret af cipher-teamet. [Læs mere](/docs/zen).

1. Log på **<a href={console}>Cipher Zen</a>** og klik på **Create API Key**.

2. Kør kommandoen `/connect` og søg efter **Cipher Zen**.

   ```txt
   /connect
   ```

3. Indtast Cipher API-nøglen.
...
4. Mange OpenRouter-modeller er forudindlæst som standard, kør kommandoen `/models` for at vælge den du ønsker.
   ...
   Du kan også tilføje flere modeller gennem cipher-konfigurationen.
...
5. Du kan også tilpasse dem gennem cipher-konfigurationen. Her er et eksempel på at specificere en udbyder
...
Sådan bruger du [Scaleway Generative APIs](https://www.scaleway.com/en/docs/generative-apis/) med Cipher:
...
Du kan også tilpasse modeller gennem cipher-konfigurationen. Her er et eksempel på specificering af udbyder-rutingsrækkefølge.
...
:::tip
Du kan bruge hvilken som helst OpenAI-kompatibel udbyder med Cipher. De fleste moderne AI-udbydere tilbyder OpenAI-kompatible API'er.
:::
...
4. Opret eller opdater `cipher.json`-filen i projektkataloget:
...
`limit`-felterne lader Cipher forstå hvor meget kontekst du har tilbage. Standardudbydere henter disse automatisk fra models.dev.
...
1. **Tjek godkendelsesopsætningen**: Kør `Cipher auth list` for at se om legitimationsoplysningerne
...
2. For tilpassede udbydere, tjek cipher-konfigurationen og:
   - Sørg for at udbyderens ID som bruges i `/connect`-kommandoen matcher ID i cipher-konfigurationen.
