...
Sie können externe Tools zu Cipher hinzufügen, indem Sie das _Model Context Protocol_ oder MCP verwenden. Cipher unterstützt sowohl lokale als auch Remote-Server.
...
Sie können den MCP-Server in Ihrem [Cipher Config](https://opencode.ai/docs/config/) unter `mcp` definieren. Fügen Sie jedem MCP einen eindeutigen Namen hinzu. Sie können bei der Eingabe von LLM namentlich auf diesen MCP verweisen.
...
Organisationen können über ihren `.well-known/cipher`-Endpunkt Standard-MCP-Server bereitstellen. Diese Server sind möglicherweise standardmäßig deaktiviert, sodass Benutzer sich für den Server entscheiden können, die sie benötigen.
...
Cipher übernimmt automatisch die OAuth-Authentifizierung für Remote-MCP-Server. Wenn ein Server eine Authentifizierung erfordert, wird Cipher Folgendes tun:
...
Wenn der Server eine Authentifizierung erfordert, werden Sie von Cipher beim ersten Versuch, ihn zu verwenden, zur Authentifizierung autorisiert. Wenn nicht, können Sie [manually trigger the flow](#authenticating) mit `Cipher mcp auth <server-name>` verwenden.
...
Der Befehl `mcp auth` öffnet Ihren Browser zur Autorisierung. Nach der Autorisierung speichert Cipher die Token sicher in `~/.local/share/opencode/mcp-auth.json`.
...
Ihre MCPs sind neben integrierten Tools auch als Tools in Cipher verfügbar. Sie können sie also wie jedes andere Tool über die cipher-Konfiguration verwalten.
...
Dadurch wird ein Browserfenster geöffnet, um den OAuth-Ablauf abzuschließen und Cipher mit Ihrem Sentry-Konto zu verbinden.
