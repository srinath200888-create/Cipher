...
Cipher Go, popüler açık kodlama modellerine güvenilir erişim sağlayan düşük maliyetli bir aboneliktir — **ilk ayınız için 5$**, sonrasında **aylık 10$**.

Go, cipher'daki diğer sağlayıcılar gibi çalışır. Cipher Go'ya abone olur ve API anahtarınızı alırsınız. Bu **tamamen isteğe bağlıdır** ve cipher'u kullanmak için bunu kullanmanıza gerek yoktur.
...
Cipher Go, bu modellere **ilk ayınız için 5$**, ardından **aylık 10$** karşılığında erişmenizi sağlar.

---

## Nasıl çalışır?

Cipher Go, cipher'daki diğer sağlayıcılar gibi çalışır.

1. **<a href={console}>Cipher Zen</a>**'e giriş yapın, Go'ya abone olun ve API anahtarınızı kopyalayın.
2. TUI'de `/connect` komutunu çalıştırın, `Cipher Go`yu seçin ve API anahtarınızı yapıştırın.
3. Go üzerinden kullanabileceğiniz modellerin listesini görmek için TUI'de `/models` komutunu çalıştırın.

:::note
Her çalışma alanından yalnızca bir üye Cipher Go'ya abone olabilir.
:::
...
Cipher Go aşağıdaki limitleri içerir:
...
Cipher yapılandırmanızdaki [model id](/docs/config/#models) formatı `cipher-go/<model-id>` şeklindedir. Örneğin, Kimi K2.7 Code için yapılandırmanızda `cipher-go/kimi-k2.7-code` kullanmalısınız.
...
Cipher Go'yu şu amaçlarla oluşturduk:
...
4. Cipher ile diğer sağlayıcıları da kullanmanıza olanak tanıyarak **kilitlenme (vendor lock-in) olmamasını** sağlamak.
