...
Agent skills omogucavaju Cipher da pronade uputstva koja se mogu ponovo koristiti iz repozitorija ili home direktorija.
...
Cipher pretrazuje ove lokacije:

- Konfiguracija projekta: `.cipher/skills/<name>/SKILL.md`
...
Za projektne lokalne putanje, Cipher ide prema gore od trenutnog radnog direktorija dok ne dode do git worktree-ja.
Usput ucitava sve odgovarajuce `skills/*/SKILL.md` u `.cipher/` i odgovarajuce `.claude/skills/*/SKILL.md` ili `.agents/skills/*/SKILL.md`.
...
Kreirajte `.cipher/skills/git-release/SKILL.md` ovako:
...
Cipher navodi dostupne skills u opisu `skill` alata.
...
Kontrolisite kojim skills agenti mogu pristupiti pomocu dozvola baziranih na obrascima u `cipher.json`:
...
**Za ugradene agente** (u `cipher.json`):
