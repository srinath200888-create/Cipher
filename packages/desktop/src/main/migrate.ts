...
function tauriAppId() {
  return app.isPackaged ? TAURI_APP_IDS[CHANNEL] : "ai.cipher.desktop.dev"
}

// Migrate a single Tauri .dat file into the corresponding electron-store.
// `cipher.settings.dat` is special: it maps to the `cipher.settings` store
...
// renderer already passes via IPC (e.g. `"default.dat"`, `"cipher.global.dat"`).
function migrateFile(datPath: string, filename: string) {
  ...
  // cipher.settings.dat → the electron settings store ("cipher.settings").
  // All other .dat files keep their full filename as the store name so they match
  // what the renderer passes via IPC (e.g. "default.dat", "cipher.global.dat").
  const storeName = filename === "cipher.settings.dat" ? "cipher.settings" : filename
